...
class AIAPIV2Client:
    ...
    def __init__(self, base_url: str, auth_url: str = None, client_id: str = None, client_secret: str = None,
                 cert_file_path: str = None, key_file_path: str = None, token_creator: Callable[[], str] = None,
                 resource_group: str = None, read_timeout=Timeouts.READ_TIMEOUT.value,
                 connect_timeout=Timeouts.CONNECT_TIMEOUT.value,
        ...
        if not token_creator:
            if not (auth_url and client_id and client_secret):
                if not (auth_url and cert_file_path and key_file_path):
                    raise AIAPIAuthenticatorException(
                        error_message='Either token_creator or auth_url & client_id & client_secret or '
                                      'auth_url & client_id & cert_file_path & key_file_path should be provided')
                else:
                    token_creator = Authenticator(auth_url=auth_url, client_id=client_id, cert_file_path=cert_file_path,
                                                  key_file_path=key_file_path).get_token
            else:
                token_creator = Authenticator(auth_url=auth_url, client_id=client_id,
