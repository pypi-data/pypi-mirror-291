...
#====================================================================================================
    ...
    async def screenshot(file, location, duration, logs=True, extension=Scripted.DATA11):
        try:
            ...
            moonus = str(errors.stderr.decode('utf-8'))
            print(moonus) if logs else Scripted.DATA01
            return None
        except Exception as errors:
            print(errors) if logs else Scripted.DATA01
            return None
...
#====================================================================================================
    ...
    async def screenshots(file, location, numbers, duration, durations=5, images=None):
        ...
        if duration < durations:
            return imageess
        interval = await Media.interval(durations, duration, numbers)
        for looper in range(numbers):
            valuesom = interval[0]
            imagesve = await Media.screenshot(file, location, valuesom)
            if imagesve == None:
                continue
