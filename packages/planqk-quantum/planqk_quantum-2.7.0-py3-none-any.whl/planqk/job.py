...
class PlanqkBaseJob:
    ...
    def result(self) -> any:
        ...
        if status not in JOB_FINAL_STATES:
            status = self.wait_for_final_state()
        ...
        if status == JOB_STATUS.FAILED:
            if self._job_details.error_data is None:
                result_data = self._client.get_job_result(self._job_id)
    ...
    def wait_for_final_state(self, timeout: Optional[float] = None, wait: float = 5) -> JOB_STATUS:
        ...
        status = PlanqkBaseJob.status(self)
        while status not in JOB_FINAL_STATES:
            ...
            time.sleep(wait)
            status = PlanqkBaseJob.status(self) #TODO remove self_status
        return status
    
    def status(self, use_cached_value: bool = None) -> JOB_STATUS:
        """
        ...
        """
        if not use_cached_value or self._status not in JOB_FINAL_STATES:
    ...
    def _refresh(self, id: str = None):
        ...
        job_id = id if id is not None else self.id
