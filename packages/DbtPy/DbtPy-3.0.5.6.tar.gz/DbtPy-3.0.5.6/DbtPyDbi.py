#//////////////////////////////////////////////////////////////////////////
# Copyright 2017-2020 OpenInformix & GBase
#
...
"""
This module implements the Python DB API Specification v2.0 for GBase 8s database.
"""
...
import DbtPy
__version__ = DbtPy.__version__

# Constants for specifying database connection options.
SQL_ATTR_AUTOCOMMIT = DbtPy.SQL_ATTR_AUTOCOMMIT
#SQL_ATTR_CURRENT_SCHEMA = DbtPy.SQL_ATTR_CURRENT_SCHEMA
SQL_AUTOCOMMIT_OFF = DbtPy.SQL_AUTOCOMMIT_OFF
SQL_AUTOCOMMIT_ON = DbtPy.SQL_AUTOCOMMIT_ON
ATTR_CASE = DbtPy.ATTR_CASE
CASE_NATURAL = DbtPy.CASE_NATURAL
CASE_LOWER = DbtPy.CASE_LOWER
CASE_UPPER = DbtPy.CASE_UPPER
SQL_FALSE = DbtPy.SQL_FALSE
SQL_TRUE = DbtPy.SQL_TRUE
SQL_TABLE_STAT = DbtPy.SQL_TABLE_STAT
SQL_INDEX_CLUSTERED = DbtPy.SQL_INDEX_CLUSTERED
SQL_INDEX_OTHER = DbtPy.SQL_INDEX_OTHER
SQL_DBMS_VER = DbtPy.SQL_DBMS_VER
SQL_DBMS_NAME = DbtPy.SQL_DBMS_NAME
USE_WCHAR = DbtPy.USE_WCHAR
WCHAR_YES = DbtPy.WCHAR_YES
WCHAR_NO = DbtPy.WCHAR_NO
FIX_RETURN_TYPE = 1
...
class Error(exception):
    ...
    def __str__(self):
        ...
        return 'DbtPyDbi::'+str(self.__class__.__name__)+': '+str(self._message)
...
def connect(ConStr, user='', password='', host='', database='', conn_options=None):
    ...
    if password != '' and ConStr.find('PWD=') == -1:
        ConStr = ConStr + "PWD=" + password + ";"
    try:
        # conn = DbtPy.connect(ConStr, '', '', conn_options)
        conn = DbtPy.connect(ConStr, '', '')
        #DbtPy.set_option(conn, {SQL_ATTR_CURRENT_SCHEMA : user}, 1)
    except Exception as inst:
        raise _get_exception(inst)
...
class Connection(object):
    ...
    def __init__(self, conn_handler):
        """Constructor for Connection object. It takes DbtPy 
        ...
        self._cursor_list = []
        self.__dbms_name = DbtPy.get_db_info(conn_handler, SQL_DBMS_NAME)
        self.__dbms_ver = DbtPy.get_db_info(conn_handler, SQL_DBMS_VER)
    ...
    def close(self):
        ...
        try:
            if self.conn_handler is None:
                ...
                return_value = DbtPy.close(self.conn_handler)
        except Exception as inst:
            raise _get_exception(inst)
    ...
    def commit(self):
        ...
        """
        try:
            return_value = DbtPy.commit(self.conn_handler)
        except Exception as inst:
            raise _get_exception(inst)
        return return_value
    ...
    def rollback(self):
        ...
        """
        try:
            return_value = DbtPy.rollback(self.conn_handler)
        except Exception as inst:
            raise _get_exception(inst)
        return return_value
    ...
    def set_option(self, attr_dict):
        ...
        """
        return DbtPy.set_option(self.conn_handler, attr_dict, 1)
    ...
    def get_option(self, attr_key):
        ...
        """
        return DbtPy.get_option(self.conn_handler, attr_key, 1)
    ...
    def set_autocommit(self, is_on):
        ...
        try:
          if is_on:
            is_set = DbtPy.set_option(self.conn_handler, {SQL_ATTR_AUTOCOMMIT : SQL_AUTOCOMMIT_ON}, 1)
          else:
            is_set = DbtPy.set_option(self.conn_handler, {SQL_ATTR_AUTOCOMMIT : SQL_AUTOCOMMIT_OFF}, 1)
        except Exception as inst:
          raise _get_exception(inst)
        return is_set
    ...
    def set_current_schema(self, schema_name):
        ...
        try:
          # is_set = DbtPy.set_option(self.conn_handler, {SQL_ATTR_CURRENT_SCHEMA : schema_name}, 1)
          is_set = 'dummy'
        except Exception as inst:
          raise _get_exception(inst)
        return is_set
    ...
    def get_current_schema(self):
        ...
        """
        try:
          # conn_schema = DbtPy.get_option(self.conn_handler, SQL_ATTR_CURRENT_SCHEMA, 1)
          conn_schema = 'dummy'
    ...
    def tables(self, schema_name=None, table_name=None):
        """Input: connection - DbtPy.IFXConnection object
        ...
        try:
          stmt = DbtPy.tables(self.conn_handler, None, schema_name, table_name)
    ...
    def indexes(self, unique=True, schema_name=None, table_name=None):
        ...
        try:
          stmt = DbtPy.statistics(self.conn_handler, None, schema_name, table_name, unique)
          row = DbtPy.fetch_assoc(stmt)
          i = 0
          while (row):
    ...
    def primary_keys(self, unique=True, schema_name=None, table_name=None):
        ...
        try:
          stmt = DbtPy.primary_keys(self.conn_handler, None, schema_name, table_name)
          row = DbtPy.fetch_assoc(stmt)
          i = 0
          while (row):
              result.append( row )
    ...
    def foreign_keys(self, unique=True, schema_name=None, table_name=None):
        ...
        try:
          stmt = DbtPy.foreign_keys(self.conn_handler, None, None, None, None, schema_name, table_name)
          row = DbtPy.fetch_assoc(stmt)
          i = 0
          while (row):
              result.append( row )
    ...
    def columns(self, schema_name=None, table_name=None, column_names=None):
        ...
        try:
          stmt = DbtPy.columns(self.conn_handler, None, schema_name, table_name)
          row = DbtPy.fetch_assoc(stmt)
          i = 0
          while (row):
            result.append( row )
...
class Cursor(object):
    ...
    def __get_description(self):
        ...
        try:
            num_columns = DbtPy.num_fields(self.stmt_handler)
            ...
            for column_index in range(num_columns):
                column_desc = []
                column_desc.append(DbtPy.field_name(self.stmt_handler,
                                                          column_index))
                type = DbtPy.field_type(self.stmt_handler, column_index)
                type = type.upper()
                if STRING == type:
...
#                    column_desc.append(XML)
                ...
                column_desc.append(DbtPy.field_display_size(
                                             self.stmt_handler, column_index))

                column_desc.append(DbtPy.field_display_size(
                                             self.stmt_handler, column_index))

                column_desc.append(DbtPy.field_precision(
                                             self.stmt_handler, column_index))

                column_desc.append(DbtPy.field_scale(self.stmt_handler,
                                                                column_index))

                column_desc.append(DbtPy.field_nullable(
                                             self.stmt_handler, column_index))
    ...
    def __init__(self, conn_handler, conn_object=None):
        """Constructor for Cursor object. It takes DbtPy connection
    ...
    def close(self):
        ...
        if self.conn_handler is None:
            ...
            raise self.messages[len(self.messages) - 1]
        try:
            return_value = DbtPy.free_stmt(self.stmt_handler)
        except Exception as inst:
            self.messages.append(_get_exception(inst))
            raise self.messages[len(self.messages) - 1]
        self.stmt_handler = None
    ...
    def _callproc_helper(self, procname, parameters=None):
        if parameters is not None:
            ...
            try:
                result = DbtPy.callproc(self.conn_handler, procname,parameters)
            except Exception as inst:
                self.messages.append(_get_exception(inst))
                raise self.messages[len(self.messages) - 1]
        else:
            try:
                result = DbtPy.callproc(self.conn_handler, procname)
            except Exception as inst:
                self.messages.append(_get_exception(inst))
                raise self.messages[len(self.messages) - 1]
        return result
    ...
    def _prepare_helper(self, operation, parameters=None):
        try:
            DbtPy.free_stmt(self.stmt_handler)
        except:
            pass

        try:
            self.stmt_handler = DbtPy.prepare(self.conn_handler, operation)
        except Exception as inst:
            self.messages.append(_get_exception(inst))
            raise self.messages[len(self.messages) - 1]
    ...
    def _set_cursor_helper(self):
        if (DbtPy.get_option(self.stmt_handler, DbtPy.SQL_ATTR_CURSOR_TYPE, 0) != DbtPy.SQL_CURSOR_FORWARD_ONLY):
    ...
    def _execute_helper(self, parameters=None):
        if parameters is not None:
            ...
            parameters = tuple(buff)
            try:
                return_value = DbtPy.execute(self.stmt_handler, parameters)
                if not return_value:
                    if DbtPy.conn_errormsg() is not None:
                        self.messages.append(Error(str(DbtPy.conn_errormsg())))
                        raise self.messages[len(self.messages) - 1]
                    if DbtPy.stmt_errormsg() is not None:
                        self.messages.append(Error(str(DbtPy.stmt_errormsg())))
                        raise self.messages[len(self.messages) - 1]
            except Exception as inst:
                self.messages.append(_get_exception(inst))
                raise self.messages[len(self.messages) - 1]
            ...
            try:
                return_value = DbtPy.execute(self.stmt_handler)
                if not return_value:
                    if DbtPy.conn_errormsg() is not None:
                        self.messages.append(Error(str(DbtPy.conn_errormsg())))
                        raise self.messages[len(self.messages) - 1]
    ...
    def _set_rowcount(self):
        ...
        if not self._result_set_produced:
            try:
                counter = DbtPy.num_rows(self.stmt_handler)
            except Exception as inst:
                self.messages.append(_get_exception(inst))
                raise self.messages[len(self.messages) - 1]
            ...
            try:
                counter = DbtPy.get_num_result(self.stmt_handler)
            except Exception as inst:
                self.messages.append(_get_exception(inst))
                raise self.messages[len(self.messages) - 1]
            if counter >= 0:
    ...
    # Retrieves the last generated identity value from the database catalog
    def _get_last_identity_val(self):
        ...
        try:
            stmt_handler = DbtPy.prepare(self.conn_handler, operation)
            if DbtPy.execute(stmt_handler):
                row = DbtPy.fetch_tuple(stmt_handler)
    ...
    def executemany(self, operation, seq_parameters):
        ...
        self._prepare_helper(operation)
        try:
            autocommit = DbtPy.autocommit(self.conn_handler)
            if autocommit !=  0:
                DbtPy.autocommit(self.conn_handler, 0)
            self.__rowcount = DbtPy.execute_many(self.stmt_handler, seq_parameters)
            if autocommit != 0:
                DbtPy.commit(self.conn_handler)
                DbtPy.autocommit(self.conn_handler, autocommit)
    ...
    def _fetch_helper(self, fetch_size=-1):
        ...
        while (fetch_size == -1) or (fetch_size != -1 and rows_fetched < fetch_size):
            try:
                row = DbtPy.fetch_tuple(self.stmt_handler)
            except Exception as inst:
                if DbtPy.stmt_errormsg() is not None:
                    self.messages.append(Error(str(DbtPy.stmt_errormsg())))
                else:
    ...
    def nextset(self):
        ...
        try:
            ...
            self.stmt_handler = DbtPy.next_result(self._all_stmt_handlers[0])
        except Exception as inst:
            self.messages.append(_get_exception(inst))
            raise self.messages[len(self.messages) - 1]
    ...
    def _fix_return_data_type(self, row):
        ...
        for index in range(len(row)):
            if row[index] is not None:
                type = DbtPy.field_type(self.stmt_handler, index)
                type = type.upper()
