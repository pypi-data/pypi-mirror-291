...
class DbtPyTestCase(unittest.TestCase):
  ...
  def run_test_000(self):
    ...
    try:
      ...
      pass
    # Create test view
    DbtPy.exec_immediate(conn, """CREATE VIEW anime_cat AS
    ...
    for animal in animals:
      ...
      if (not picture):
        ...
        return 0
      DbtPy.bind_param(stmt, 1, name, DbtPy.SQL_PARAM_INPUT)
      DbtPy.bind_param(stmt, 2, picture, DbtPy.SQL_PARAM_INPUT)
    ...
    if stmt:
      for dept in department:
        result = DbtPy.execute(stmt, dept)
    ...
    if stmt:
      for photo in emp_photo:
        ...
        DbtPy.bind_param(stmt, 1, empno, DbtPy.SQL_PARAM_INPUT)
        DbtPy.bind_param(stmt, 2, photo_format, DbtPy.SQL_PARAM_INPUT)
        DbtPy.bind_param(stmt, 3, picture, DbtPy.SQL_PARAM_INPUT)
 #       result = DbtPy.execute(stmt)
    ...
    if stmt:
      for orgpart in org:
        result = DbtPy.execute(stmt, orgpart)
    ...
    if stmt:
      for proj in project:
        result = DbtPy.execute(stmt, proj)
    ...
    if stmt:
      for sale in sales:
        result = DbtPy.execute(stmt, sale)
    ...
    if (server.DBMS_NAME[0:3] == 'Inf'):
      result = DbtPy.exec_immediate(conn, """
    ...
    try:
      result = DbtPy.exec_immediate(conn, 'DROP TABLE t_string')
    except:
      pass
    result = DbtPy.exec_immediate(conn, 'CREATE TABLE t_string(a INTEGER, b DOUBLE PRECISION, c VARCHAR(100))')
