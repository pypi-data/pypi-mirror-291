...
class DbtPyTestFunctions(unittest.TestCase):
  #prepconn = DbtPy.connect(config.database, config.user, config.password)
  prepconn = DbtPy.connect(config.ConnStr, "", "")
  server = DbtPy.server_info(prepconn)
  DbtPy.close(prepconn)
