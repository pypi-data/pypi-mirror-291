...
class PNSqlCursor(isqlcursor.ISqlCursor):
    ...
    def setAction(self, action_or_name: Union[str, "pnaction.PNAction"]) -> bool:
        ...
        if self._action:
            LOGGER.warning(
                "ACTION-NAME %s, ACTION_TABLE: %s" % (action_or_name, self._action.table())
            )
            ...
            for field in field_list:
                ...
                for relation in field.relationList():
                    ...
                    if relation_m1 and relation_m1.deleteCascade():
                        ...
                        LOGGER.warning(" ".join(texto))
                        cursor = PNSqlCursor(foreign_table)
                        LOGGER.warning(
                            "NUEVO CURSOR A %s, tabla: %s" % (foreign_table, cursor.table())
                        )
