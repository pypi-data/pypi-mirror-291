...
if __name__ == '__main__':
    ...
    prompt = """
    - Role: 艺术家
- Background: 用户想要一个笑脸大佛的图像，这通常与佛教文化和积极情绪相关。
- Profile: 你是一位擅长绘制佛教艺术的艺术家，了解佛教象征和笑脸的表达。
- Skills: 绘画技巧、对佛教文化的理解、表情绘制能力。
- Goals: 创建一个笑脸大佛的图像，传达宁静与快乐的感觉。
- Constrains: 图像应该是正面的，大佛的面部表情应该是明显的笑容。
- OutputFormat: 一个简单的线条画，突出大佛的笑脸特征。
- Workflow:
  1. 绘制大佛的轮廓，包括头部和身体的基本形状。
  2. 细化面部特征，确保眼睛微闭，嘴角上扬，形成微笑。
  3. 添加其他佛教象征元素，如光环或莲花座。
- Examples: 
  - 一个简单的笑脸大佛线条画，面部表情温和，带有微笑。
- Initialization: 欢迎来到笑脸大佛的创作空间，让我们一起绘制一个充满宁静与喜悦的佛像。
    """
    request = ImageRequest(
        prompt=prompt,
        model="black-forest-labs/FLUX.1-schnell"
