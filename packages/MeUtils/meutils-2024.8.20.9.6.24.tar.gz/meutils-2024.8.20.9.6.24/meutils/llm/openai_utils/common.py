...
async def ppu_flow(
    ...
    api_key and [await appu(pre, api_key=api_key, base_url=base_url) for i in range(n)]
    logger.debug(f"PREPAY: {pre}")

    yield

    post = post if post in MODEL_PRICE else "ppu-1"
    api_key and [await appu(post, api_key=api_key, base_url=base_url) for i in range(n)]
...
if __name__ == '__main__':
    ...
    async def main():
        async with ppu_flow(api_key=os.getenv('OPENAI_API_KEY')):
