...
class GamePreprocess(Workflow[Input, Output, Queues, Params, pre.Artifacts, Pipelines]): # type: ignore
  ...
  def run(self, queues: Queues, params: Params):
    ...
    artifs = self.pipelines['preprocess'].run(queues['preprocess'], pre.Params(logger=logger, images=images, descaled_h=params.get('descaled_h', 768), autocorrect=autocorrect))
    artifs.processes = {
