...
class Entry:
    class _NoneQuotedValue(str):... #basically a string, but we only quote str class, so this subclass avoids getting quoted
    ...
    def __str__(self):
        ...
        if(self.Tag == '#EXT-X-STREAM-INF'): #specification states mandatory int attribute bandwidth, therefore reasonable to avoid qouting
            self.Attributes['BANDWIDTH'] = self._NoneQuotedValue(self.Attributes['BANDWIDTH'])
            if("RESOLUTION" in self.Attributes): #not mandatory, but specs wants it without quotes eg "<width>x<height>"
                self.Attributes['RESOLUTION'] = self._NoneQuotedValue(self.Attributes['RESOLUTION'])


        if(len(self.Attributes) > 0):
            attributeStrings = []
            for key,value in self.Attributes.items():
                if(isinstance(value, str)) and (type(value) is not self._NoneQuotedValue):
                    value = f'"{value}"'
...
class M3U8Reader:
    ...
    def __init__(self) -> None:
        self.Entries:list[Entry] = [] #some lines might not be entries such as empty lines or comments
    ...
    def Load(self, stream:TextIO|BinaryIO):
        ...
        for line in data.splitlines():
            ...
            if(line.startswith('#EXT')):
                ...
                entry = Entry(tagAndAttributes[0])
