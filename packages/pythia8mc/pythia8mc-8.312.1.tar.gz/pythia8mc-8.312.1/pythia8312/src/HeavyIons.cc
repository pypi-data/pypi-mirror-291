...
bool Angantyr::setBeamIDs(int idAIn, int idBIn) {
  ...
  int idANuc = idAIn, idBNuc = idBIn;
  if (abs(idAIn/100000000) == 10)
    idANuc = (idAIn > 0) ? 2212 : -2212;
  if (idBIn == 0)
    idBIn = idTarg;
  if (abs(idBIn/100000000) == 10)
    idBNuc = (idBIn > 0) ? 2212 : -2212;

  if (!pythia[MBIAS]->setBeamIDs(idANuc, idBNuc))
    return false;
  if (!pythia[SASD]->setBeamIDs(idANuc, idBNuc))
    return false;

  sigTotNN.calc(idANuc, idBNuc, beamSetupPtr->eCM);

  projPtr->setParticle(idAIn);
  targPtr->setParticle(idBIn);

  collPtr->setIDA(beamSetupPtr->represent(idANuc));
  bGenPtr->updateWidth();
  unifyFrames();
