...
class HTTPClient(Session):
    ...
    def get_response_data(self, status_code, *, message_key='message', data_key='data', **kwargs):
        if not self._response:
            raise ValueError('响应数据为空！')
        assert self._response.status_code == status_code, f'请求失败，预期[{status_code}]、实际[{self._response.status_code}]。响应结果为：{self._response.text}'
        ...
        for key, value in kwargs.items():
            ...
            assert data[key] == value, f'响应数据[{key}]值不匹配，预期[{value}]、实际[{data[key]}](提示信息：{message})'
