...
async def main(  # noqa: C901, PLR0912, PLR0913, PLR0915
    ...
    continue_max_id: Optional[int] = None
    if continue_from_saved_progress and save_progress_path:
    ...
    for status in tqdm(
        ...
        try:
            ...
            if save_progress_path:
                last_deleted_id = status if isinstance(status, int) else int(status["id"])
