...
class mkfit:
    r"""Multi-kernel mcfit.

    """
    def __init__(*mcfits):
        # lowring can actually be True?
        # assert common Delta, x[0] (not jittable, so maybe class mkfit?), length N, backend (warning?)
        # individual MK, q, lowring, xy

    Cs = [t.matrix(full=False, keeppads=True)[2] for t in mcfits]
    r = T[0]._y_
    A = T[0]._pad(A, 0, True, False)
    B = 1 / Delta * r[:, None]**-q * H[0] @ (A * H[2]) * r**-q
    B = T[0]._unpad(B, 0, True)
    B = T[0]._unpad(B, 1, True)
...
class mkfit:
    ...
    The generic form is
    .. math:: G(y) = \int_0^\infty F(x) K_1(xy) \frac{dx}x
