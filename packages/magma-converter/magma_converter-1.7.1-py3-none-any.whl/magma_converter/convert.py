...
class Convert(Search):
    def __init__(self,
        ...
        self.database: DatabaseConverter | None = None
    ...
    def plot_gaps(self) -> Self:
        """Plot seismogram with gaps."""
        self._plot_gaps = True
        return self
    ...
    def run(self) -> Self:
        ...
        if self.save_to_database is True:
            ...
            if len(self.success) > 0:
                self.database = DatabaseConverter(sds_results=self.success).update()
            ...
            if trace.stats.channel[0] not in ['M', 'O']:
                ...
                if sds.save(min_completeness=min_completeness, **kwargs) is True:
                    ...
                    if (self.plot_seismogram is True) and (file_exists is False):
                        Plot(sds=sds, plot_type=self.plot_type).plot_with_gaps(plot_gaps=self._plot_gaps).save()
