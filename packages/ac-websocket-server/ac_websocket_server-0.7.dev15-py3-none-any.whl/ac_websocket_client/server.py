...
class ServerUI(GriddedFrame):
    ...
    def __init__(self, parent):
        ...
        GriddedButton(self, grid_row=grid_row, grid_col=3,
                      ...
                      command=lambda: self.parent.loop.create_task(self.parent.toggle_session('Practice')))
        ...
        GriddedButton(self, grid_row=grid_row, grid_col=3,
                      ...
                      command=lambda: self.parent.loop.create_task(self.parent.toggle_session('Qualify')))
        ...
        GriddedButton(self, grid_row=grid_row, grid_col=3,
                      ...
                      command=lambda: self.parent.loop.create_task(self.parent.toggle_session('Race')))
    ...
    def _trace_sessions(self, *args):
        ...
        for session in ('Practice', 'Qualify', 'Race'):
            ...
            if old_session := self.parent.sessions.get(session, None):
                old_laps = old_session['laps']
                old_time = old_session['time']
            else:
                old_laps = None
                old_time = None
            ...
            if m:
                ...
                if new_type == 'laps' and old_laps != new_value:
                    print(f'Change {session} laps to {new_value} laps')
                    # self.parent.loop.create_task(
                    #     self.parent.update_session(session, 'laps', new_value))
                if new_type == 'minutes' and old_time != new_value:
                    print(f'Change {session} time to {new_value} minutes')
                    # self.parent.loop.create_task(
                    #     self.parent.update_session(session, 'time', new_value))
