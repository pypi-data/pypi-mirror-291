...
from gammalearn.callbacks import (LogGradientNorm, LogModelWeightNorm, LogModelParameters,
                                  LogUncertaintyTracker, LogReLUActivations,
                                  LogLinearGradient, LogFeatures, WriteDL2Files)
import gammalearn.utils as utils
import gammalearn.datasets as dsets
from gammalearn.data_handlers import GLearnDataModule, GLearnDomainAdaptationDataModule
...
targets = collections.OrderedDict({
    ...
    'domain_class': {
        ...
        'grad_weight': utils.ExponentialW(gamma=10),
        'loss_weight': 1.,
        'metrics': {
...
net_parameters_dic = {
    'model': nets.GammaPhysNetMegatron,
    'parameters': {
        'backbone': {
            'parameters': {
                'num_channels': 2,
...
training_step = steps.get_training_step_dann(
...
eval_step = steps.get_eval_step_dann(
    **{'add_pointing': net_parameters_dic['parameters']['backbone']['parameters']['add_pointing']}
)
