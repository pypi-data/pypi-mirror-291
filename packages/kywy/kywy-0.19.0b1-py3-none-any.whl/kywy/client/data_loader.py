...
class KawaDataLoader:
    ...
    def _load_data(self,
        ...
        query_params = 'datasource={}&format={}&reset={}&session={}&optimize={}'.format(datasource_id,
                                                                                        data_format,
                                                                                        reset_data,
                                                                                        session_id,
                ...
                if re.match(r'^datetime64', column_type_name):
                    try:
                        if self._use_arrow:
                            ...
                            nanos = pc.cast(timestamp_column, pa.int64())
        ...
        try:
            ...
            if self._use_arrow:
                ...
                print('> {} rows were imported in {}ms'.format(num_rows, end - start))
                return datasource
    ...
    def _loading_thread_arrow(self, ingestion_url, arrow_table: pa.Table):
        ...
        writer = pa.ipc.new_stream(output_stream, arrow_table.schema)
        writer.write_table(arrow_table)
        writer.close()

        self._k.post_stream(
            stream=output_stream.getvalue(),
            url=ingestion_url
        )
