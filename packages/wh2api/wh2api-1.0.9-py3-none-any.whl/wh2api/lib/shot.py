...
# === changed ===
def relation(project_idx, which,which_idx):
    '''
    샷에 연결된 Asset 리스트 출력하기
    :param project_idx:프로젝트 인덱스
    :param which: 'episode,sequence,shot'중 선택
    :param which_idx:which의 인덱스 입력
    :return:
    '''

    api = api_list.shot_relation %(project_idx,which,which_idx)
    result = wh_setting.get_requests(api=api)
    return result


def last_version_list(episode_idx):
    '''
    에피소드에 포함된 모든 샷의 마지막 버전 정보 취득
    :param episode_idx: 에피소드 인덱스
    :return:
    '''
    api = api_list.shot_last_version_list
