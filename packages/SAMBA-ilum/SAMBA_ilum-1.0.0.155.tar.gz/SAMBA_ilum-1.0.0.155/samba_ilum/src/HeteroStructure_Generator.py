...
#------------------------------------------------------------------------------------------------------------------------------------------------------------
MT00 = [[]*(len(v1Ax)) for i in range(3)]
MT01 = [[]*(len(v1Ax)) for i in range(3)]
MT10 = [[]*(len(v1Ax)) for i in range(3)]
MT11 = [[]*(len(v1Ax)) for i in range(3)]
#----------------------------------------  
MT00[:,0] = SLattice1[:,7];  MT01[:,0] = SLattice1[:,8];  MT10[:,0] = SLattice1[:,9];  MT11[:,0] = SLattice1[:,10]
MT00[:,1] = SLattice2[:,7];  MT01[:,1] = SLattice2[:,8];  MT10[:,1] = SLattice2[:,9];  MT11[:,1] = SLattice2[:,10]
#-----------------------------------------------------------------------------------------------------------------
           ...
           if (mismatch_A <= crit_mismatch):
              ...
              if (mismatch_B <= crit_mismatch):
                 ...
                 if (n_Lattice == 2):
                    ...
                    Structures.write(f'{MT00[0][i]} {MT01[0][i]} {MT10[0][i]} {MT11[0][i]} ')
                    Structures.write(f'{MT00[1][i]} {MT01[1][i]} {MT10[1][i]} {MT11[1][i]} ')
                    Structures.write(f'0.0 0.0 0.0 0.0 \n')
                    #============================================================================================
                 if (n_Lattice == 3):
                    #========================================================================================
...
if (n_Lattice == 3):
   ...
   #----------------------------------------------------------------------------------------------------------------------------------------------------------------
   MT00[:,2] = SLattice3[:,7];  MT01[:,2] = SLattice3[:,8];  MT10[:,2] = SLattice3[:,9];  MT11[:,2] = SLattice3[:,10]
   #-----------------------------------------------------------------------------------------------------------------
           ...
           if (mismatch_ang <= crit_angle):
              ...
              if (mismatch_A <= crit_mismatch):
                 ...
                 if (mismatch_B <= crit_mismatch):
                    ...
                    Structures.write(f'{v3Ax[j]:>14,.9f} {v3Ay[j]:>14,.9f} {v3Bx[j]:>14,.9f} {v3By[j]:>14,.9f} ')          # ????????? Verificar o loop [i] e [j] ?????????
                    Structures.write(f'{mismatch_A12[i]:>7,.4f} {mismatch_B12[i]:>7,.4f} {mismatch_ang12[i]:>7,.4f} ')
                    Structures.write(f'{mismatch_A:>7,.4f} {mismatch_B:>7,.4f} {mismatch_ang:>7,.4f} \n')
                    Structures.write(f'{MT00[0][i]} {MT01[0][i]} {MT10[0][i]} {MT11[0][i]} ')
                    Structures.write(f'{MT00[1][i]} {MT01[1][i]} {MT10[1][i]} {MT11[1][i]} ')
                    Structures.write(f'{MT00[2][j]} {MT01[2][j]} {MT10[2][j]} {MT11[2][j]} \n')                            # ????????? Verificar o loop [i] e [j] ?????????
                    #====================================================================================
   #-----------------
   Structures.close()
   #-----------------
...
mismatch_A12   = tfile[:,12]
mismatch_B12   = tfile[:,13]
mismatch_ang12 = tfile[:,14]
mismatch_12    = (mismatch_A12 + mismatch_B12 + mismatch_ang12)/3

if (n_Lattice == 3):
   mismatch_A13   = tfile[:,15]
   mismatch_B13   = tfile[:,16]
   mismatch_ang13 = tfile[:,17]
...
if (n_structures != 0):
   ...
   for i in range(n_structures):
       ...
       poscar_new.write(f' | MTrans_1 = {MT00[0][i]}_{MT01[0][i]}_{MT10[0][i]}_{MT10[0][i]}')
       poscar_new.write(f' | MTrans_2 = {MT00[1][i]}_{MT01[1][i]}_{MT10[1][i]}_{MT10[1][i]}')
       if (n_Lattice == 3): poscar_new.write(f' | MTrans_3 = {MT00[2][i]}_{MT01[2][i]}_{MT10[2][i]}_{MT10[2][i]}')
...
for i in range(len(files)):
    ...
    if (n_Lattice == 2):
       new_name = diret2 + '/' +  n_ion + '_' + mismatch_12 + '_' + angle_12 + '_' + str(VTemp1[-1]) + '+' + number
    if (n_Lattice == 3):
       new_name = diret2 + '/' +  n_ion + '_' + mismatch_12 + '_' + angle_12 + '_'  + mismatch_13 + '_' + angle_13 + '_'+ str(VTemp1[-1]) + '+' + number
