...
class SmartTrack:
    ...
    def update_with_tensors(self, tensors: np.ndarray, frame) -> List[STrack]:
        ...
        if self.rematch_new_tracks and len(new_stracks) > 0 :
            # asynchronously rematch the tracks
            rematch_thread = Thread(target=async_rematch, args=(joint_tracks(self.tracked_tracks, self.lost_tracks), joint_tracks(self.tracked_tracks, new_stracks), frame, lambda x: self.rematch_tracks_lost(x)))
        ...
        if self.rematch_swapped_tracks and len(self.tracked_tracks) > self.last_tracks:
            rematch_thread = Thread(target=async_rematch, args=(self.tracked_tracks, self.tracked_tracks, frame, lambda x: self.rematch_tracks_swapped(x), 0.4))
            rematch_thread.start()
    ...
    def rematch_tracks_lost(self, matches):
        ...
        for i, j in matches:
            i_track = [t for t in joint_tracks(joint_tracks(self.tracked_tracks, self.lost_tracks), self.removed_tracks) if t.external_track_id == i][0]
            j_track = [t for t in joint_tracks(joint_tracks(self.tracked_tracks, self.lost_tracks), self.removed_tracks) if t.external_track_id == j][0]
