...
def _convert_semantic(fmt: str, old_semantic: str, **kwargs: Any) -> str:
    ...
    if "semantic" in kwargs:
        ...
        mapping_names = load_resource_file("metadata/mapping_legacy_dpr.json")

        if old_semantic in mapping_names:
            new_semantic = mapping_names[old_semantic]
            if isinstance(new_semantic, str):
                return new_semantic
            elif isinstance(new_semantic, list):
                raise DataSemanticConversionError(
                    f"Multiple values for legacy {old_semantic!r}: {new_semantic!r}.\n"
...
def _extract_data_from_product_filename(filename: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if fmt.startswith("product/eopf"):
        ...
        semantic = _convert_semantic(fmt, old_semantic, **kwargs)  # eg MSIL1C
