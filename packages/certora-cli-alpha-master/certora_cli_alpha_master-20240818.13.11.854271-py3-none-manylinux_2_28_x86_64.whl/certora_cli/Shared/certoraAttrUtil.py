...
class AttributeDefinition:
    ...
    typechecker_flag: Optional[str] = \
        None  # the flag that is sent to the typechecker jar (if attr is sent to the typechecker jar)
    ...
    def get_flag(self) -> str:
        return self.flag if self.flag is not None else '--' + str(self.name.lower())
...
class Attributes:
    ...
    attribute_list: List[AttributeDefinition] = []
    all_conf_names: List[str] = []
    ...
    def set_attribute_list(cls) -> None:
        ...
        if not cls.attribute_list:
            cls.attribute_list = [set_name(name) for name in dir(cls) if name.isupper()]
            cls.all_conf_names = [f"{attr.name}".lower() for attr in cls.attribute_list]
