...
def collect_args_with_jar_flags(context: CertoraContext) -> List[AttrUtil.AttributeDefinition]:
    return [attr for attr in Attrs.ContextAttributes.attribute_list if get_attr_value(context, attr) and attr.jar_flag]


def collect_args_build_cache_affecting(context: CertoraContext) -> List[AttrUtil.AttributeDefinition]:
    return [attr for attr in Attrs.ContextAttributes.attribute_list if get_attr_value(context, attr) and
...
def collect_jar_args(context: CertoraContext) -> List[str]:
    ...
    prover_args_values = getattr(context, Attrs.ContextAttributes.PROVER_ARGS.get_conf_key(), None)
    ...
    type_checker_values = getattr(context, Attrs.ContextAttributes.TYPECHECKER_ARGS.get_conf_key(), None)
...
def get_typechecker_arguments(context: CertoraContext) -> List[str]:
    """
    ...
    """
    args = []
    if context.typechecker_args:
        args += context.typechecker_args

    relevant_attrs = filter(lambda arg: get_attr_value(context, arg),
                            [Attrs.ContextAttributes.RULE, Attrs.ContextAttributes.EXCLUDE_RULE,
                             Attrs.ContextAttributes.FOUNDRY_TESTS_MODE])
    args += create_jar_flag_list(context, list(relevant_attrs))
...
class ProverParser(AttrUtil.ContextAttributeParser):
    ...
    def format_help(self) -> str:
        ...
        Attrs.ContextAttributes.print_attr_help()
...
def __get_argparser() -> argparse.ArgumentParser:
    ...
    for arg in Attrs.ContextAttributes.attribute_list:
        flag = arg.get_flag()
...
def handle_flags_in_args(args: List[str]) -> None:
    ...
    all_flags = list(map(lambda member: member.get_flag(), Attrs.ContextAttributes.attribute_list))
    ...
    for index, arg in enumerate(args):
        ...
        while True:
            if arg and (surrounded(arg, '\'') or surrounded(arg, '\"')):
                ...
                break
        if len(arg) > 0 and arg[0] == '-' and (args[index - 1] == Attrs.ContextAttributes.JAVA_ARGS.get_flag()):
            arg = f" {arg}"
        if arg != args[index]:
            args[index] = arg
...
def run_typechecker(typechecker_name: str, with_typechecking: bool, args: List[str]) -> None:
    ...
    exit_code = Util.run_jar_cmd(typecheck_cmd, False, custom_error_message="Failed to compile spec files:",
...
def run_local_spec_check(with_typechecking: bool, context: CertoraContext) -> None:
    ...
    if context.disable_local_typechecking or Util.is_ci_or_git_action():
        return
    args = get_typechecker_arguments(context)
