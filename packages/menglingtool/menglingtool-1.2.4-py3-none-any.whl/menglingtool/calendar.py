...
# 阳历转阴历
def yang_yin(dtime=None):
    ln = Lunar(dtime if dtime != None else datetime.datetime.now())
    yindate = ln.ln_date()
    return yindate


# 获取节气
def getJQ(dtime=None):
    ln = Lunar(dtime if dtime != None else datetime.datetime.now())
    jq = ln.ln_jie()
    return jq
...
# 获取对应农历完整文本信息
def getAllStr(dtime=None):
    ln = Lunar(dtime if dtime != None else datetime.datetime.now())
    txts = []
    txts.append(ln.ln_date_str())
    txts.append('节气：%s' % ln.ln_jie())
    return '\n'.join(txts)
...
if __name__ == '__main__':
    # ct = datetime.datetime(2021, 2, 11)
    print(getAllStr())
    # test(ct)
