...
class Pyranocam:
    ...
    def download_image(self,variable, file_path_on_server, download_path=""):
        """Downloads a single JPG file."""
        timestamp_pattern = re.compile(r"\d{4}-\d{2}-\d{2}")
        date = timestamp_pattern.search(file_path_on_server).group()
        url = f"{self.base_url}/download/{variable}/{date}/{file_path_on_server}"
        print(url)
        self._download_file(url, os.path.basename(file_path_on_server), download_path)

    def download_csv_file(self,variable, file_path_on_server, download_path=""):
    ...
    def download_images_in_range(self, variable, start_datetime, end_datetime, download_path="."): 
        ...
        # Filter files based on the time range using regex
        filtered_files = []
        ...
        # Download the filtered files using the download_image method
        for file_name in tqdm(filtered_files, desc="Downloading", unit="file"):
            self.download_image(variable,file_name, download_path)