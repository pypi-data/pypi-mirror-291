...
def tier1_co2_pp(
    ...
    seq.read_parameter(
        name="processtype", table="processtype", coords=[year, region, product]
    )
    ...
    seq.read_parameter(
        ...
        coords=[
            ...
            region,
            product,
            seq.step.processtype.value,
            seq.step.feedstocktype.value,
        ],
    )
    ...
    value = seq.elementary.eco2_tier1(
        pp=seq.step.pp_i.value, ef=seq.step.ef_co2_i_k.value, gaf=seq.step.gaf.value
    )
...
def tier1_co2_fa(
    ...
    """Tier 1 method CO2 Emissions.
    Starting with data input for parameter fa (anual consumption of feedstock)
    ...
    d = seq.get_dimension_levels(year, region, product, uncert="def", table="fa_i_k")
    value = 0.0
    for feedstocktype in d:
        ...
        value += seq.elementary.pp_i(
            fa=getattr(getattr(seq.step, f"fa_i_k_xxx_{feedstocktype}_xxx"), "value"),
            spp=getattr(getattr(seq.step, f"spp_i_k_xxx_{feedstocktype}_xxx"), "value"),
        )
    seq.store_result(
        name="pp_i",
        value=value,
...
def tier2_co2(
    ...
    value = seq.step.pp_pc(pp=seq.step.pp_i.value, pc=seq.step.pc_i.value)
...
def tier1_ch4_pp(
    ...
    fug_value = seq.elementary.ech4_fugitive(
        pp=seq.step.pp_i.value, ef=seq.step.eff_i.value
    )
    ...
    vent_value = seq.elementary.ech4_process_vent(
        pp=seq.step.pp_i.value, ef=seq.step.efp_i.value
    )
