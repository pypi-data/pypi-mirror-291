...
def tier1a_co2_coke(
    ...
    seq.read_parameter(
        name="ef_co2_v3c4", table="ef_co2_v3c4", coords=[year, region, activity]
    )

    value = seq.elementary.co2_coke_tier1a_(
        ck=seq.step.ck.value, ef_co2=seq.step.ef_co2_v3c4.value
    )
...
def tier1_co2_steel(year=2019, region="DE", activity="bof", uncertainty="def"):
    ...
    # pigiron
    seq.read_parameter(name="q_pigiron", table="q_pigiron", coords=[year, region])
    seq.read_parameter(
        name="ef_co2_pigiron",
    ...
    # dri
    seq.read_parameter(name="q_dri", table="q_dri", coords=[year, region])
    seq.read_parameter(
        name="ef_co2_dri",
        table="ef_co2_dri",
        coords=[year, region],
    ...
    # sinter
    seq.read_parameter(name="q_sinter", table="q_sinter", coords=[year, region])
    seq.read_parameter(
        name="ef_co2_sinter",
        table="ef_co2_sinter",
        coords=[year, region],
    ...
    # ironpellet
    seq.read_parameter(name="q_pellet", table="q_pellet", coords=[year, region])
    seq.read_parameter(
        name="ef_co2_pellet",
        table="ef_co2_pellet",
        coords=[year, region],
...
def tier2_co2_steel(year=2019, region="DE", uncertainty="def"):
    ...
    seq.read_parameter(
        name="l", table="l", coords=[year, region], lci_flag="use|product|limestone"
    )
    seq.read_parameter(
        name="c_l",
        table="c_l",
    ...
    seq.read_parameter(
        name="s", table="s", coords=[year, region], lci_flag="supply|product|steel"
    )
    seq.read_parameter(
        name="c_s",
        table="c_s",
    ...
    d = seq.get_dimension_levels(year, region, uncert="def", table="cob_a_tier2")
    value = 0.0
    for byproduct_type in d:
        seq.read_parameter(
            ...
            table="cob_a_tier2",
            coords=[year, region, byproduct_type],
            lci_flag=f"supply|by-prodcut|{byproduct_type}",
        )
        seq.read_parameter(
            name=f"c_b_xxx_{byproduct_type}_xxx",
            table="c_b",
    ...
    d1 = seq.get_dimension_levels(year, region, uncert="def", table="o_b")
    value = 0.0
    ...
    d = seq.get_dimension_levels(year, region, uncert="def", table="pm_a_sinter")
    value = 0.0
    for material_type in d:
        seq.read_parameter(
...
def tier1_ch4_steel(year=2019, region="DE", uncertainty="def"):
    """Template calculation sequence for tier 1 method.
    ...
    value = seq.elementary.ch4_sinter(
        si=seq.step.si.value, ef_si=seq.step.ef5_ch4_si.value
    )
    ...
    value = seq.elementary.ch4_pigiron(
        pi=seq.step.pi.value, ef_pi=seq.step.ef5_ch4_pi.value
    )
    ...
    value = seq.elementary.ch4_dri(
        dri=seq.step.dri.value, ef_dri=seq.step.ef5_ch4_dri.value
    )
...
def tier1_n2o_flaring(year=2019, region="DE", uncertainty="def"):
    ...
    seq.read_parameter(
        name="q_bfg", table="q_bfg", coords=[year, region], lci_flag="use|product|bfg"
    )
    seq.read_parameter(
        name="q_ldg", table="q_ldg", coords=[year, region], lci_flag="use|product|ldg"
    )
    seq.read_parameter(
        name="r_bfg",
        table="r_bfg",
