    ...
    def train(self, path_to_save=None, **kwargs):
        ...
        for epoch in range(self.epochs):
            ...
            for step, batch in enumerate(train_it):
                batch = batch.to(self.device)  # 修复：应当将batch放到device上
                ...
                loss = loss / self.gradient_accumulation_steps  # 将loss除以梯度累积步数，避免重复计算
        ...
        with torch.no_grad():
            ...
            for batch in it:
                ...
                if self.fast_dtype:
                    ...
                    predictions = self.model.predict(batch)["predictions"]
                val_truth.append(batch["labels"].detach().cpu().numpy(force=True))
        ...
        with torch.no_grad():
            ...
            for batch in it:
                ...
                if self.fast_dtype:
                    ...
                    predictions = self.model.predict(batch)["predictions"]
                truth.append(batch["labels"].cpu().numpy(force=True))
