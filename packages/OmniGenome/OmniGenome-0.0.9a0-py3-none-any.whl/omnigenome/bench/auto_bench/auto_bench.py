        ...
        for bench in self.bench_metadata.bench_list:
            ...
            for key, value in _kwargs.items():
                if key in bench_config:
                    ...
                    warnings.warn(f"kwarg: {key} not found in bench_config while setting {key} = {value}")
                    bench_config.update({key: value})
            ...
            for seed in bench_config["seeds"]:
                ...
                if hasattr(model.config, "max_position_embeddings"):
                    max_length = min(bench_config["max_length"], model.config.max_position_embeddings) - 2
                else:
                ...
                train_loader = torch.utils.data.DataLoader(
                    ...
                    num_workers=_kwargs['num_workers'] if 'num_workers' in _kwargs else 0,
                    shuffle=True
                )
