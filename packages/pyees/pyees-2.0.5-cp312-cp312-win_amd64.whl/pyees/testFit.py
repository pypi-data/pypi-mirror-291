...
from fit import lin_fit, pow_fit, pol_fit, exp_fit, logistic_fit, variable, _fit, crateNewFitClass, multivariablefit
...
class gaussian(_fit):
    ...
    def func_name(self):
        ...
        return rf'$y_mu \cdot exp(-(x-mu)^2/(2 sigma ^ 2)),\quad mu={mu.__str__(pretty = True)}, \quad sigma={sigma.__str__(pretty = True)}, \quad y_mu={y_mu.__str__(pretty = True)}$'
...
class absolute_power(_fit):
    ...
    def func_name(self):
        ...
        return rf'$ y_mu*abs(x-mu)^*u,\quad mu={mu.__str__(pretty = True)}, \quad nu={nu.__str__(pretty = True)}, \quad y_mu={y_mu.__str__(pretty = True)}$'
...
class test(unittest.TestCase):
    ...
    def testMultivariableFit(self):
        ...
        x_squared = x**2
        ones = variable([1] * n)
        F = multivariablefit([x_squared, x, ones], y)
        Fa = F.coefficients[0]
        Fb = F.coefficients[1]
        Fc = F.coefficients[2]
        ...
        x.append(P.value)
        x.append(RH.value)
        x.append(T.value * P.value)
        x.append(T.value * RH.value)
        x = np.transpose(x)
        model = LinearRegression()
        model.fit(x, D.value)

        ones = variable([1] * len(T))
        F = multivariablefit([ones, 1/T, P, RH, T*P, T*RH], D)

        pred = F.predict([ones, 1/T, P, RH, T*P, T*RH]).value
        correct = model.predict(x)
        
        for p, c in zip(pred, correct):
            self.assertRelativeDifference(p, c, 0.035)
