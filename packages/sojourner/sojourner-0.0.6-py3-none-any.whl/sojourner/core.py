...
# %% auto 0
__all__ = ['MANIFEST_FILENAME', 'Metadata', 'get_storage_client', 'Result', 'Sojourner']
...
# %% ../nbs/00_core.ipynb 7
class Result(Enum):
    SUCCESS = "Successfully stored blob and metadata"
    BLOB_EXISTS = "Blob already exists"
    UPLOAD_ERROR = "Failed to upload blob"
    METADATA_ERROR = "Failed to update metadata"


class Sojourner:
    def __init__(self, credentials_path=None):
        if credentials_path:
            self.client = get_storage_client(credentials_path)
        else:
    ...
    def store(
        ...
        """
        bucket = self.client.bucket(os.environ["BUCKET"])
