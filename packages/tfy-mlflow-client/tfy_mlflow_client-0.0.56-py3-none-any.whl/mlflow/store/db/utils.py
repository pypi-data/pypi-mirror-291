...
from mlflow.store.db.constants import (
    DB_POSTGRES_SCHEMA,
    DB_SSL_CA_PATH,
    DB_SSL_CERT_PATH,
    DB_SSL_KEY_PATH,
...
def create_sqlalchemy_engine(db_uri):
    ...
    connect_args = {}
    if db_uri.startswith("postgresql") and DB_POSTGRES_SCHEMA:
        _logger.info(f"Connecting with schema {DB_POSTGRES_SCHEMA} for postgresql")
        connect_args = {"options": f"-csearch_path={DB_POSTGRES_SCHEMA}"}

    if DB_SSL_CERT_PATH:
        connect_args["sslcert"] = DB_SSL_CERT_PATH
    if DB_SSL_KEY_PATH:
        connect_args["sslkey"] = DB_SSL_KEY_PATH
    if DB_SSL_CA_PATH:
        connect_args["sslrootcert"] = DB_SSL_CA_PATH
    if DB_SSL_MODE:
