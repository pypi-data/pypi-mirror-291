...
def _validate_run_name(name):
    ...
    if not name or not _RUN_NAME_REGEX.match(name):
        raise MlflowException(
            f"Invalid run name {name} . Name should not empty string"
            " may only contain alphanumerics or dashes (-)",
            INVALID_PARAMETER_VALUE,
        )
...
def _validate_run_log_input(
    ...
    if not key or not _RUN_LOG_KEY_REGEX.match(key):
        raise MlflowException(
            f"Invalid run log key: {key} should only contain alphanumeric, hyphen or underscore"
        )
    ...
    if value is not None:
        try:
            ...
            raise MlflowException(
                f"{key} key does not contain a valid json", INVALID_PARAMETER_VALUE
            )
