...
def call_endpoint(host_creds, endpoint, method, json_body, response_dto_cls):
    if method == "GET":
    ...
    response_dto = response_dto_cls.parse_obj(js_dict)
    return response_dto


def call_endpoints(host_creds, endpoints, json_body, response_dto_cls):
    ...
    for i, (endpoint, method) in enumerate(endpoints):
        try:
            return call_endpoint(host_creds, endpoint, method, json_body, response_dto_cls)
...
def cloud_storage_http_request(
    ...
    **kwargs,
    ...
    if "blob.core.windows.net" in url:
        kwargs["headers"] = {"x-ms-blob-type": "BlockBlob"}
