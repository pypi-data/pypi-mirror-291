from mlflow.dto.runs_dto import ParamDto, RunDataDto, RunTagDto
...
class RunData(_MLflowObject):
    ...
    def to_dto(self) -> RunDataDto:
        metrics = []
        params = []
        tags = []
        for metric in self._metric_objs:
            metrics.append(metric.to_dto())
        for key, val in self.params.items():
            params.append(ParamDto(key=key, value=val))
        for key, val in self.tags.items():
            tags.append(RunTagDto(key=key, value=val))
    ...
    def from_dto(cls, dto):
        run_data = cls()
        # iterate dto and add metrics, params, and tags
        for dto_metric in dto.metrics:
            run_data._add_metric(Metric.from_dto(dto_metric))
        for dto_param in dto.params:
            run_data._add_param(Param.from_dto(dto_param))
        for dto_tag in dto.tags:
            run_data._add_tag(RunTag.from_dto(dto_tag))
        return run_data
