...
class FileStore(AbstractStore):
    ...
    def create_experiment(
        self, name, storage_root=None, tags=None, description=None, storage_integration_fqn=None
    ):
        self._check_root_dir()
    ...
    def update_run_info(self, run_id, run_status, end_time, description=SENTINEL):
        ...
        run_info = self._get_run_info(run_id)
        check_run_is_active(run_info)
        new_info = run_info._copy_with_overrides(RunStatus.to_string(run_status), end_time)
        self._overwrite_run_info(new_info)
        return new_info
