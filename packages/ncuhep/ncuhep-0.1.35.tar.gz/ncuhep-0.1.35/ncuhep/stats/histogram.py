...
class Histogram:
    def __init__(self, sample, bin_size=None, confidence_level=0.95, bin_count=None, range=None):
        ...
        if range is not None:
            self.bins = int((range[1] - range[0]) // bin_size + 1)
        else:
            self.bins = int((np.amax(sample) - np.amin(sample)) // bin_size + 1)

        if self.range is not None:
            self.__min = self.range[0] // bin_size - 1
        else:
    ...
    @x.setter
    def x(self, value):
        self._x = value - self.__shift - self.bin_size / 2
