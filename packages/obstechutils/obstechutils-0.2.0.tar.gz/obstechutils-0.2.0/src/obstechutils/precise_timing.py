...
time intervals in seconds and not more, the synchronisation tries to call 
the function at rounded times, e.g. on each minute sharp.
...
"""
    def decorator(f):
        ...
        def wrapper(*args, **kwargs):
            ...
            sync_offset = sync_offset - TimeDelta(call_overhead, format='sec')
            t_target = t_sync + sync_offset
