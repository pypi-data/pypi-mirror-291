...
class _AbroadHexMode(tc.IntegerFlag):
    "\\@since 0.3.26c2. Internal class for class method `Tense.abroadHex()` parameter `mode`"
    INCLUDE = 0
    INCLUDE_HASH = 1
    EXCLUDE = 2
...
class NennaiAbroads:
    ...
    def abroadPack(self, *values: _AbroadPackType[_T]) -> _AbroadInitializer:
        ...
        """
        from tense._primal import reckonLeast
    ...
    def abroadConvect(self, *values: _AbroadConvectType[_T]) -> _AbroadInitializer:
        ...
        i = 0
        if reckon(values) == 0:
            err, s = (tc.MissingValueError, "Expected at least one item in parameter 'values'.")
            raise err(s)
        for e in values:
    ...
    def abroadFloaty(self, value1: _AbroadValue1[_T1], /, value2: _AbroadValue2[_T2] = None, modifier: _AbroadModifier[_T3] = None, div: tc.FloatOrInteger = 10) -> _AbroadFloatyInitializer:
        ...
        """
        ret: list[float] = []
        if not isinstance(div, (int, float, tc.SupportsFloat, tc.SupportsInt, tc.SupportsIndex)):
...
class _SanitizeMode(tc.IntegerFlag):
    "\\@since 0.3.26c2. Interal class for method `Tense.sanitize()` parameter `mode`"
    AROUND = 0
    ALL = 1
    ACF = 2
    LEFT = 3
...
class NennaiRandomize:
    ...
    def randomize(self, sequence: _RandomizeType[_T], /) -> _T:
        ...
        """
        return sequence[ra.randint(0, reckon(sequence) - 1)]
    @_cm
