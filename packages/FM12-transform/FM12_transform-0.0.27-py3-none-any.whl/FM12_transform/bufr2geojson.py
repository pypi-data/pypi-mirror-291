...
LOCATION_DESCRIPTORS = ["latitude", "latitude_increment",
                        ...
                        "height_of_station_ground_above_mean_sea_level", "height_of_station"]
...
class BUFRParser:
    ...
    def get_qualifiers(self) -> list:
        ...
        classes = ("01", "02", "03", "04", "05", "06", "07", "08", "22", "33")
        result = list()
        # name, value, units
        for c in classes:
    ...
    def get_location(self) -> Union[dict, None]:
        ...
        elif "height_of_station" in self.qualifiers["07"]:  # noqa
    ...
    def get_time(self) -> str:
        ...
        second = self.get_qualifier(xx, "second", 0) if "second" in self.qualifiers["04"] else 0
        ...
        while codes_bufr_keys_iterator_next(key_iterator):
            ...
            if xx < 9:
                ...
                if value is not None or description is not None:
                    ...
                    if identification["type"] == "icao_location_identifier":
                        station_id = identification['icao']
                        feature_id = f"ICAO_{identification['icao']}_{characteristic_date}T{characteristic_time}"  # noqa
                    elif identification["type"] == "callsign":
                        station_id = identification['callsign']
                        if sequence == '307090':
                            feature_id = f"SYNOP_MOBIL_{identification['callsign']}_{characteristic_date}T{characteristic_time}"  # noqa
                        else:
                            feature_id = f"SHIP_{identification['callsign']}_{characteristic_date}T{characteristic_time}"
                    else:
                        station_id = identification['wsi']
                        feature_id = f"WIGOS_{identification['wsi']}_{characteristic_date}T{characteristic_time}"  # noqa
