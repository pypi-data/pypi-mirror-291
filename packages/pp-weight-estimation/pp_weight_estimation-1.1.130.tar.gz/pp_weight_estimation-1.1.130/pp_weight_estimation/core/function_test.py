...
def process_pipeline(config_path: str, logger: logging.Logger = None, **kwargs) -> Union[pd.DataFrame, List]:
    ...
    if isinstance(input_df['ground_truth_kg'],list):
        input_df['ground_truth_g']=[input_df['ground_truth_kg'][i]*1000 for i in range(len(input_df['ground_truth_kg']))]
    else:
        input_df['ground_truth_g']=input_df['ground_truth_kg']
...
def chatgpt_only_pipeline(config_path: str, prompt: str = None,use_annotate_data: str = False,gpt_reference:str = False,bb_img:str = False,logger: logging.Logger = None, **kwargs) -> Union[pd.DataFrame, List]:
    ...
    for _, row in input_df.iterrows():
        ...
        if s3_reference_image_path is None:
            ...
            if isinstance(row['s3_reference_image_path'],str):
                s3_reference_weight = row['ground_truth_g']
                rework_g = row['rework_kg']*1000
            if isinstance(row['s3_reference_image_path'],list):
                prompt_multi_ref = "Which image is the most similar to the first image? Just respond with number {'image': value}"   
                multi_ref_res = multi_ref_prompt(s3_image_path, row['s3_reference_image_path'], prompt_multi_ref, gpt_api_key)
                if multi_ref_res:
                    multi_ref_res = eval(multi_ref_res)
                    print(multi_ref_res)
                    multi_ref_res_image = multi_ref_res['image']
                    s3_reference_weight = row['ground_truth_g'][multi_ref_res_image-1]
                    s3_reference_image_path = row['s3_reference_image_path'][multi_ref_res_image-1]
                    rework_g = row['rework_kg']*1000
                else:
                    if logger:
