...
class LoggerExtraDataTestCase(TestCase):
    ...
    def test_http_headers(self):
        self.assertDictEqual(
            self.log._get_extra_data({'http_headers': {'X-Request-ID': '123'}}, logging.ERROR),
            {'http_headers': {'X-Request-ID': '123'}}
        )

    def test_http_payload(self):
        self.assertDictEqual(
            self.log._get_extra_data({'http_payload': {'data': '123'}}, logging.ERROR),
            {'http_payload': {'data': '123'}}
        )

    def test_another_key(self):
        self.assertDictEqual(
            self.log._get_extra_data({'another_key': '123'}, logging.ERROR),
            {'another_key': '123'}
        )

    def test_http_headers_as_param(self):
        func = mock.MagicMock(return_value={'X-Request-ID': 'flask'})
        self.log._http_functions = {'flask': {'http_headers': func}}
        with log_module.LoggerManager(http_headers={'X-Request-ID': 'manager'}):
            self.assertDictEqual(
                self.log._get_extra_data({'http_headers': {'X-Request-ID': 'param'}}, logging.ERROR),
                {'http_headers': {'X-Request-ID': 'param'}}
            )
    ...
    def test_http_headers_as_manager(self):
        ...
        with log_module.LoggerManager(http_headers={'X-Request-ID': 'manager'}):
            self.assertDictEqual(
                ...
                {'http_headers': {'X-Request-ID': 'manager'}}
            )
        func.assert_not_called()
    ...
    def test_http_headers_as_flask(self):
        ...
        with log_module.LoggerManager(http_headers={}):
            self.assertDictEqual(
                self.log._get_extra_data({}, logging.ERROR),
                {'http_headers': {'X-Request-ID': 'flask'}}
            )
...
class LoggerMethodsTestCase(TestCase):
    ...
    def test_critical(self, log_mock: mock.MagicMock):
        ...
        log_mock.log.assert_called_once_with(logging.CRITICAL, 'critical', extra={}, stacklevel=2)

    def test_debug(self, log_mock: mock.MagicMock):
        log = log_module.Logger('everysk-log-methods-test')
        log.debug('debug')
        log_mock.log.assert_called_once_with(logging.DEBUG, 'debug', extra={}, stacklevel=2)

    def test_error(self, log_mock: mock.MagicMock):
        log = log_module.Logger('everysk-log-methods-test')
        log.error('error')
        log_mock.log.assert_called_once_with(logging.ERROR, 'error', extra={}, stacklevel=2)

    def test_info(self, log_mock: mock.MagicMock):
        log = log_module.Logger('everysk-log-methods-test')
        log.info('info')
        log_mock.log.assert_called_once_with(logging.INFO, 'info', extra={}, stacklevel=2)

    def test_warning(self, log_mock: mock.MagicMock):
        log = log_module.Logger('everysk-log-methods-test')
        log.warning('warning')
        log_mock.log.assert_called_once_with(logging.WARNING, 'warning', extra={}, stacklevel=2)

    def test_deprecated(self, log_mock: mock.MagicMock):
        log = log_module.Logger(name='everysk-log-deprecated')
        log.deprecated('Teste')
        log_mock.log.assert_called_once_with(logging.WARNING, 'DeprecationWarning: Teste', extra={}, stacklevel=2)

    def test_deprecated_once(self, log_mock: mock.MagicMock):
        log = log_module.Logger(name='everysk-log-deprecated')
        log.deprecated('Teste 1', show_once=True)
        log.deprecated('Teste 1', show_once=True)
        log_mock.log.assert_called_once_with(logging.WARNING, 'DeprecationWarning: Teste 1', extra={}, stacklevel=2)
    ...
    def test_deprecated_twice(self, log_mock: mock.MagicMock):
        ...
        log_mock.log.assert_has_calls([
            mock.call(logging.WARNING, 'DeprecationWarning: Teste 2', extra={}, stacklevel=2),
            mock.call(logging.WARNING, 'DeprecationWarning: Teste 2', extra={}, stacklevel=2),
        ])
...
class LoggerSlackTestCase(TestCase):
    ...
    def test_danger(self, slack: mock.MagicMock, log_mock: mock.MagicMock):
        ...
        log_mock.log.assert_called_once_with(logging.ERROR, 'Slack message: Error -> Error message', extra={}, stacklevel=2)
    ...
    def test_warning(self, slack: mock.MagicMock, log_mock: mock.MagicMock):
        ...
        log_mock.log.assert_called_once_with(logging.WARNING, 'Slack message: Warning -> Warning message', extra={}, stacklevel=2)
    ...
    def test_success(self, slack: mock.MagicMock, log_mock: mock.MagicMock):
        ...
        log_mock.log.assert_called_once_with(logging.INFO, 'Slack message: Success -> Success message', extra={}, stacklevel=2)
