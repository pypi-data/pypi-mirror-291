...
if [ "$NUM_BASES" -gt 0 ]; then
  for i in $(seq 1 "$NUM_BASES"); do
    if [[ $OSTYPE == 'darwin'* ]]; then
      run_py_in_new_tab_mac "python3 $BASH_DIR/examples/$BASE_SCRIPT -l $LOCAL -s $SP -v true -n true -t true -st true"
    elif is_raspberry_pi; then
      run_py_in_new_win_lxterminal "python3 $BASH_DIR/examples/$BASE_SCRIPT -l $LOCAL -s $SP -v true -n true -t true -st false"
    elif is_ubuntu; then
      run_py_in_new_tab_gnome "python3 $BASH_DIR/examples/$BASE_SCRIPT -l $LOCAL -s $SP -v true -n true -t true -st true"
    else
...
if [ "$NUM_SYNCHRONIZER" -gt 0 ]; then
  if [[ $OSTYPE == 'darwin'* ]]; then
    run_py_in_new_tab_mac "python3 $BASH_DIR/examples/run_synchronizer.py -b $BT -s $SP -d false"
  elif is_raspberry_pi; then
    run_py_in_new_win_lxterminal "python3 $BASH_DIR/examples/run_synchronizer.py -b $BT -s $SP -d false"
  elif is_ubuntu; then
    run_py_in_new_tab_gnome "python3 $BASH_DIR/examples/run_synchronizer.py -b $BT -s $SP -d false"
  else
    echo "Unknown OS or not supported."
  fi
fi