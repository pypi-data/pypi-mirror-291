...
class Colors:
    ...
    def __init__(self, start: str, end: str) -> None:
        if start not in Colors.colors or end not in Colors.colors:
            raise ValueError(f'Color gradient not found: start="{start}", end="{end}"')
        self.start = start
        self.end = end

    def _interpolate(self, t: float, s: tuple[int, int, int], e: tuple[int, int, int]) -> tuple[int, int, int]:
        return tuple(int(s[i] + t * (e[i] - s[i])) for i in range(3))

    def _apply_gradient(self, text: str) -> typing.List[tuple[int, int, int]]:
        gradient = []
    ...
    @classmethod
    def list_gradients(cls) -> typing.List[str]:
