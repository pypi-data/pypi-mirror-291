...
from src.mpypip.create import create_project


@pytest.fixture
def mock_cwd(tmp_path):
    with patch("mpypip.create.getcwd", return_value=str(tmp_path)):
        yield tmp_path
...
def test_create_project(mock_cwd):
    ...
    with (
        patch("src.mpypip.create.Path.mkdir") as mock_mkdir,
        patch("src.mpypip.create.Path.write_text") as mock_write_text,
        patch("src.mpypip.create.Path.touch") as mock_touch,
        patch(
            "src.mpypip.create.create_pyproject_toml",
...
def test_create_project_with_local_deps(mock_cwd):
    with (
        patch("src.mpypip.create.Path.mkdir"),
        patch("src.mpypip.create.Path.write_text"),
        patch("src.mpypip.create.Path.touch"),
        patch("src.mpypip.create.create_pyproject_toml") as mock_create_pyproject,
    ):
        create_project(
            "local_project",
            "Local Author",
...
def test_create_project_existing_directory(mock_cwd):
    with (
        patch("mpypip.create.Path.mkdir") as mock_mkdir,
        patch("mpypip.create.Path.write_text"),
        patch("mpypip.create.Path.touch"),
    ):
