...
import graph_vec_rag.exceptions as ex
...
class Graph_RAG_Engine:
    ...
    def create_knowledge_base(self, file_path: str, knowledge_base_name: str) -> None:
        ...
        if knowledge_base_name in data["knowledge_bases"]:
            raise ex.KnowledgeBaseAlreadyExists(f"Knowledge Base name should be unique. Already existing KBs {data["knowledge_bases"]}")
        ...
        data["knowledge_base_id"].update(
            {knowledge_base_name: unique_id}
        )
    ...
    def load_knowledge_base(self, knowledge_base_name: str) -> bool:
        ...
        if knowledge_base_name not in data["knowledge_bases"]:
            raise ex.KnowledgeBaseNotFound(f"Invalid Knowledge Base name. Existing KBs {data["knowledge_bases"]}")
    ...
    def query_knowledge_base(self, query: str) -> str:
        ...
        if self.graph_vector_rag_query_engine is None:
            raise ex.KnowledgeBaseNotConfigured("Select your KB using load_knowledge_base function before querying.")
