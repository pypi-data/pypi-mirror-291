...
class Danila_rama_text_recognize_number_shift:
    ...
    def rama_text_recognize(self, rama_prod, img_cut, image_text_areas):
        res_labels = self.danila_rama_text_recognize(rama_prod, img_cut, image_text_areas)
        (number_text, _) = res_labels['number']
        if len(number_text) == self.danila_rama_text_recognize.get_number_length():
            return res_labels
        else:
            ...
            res_labels_min = self.danila_rama_text_recognize(rama_prod, img_cut, image_text_areas)
            (number_text_min, _) = res_labels_min['number']
            if len(number_text_min) == self.danila_rama_text_recognize.get_number_length():
                return res_labels_min
            else:
                image_text_areas.areas[Class_text.number] = image_text_areas_max
                res_labels_max = self.danila_rama_text_recognize(rama_prod, img_cut, image_text_areas)
                (number_text_max, _) = res_labels_max['number']
                if len(number_text_max) == self.danila_rama_text_recognize.get_number_length():
