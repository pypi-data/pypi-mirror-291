...
class RetrievalService(ABC):
    ...
    async def retrieve_vectorizing(
        self, queries: list[str], top_k: int, vectorizer_params: dict | None = None, **kwargs
    ) -> list[list[Document]]:
        vectz_kwargs = vectorizer_params or {}
        return await self.retrieve(
            await self.text_vectorizer(queries, **vectz_kwargs), top_k, **kwargs
        )
