...
def decode_file(input_file, output_file, encoding):
    with codecs.open(input_file, 'r', encoding=encoding) as infile, \
         codecs.open(output_file, 'w', 'utf-8') as outfile:
        for line in infile:
            outfile.write(line)

# Locate the input file using pkg_resources
input_file = pkg_resources.resource_filename('dados', 'v2_LIWC2007_Portugues_win.dic')
output_file = pkg_resources.resource_filename('dados', 'v2_LIWC2007_Portugues_win_utf8.dic')

decode_file(input_file, output_file, 'latin-1')

if os.path.exists(output_file):
    parse, category_names = liwc.load_token_parser(output_file)
else:
    print("File decoding failed or the file was not created.")

# Load the LIWC dictionary using the parsed path
