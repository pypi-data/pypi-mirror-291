...
class Grid2D(Grid):
    ...
    def _getInterpCoords(self, geodict):
        ...
        # deal with here:
        # 1) Both xmin/xmax are both positive (left of 180 meridian)
        # 2) Both xmin/xmax are both negative (right of 180 meridian)
        # 3) xmin is positive, xmax is negative (crosses 180 meridian)
    ...
    def interpolateToGrid(self, geodict, method="linear"):
        ...
        if method in ["linear", "cubic"]:
            # RegularGridInterpolator is problematic for a number of reasons,
            # so just passing through to gdal-based method above
            return self.interpolate2(geodict, method=method)
        else:
