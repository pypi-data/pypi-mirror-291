...
def write(
    ...
    Args:
        ...
        do_compression (bool):
            Turn compression on or off. compression_algorithm will be ignored if False.
        compression_algorithm (Compression):
            An enum, described here:
            https://rasterio.readthedocs.io/en/latest/api/rasterio.enums.html#rasterio.enums.Compression
        no_data_value (float): (For TIFFs) Set the no data value in the tiff output. For floating point
            data, this will default to NaN. For integers, it will be set to 0.
    Raises:
    ...
    if format_type in ["netcdf", "hdf"]:
        ...
        is_grid_int = np.issubdtype(grid._data.dtype, np.integer)
        is_no_data_int = no_data_value is not None and isinstance(no_data_value, int)
        both_integer = (is_no_data_int) and (is_grid_int)
        both_float = not is_no_data_int and not is_grid_int
        matching = both_integer or both_float
        if no_data_value is None:
            ...
            if not matching:
                raise TypeError(
                    f"Grid data type does not match no_data value ({no_data_value}) type"
                )
            profile["nodata"] = no_data_value

        # default profile has compression on - turn it off if requested.
        if not do_compression:
            profile["compress"] = Compression.none
        else:
            profile["compress"] = compression_algorithm
