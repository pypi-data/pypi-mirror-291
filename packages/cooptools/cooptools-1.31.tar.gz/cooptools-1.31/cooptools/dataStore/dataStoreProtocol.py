...
class IdsNotInStoreException(Exception):
    def __init__(self, ids: Iterable[UniqueIdentifier]):
        err = f"ids {','.join(ids)} not in store"
        logger.error(err)
        super().__init__(err)
...
class DataStoreProtocol(Protocol):
    ...
    def add(self,
            items: Iterable[StorableData]) -> Dict[UniqueIdentifier, StorableData]:
    ...
    def remove(self,
               ...
               ids: Iterable[UniqueIdentifier] = None) -> Dict[UniqueIdentifier, StorableData]:
        raise NotImplementedError()
    ...
    def get(self,
            ...
            limit: int = None,
            query: Dict = None) -> Dict[UniqueIdentifier, StorableData]:
        raise NotImplementedError()
