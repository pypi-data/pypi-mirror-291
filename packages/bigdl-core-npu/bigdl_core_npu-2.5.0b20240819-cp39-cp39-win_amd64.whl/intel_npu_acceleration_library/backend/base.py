...
class BaseNPUBackendWithPrefetch(BaseNPUBackend):
    ...
    def load_wt_fn(self, offset, module, parameters):
        ...
        """
        backend_lib.setNNFactoryWeights(module, offset, parameters)
    ...
    def setWeights(
        ...
        """
        if wt_hash is None:
            self.load_wt_fn(offset, self._mm, self.create_parameters(args))
            return False
        in_wt_map = wt_hash in self.wt_map.keys()
        if not wt_hash == self.loaded:
            if not in_wt_map:
                self.add_to_map(wt_hash, args)
            self.load_wt_fn(offset, self._mm, self.wt_map[wt_hash])
            self.loaded = wt_hash
            return in_wt_map
        return in_wt_map

    def prefetchWeights(self, offset):
