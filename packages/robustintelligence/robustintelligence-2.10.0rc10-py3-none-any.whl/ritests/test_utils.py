...
class TestRIUtils:
    ...
    def test_await_job_completion(
        self, mocker: MockFixture, mock_client: RIClient, job_status: RimeJobStatus
    ) -> None:
        ...
        job_id = RimeUUID(uuid="test-job-id")
    ...
    def test_await_job_completion_polling(
        ...
        utils = RIUtils(client=mock_client)
        mock_response = mocker.Mock()
        mock_response.job.status = RimeJobStatus.JOB_STATUS_RUNNING
        mock_client.job_reader.get_job.return_value = mock_response
        job_id = RimeUUID(uuid="test-job-id")
