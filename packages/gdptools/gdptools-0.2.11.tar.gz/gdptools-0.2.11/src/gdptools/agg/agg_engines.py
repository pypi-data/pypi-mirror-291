...
class AggEngine(ABC):
    ...
    Returns:
        Tuple[dict[str, AggData], gpd.GeoDataFrame, List[npt.NDArray[Union[np.int, np.double]]]]: The calculated aggregations.
    """
...
class InterpEngine(ABC):
    ...
    def run(
        self,
        *,
        user_data: Union[ODAPCatData, UserCatData, UserTiffData, ClimRCatData, NHGFStacData],
    ...
    def get_variables(self, key) -> dict:
        ...
        elif user_data_type == "ClimRCatData":
...
class SerialInterp(InterpEngine):
    ...
    def grid_to_line_intersection(
        ...
        elif user_data_type in ["ClimRCatData"]:
            grid_proj = self._user_data.cat_dict[key]["crs"]
            x_coord = self._user_data.cat_dict[key]["X_name"]
            y_coord = self._user_data.cat_dict[key]["Y_name"]

        elif user_data_type in ["NHGFStacData", "UserTiffData"]:
            grid_proj = self._user_data.proj_ds
            x_coord = self._user_data.x_coord
            y_coord = self._user_data.y_coord
        ...
        interp_dataarray = xr.merge(
            [interp_dataarray, xr.DataArray(poly_id_array, dims=["pt"], name=self._user_data.id_feature),]
        )
