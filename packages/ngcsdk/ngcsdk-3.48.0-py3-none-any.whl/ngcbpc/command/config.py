...
from ngcbpc.errors import AccessDeniedException, NgcException
...
class ConfigCommand(CLICommand):  # noqa: D101
    ...
    def current(self, args):  # pylint: disable=no-self-use,unused-argument
        """Print the current configuration based on the combination of values in `config` file,
        environment variables, and global flags.
        This command also serves as a way to refresh commands if user's scopes/roles have changed.
        """  # noqa: D205
    ...
    def _set_api_key(self, interactive, base_url, key_name=None):
        ...
        if MULTIPLE_CONFIGURATIONS:
            ...
            while key_input != "no-apikey" and not Authentication.validate_api_key(app_key=key_input):
