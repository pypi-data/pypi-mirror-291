...
class ResourceAPI:  # noqa: D101
        ...
        if not mrt.version:
            if labels or label_set:
                labels_v2 = get_label_set_labels(
                    self.api_client.registry.label_set, self.resource_type, label_set, labels
                )
            else:
    ...
    def remove(self, target: str, default_yes: Optional[bool] = False) -> None:
        ...
        Args:
            ...
            default_yes: Optional; if True, skips the confirmation prompt and proceeds with removal. Defaults to False.
