...
class ImageAPI:  # noqa: D101
    ...
    def copy_metadata(self, target, source) -> None:  # noqa: D102
        ...
        _target = ImageRegistryTarget(target, org_required=True, name_required=True)
        request = PublishingRequest(
            {
                "artifactType": self.resource_type,
                "sourceArtifact": create_publish_artifact(_source.org, _source.team, _source.image),
                "targetArtifact": create_publish_artifact(_target.org, _target.team, _target.image),
            }
        )
    ...
    def copy_version(self, target, source, output=False) -> None:  # noqa: D102
        ...
        if _source.tag is None:
            _source.tag = self.get_repo_details(_source.org, _source.team, _source.image_path()).latestTag
            logger.debug("No tag specified for %s; source using latest tag: %s.", source, _source.tag)
            if output:
                self.printer.print_ok(f"No tag specified for {source}; source using latest tag: {_source.tag}.")

        _target = ImageRegistryTarget(target, org_required=True, name_required=True)
        if _target.tag is None:
            if source:
                _target.tag = _source.tag
                logger.debug("No tag specified for %s; target using implied tag: %s.", target, _target.tag)
                if output:
                    self.printer.print_ok(f"No tag specified for {target}; target using implied tag: {_target.tag}.")
            else:
                raise ArgumentTypeError("Target image tag cannot be infered.")

        request = PublishingRequest(
            {
                "artifactType": self.resource_type,
                "sourceArtifact": create_publish_artifact(_source.org, _source.team, _source.image, _source.tag),
                "targetArtifact": create_publish_artifact(_target.org, _target.team, _target.image, _target.tag),
            }
        )
        ...
        request = PublishingRequest(
            {
                "artifactType": self.resource_type,
                "targetArtifact": create_publish_artifact(_target.org, _target.team, _target.image, _target.tag),
                "publicToPublic": public,
                "publishWithGuestAccess": allow_guest,
                "publishAsListedToPublic": discoverable,
            }
        )
    ...
    def publish_artifact(  # noqa: D102
        ...
        request = PublishingRequest({"artifactType": self.resource_type})

        if source:
            _source = ImageRegistryTarget(source, org_required=True, name_required=True)
            if _source.tag is None:
                _source.tag = self.get_repo_details(_source.org, _source.team, _source.image_path()).latestTag
                logger.debug("No tag specified for %s; using latest tag: %s.", source, _source.tag)
                if output:
                    self.printer.print_ok(f"No tag specified for {source}; using latest tag: {_source.tag}.")
            request.sourceArtifact = create_publish_artifact(_source.org, _source.team, _source.image, _source.tag)

        _target = ImageRegistryTarget(target, org_required=True, name_required=True)
        if _target.tag is None:
            if source:
                _target.tag = _source.tag
                logger.debug("No tag specified for %s; using implied tag: %s.", target, _target.tag)
                if output:
                    self.printer.print_ok(f"No tag specified for {target}; using implied tag: {_target.tag}.")
            else:
                raise ArgumentTypeError("Target image tag cannot be infered.")
        request.targetArtifact = create_publish_artifact(_target.org, _target.team, _target.image, _target.tag)
