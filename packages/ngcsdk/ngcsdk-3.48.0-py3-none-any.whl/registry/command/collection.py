...
class CollectionSubCommand(RegistryCommand):  # noqa: D101
    ...
    def update(self, args):
        ...
        #     )
        labels_v2 = []
        if args.label or args.label_set:
            labels_v2 = get_label_set_labels(self.label_set_api, self.resource_type, args.label_set, args.label)
        else:
            for response in self.collection_api.get_info(
                reg_target.org, reg_target.team, reg_target.name, has_key=True
            ):
                if "collection" in response:
        ...
        collection_request = CollectionUpdateRequest(
            {
                "displayName": args.display_name,
                "labels": None,
                "labelsV2": apply_labels_update(labels_v2, args.add_label or [], args.remove_label or []),
                "logo": args.logo,
                "description": args.overview_filename,  # Read from the markdown file
                "builtBy": args.built_by,
                "publisher": args.publisher,
