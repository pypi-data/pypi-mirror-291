...
try:
    ...
    if _should_fallback or _is_read_session_optional is None:
        _logger.warn(f"Not monkeypatching BQ table download function {_is_read_session_optional=} {_should_fallback=}")
    else:
        try:
            import google.cloud.bigquery._pandas_helpers
        except:
            _logger.warn("Failed to import google.cloud.bigquery._pandas_helpers. Skipping monkey-patch.")
            pass
        else:
