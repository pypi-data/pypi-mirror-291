...
def display_image_row(cols, image_info):
    for col, info in zip(cols, image_info):
        with col:
            st.info("version is latest")
            image_extension = os.path.splitext(info["path"])[1][1:].lower()
            st.write(f"{image_extension}")  # This line is for debugging and can be removed
            html_content = f"""
            <a href="{info["link"]}" target="_blank">
                <div class="image-container"><img src="data:image/{image_extension};base64,{image_to_base64(info["path"])}"/></div>
            </a>
