...
def display_image_row(cols, image_info):
    for col, info in zip(cols, image_info):
        with col:
            image_extension = os.path.splitext(info["path"])[1][1:].lower()  # [1:] to remove the dot

            st.markdown(
                f'<a href="{info["link"]}" target="_blank">'
                f'<div class="image-container"><img src="data:image/{image_extension};base64,{image_to_base64(info["path"])}"/></div>'
                f'</a>'
