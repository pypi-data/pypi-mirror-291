    ...
    class DecoderMLP(nn.Module):
        ...
        def __init__(self, CFG, hidden_dim, dropout, batchnorm):
            ...
            if self.CFG["use_cls"]:
                ...
                input_dim = self.CFG["input_dim"] * self.CFG["hidden_dim"]

            # First layer
            mlp_layers.append(
                LinearLayer(
                    self.CFG, input_dim, self.CFG["hidden_dim"], self.CFG["dropout"]
                )
            )

            # Middle layers (if num_mlp_layers > 2)
            for _ in range(self.CFG["num_mlp_layers"] - 2):
                mlp_layers.append(
                    LinearLayer(
                        self.CFG,
                        self.CFG["hidden_dim"],
            ...
            # Last layer
            mlp_layers.append(
                LinearLayer(self.CFG, self.CFG["hidden_dim"], self.CFG["output_dim"])
            )
