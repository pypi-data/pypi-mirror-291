...
def llinput(*prompt, sep=" ", end='\n', wend='', max_length=None, min_length=None, forceint=False, negativeint=False, forcestr=False, forceinput=False, startswith=("", False), forcestartswith=[], forceendswith=[], choices=([], False), blockedchars=r"", availablechars=r"", forceinputlen=0, autocorrect=False, inputcolor=None, promptcolor=None, endcolor=None, wendcolor=None, inputtype="world"):
    ...
    inputcolor = colors.get(inputcolor.lower(), "") if inputcolor else ""
    promptcolor = colors.get(promptcolor.lower(), "") if promptcolor else ""
    endcolor = colors.get(endcolor.lower(), "") if endcolor else ""
    wendcolor = colors.get(wendcolor.lower(), "") if wendcolor else ""
    ...
    while True:
        ...
        if forcestartswith:
            if ch not in {'\r', '\n', "\b", "\x7f"}:
                match_found = any(input_str.startswith(fs) or (len(input_str) < len(fs) and ch == fs[len(input_str)]) for fs in forcestartswith)
                if not match_found:
                    continue
