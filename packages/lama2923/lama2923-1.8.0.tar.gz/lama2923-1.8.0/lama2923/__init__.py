...
def llinput(*prompt, sep=" ", end='\n', wend='', max_length=None, min_length=None, forceint=False, negativeint=False, forcestr=False, forceinput=False, startswith=("", False), forcestartswith=[], forceendswith=[], choices=([], False), blockedchars=r"", availablechars=r"", forceinputlen=0, autocorrect=False, inputcolor=None, promptcolor=None, endcolor=None, wendcolor=None, inputtype="world"):
    ...
    def _blend_multiple_colors(color_data):
        ...
        if isinstance(color_data, dict):
            for color, weight in color_data.items():
                rgb = rgb_colors.get(color.lower())
                if rgb:
                    for i in range(3):
    ...
    if inputcolor:
        inputcolor = _combine_colors(inputcolor) if isinstance(inputcolor, (dict, list)) else Fore.RESET
    if promptcolor:
        promptcolor = _combine_colors(promptcolor) if isinstance(promptcolor, (dict, list)) else Fore.RESET
    if endcolor:
        endcolor = _combine_colors(endcolor) if isinstance(endcolor, (dict, list)) else Fore.RESET
    if wendcolor:
        wendcolor = _combine_colors(wendcolor) if isinstance(wendcolor, (dict, list)) else Fore.RESET
