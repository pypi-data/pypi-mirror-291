...
def llinput(*prompt,
    ...
    def _parse_color(color):
        if isinstance(color, tuple) and len(color) == 3:
            ...
            if color.startswith('0x'):
                ...
                return rgb_colors.get(color.lower(), (255, 255, 255))
        return (255, 255, 255)
    ...
    def _combine_colors(color_data):
        ...
        return _rgb_to_ansi(blended_rgb) if blended_rgb else _rgb_to_ansi((255, 255, 255))
    ...
    while True:
        ...
        if ch in {'\r', '\n'}:  # Enter key
            ...
            if len(input_str) > 0:
                if custom_delete_ch_check_func:
                    if callable(custom_delete_ch_check_func):
                        if not custom_delete_ch_check_func(input_str, input_str[:-1]):
                            continue
            ...
            if custom_press_key_check_func:
                if callable(custom_press_key_check_func):
                    if not custom_press_key_check_func(input_str, ch):
                        continue
