...
echo "CORE_LIST                = $(taskset -cp $$ | awk -F':' '{print $2}')"
...
time timeout ${timeslurm} python3 run.py "$SLURM_SUBMIT_DIR"  > "$SLURM_SUBMIT_DIR"/out.txt
...
rm -vf EIGENVAL IBZKPT PCDAT PROCAR \
       ELFCAR LOCPOT PROOUT \
       TMPCAR vasp.dipcor
...
if [ $restart = true ]; then
  restart-relaxation --auto-restart --submit -vv --calc-mod="isif=2" --calc-mod="encut=550";
fi
