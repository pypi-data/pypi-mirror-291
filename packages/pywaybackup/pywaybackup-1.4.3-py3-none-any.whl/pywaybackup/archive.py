...
def query_list(queryrange: int, nolimit: bool, start: int, end: int, explicit: bool, mode: str, cdxbackup: str, cdxinject: str):
    ...
    def query(queryrange, nolimit, start, end, explicit):
        vb.write(message="\nQuerying snapshots...")
        query_range = ""
        if not queryrange:
        ...
        nolimit = "" if nolimit else "limit=1000000&"

        vb.write(message=f"-----> {cdx_url}")
        cdxQuery = f"https://web.archive.org/cdx/search/cdx?output=json&url={cdx_url}{query_range}&fl=timestamp,digest,mimetype,statuscode,original&{nolimit}filter!=statuscode:200"
    ...
    if not cdxResult:
        cdxResult = query(queryrange, nolimit, start, end, explicit)
    try:
        cdxResult = json.loads(cdxResult)
    except json.JSONDecodeError:

        vb.write(message="\nERROR: Could not parse json data of CDX response - maybe the connection closed before the response was complete?")
        os._exit(1)
        ...
        # timer = threading.Timer(30, input_timeout)
        # timer.start()

        # vb.write(message="\n---------- ATTENTION ----------")
        # vb.write(message="Could not parse json data of CDX response - response not complete? Maybe try to reduce the range and split into several jobs instead.")
        # vb.write(message="Try to parse partial data? (y/n)")
    ...
    if snapshot_count == 1000000:
        vb.write(message="\nWARNING: Maximum amount of snapshots detected. High possibility that not all snapshots are listed.")
        warning = True
    if snapshot_count > 1000000:
        vb.write(message="\nWARNING: Excessive amount of snapshots detected. System may run out of memory!")
        warning = True
    if warning:
        vb.write(message="---> Consider splitting the query into smaller jobs (range start/end).")
        vb.write(message=f"\nproceeding in...")
        for i in range(10):
            vb.write(message=f"{10-i}")
            time.sleep(1)            
