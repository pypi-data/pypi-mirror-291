...
class DiscoveryConfiguration(_BaseModel):
    ...
    @validator("terraform")
    def check_aws_s3_for_none(cls, val, values):
        terraform = val
        if terraform.tf_state_key is not None:
            if "aws" not in values or values["aws"] is None or values["aws"].s3 is None:
                raise ValueError(
                    "terraform/tf_state_key is defined, but missing aws/s3 attribute"
                )
        return val
