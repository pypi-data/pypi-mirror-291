...
class Filters_Infos(metaclass=DataSingleton):
    ...
    def PrintFilterValues_TempFunction(self):
        print(f"""Filters :
        Connection types :
        contralateral : {self.contralateral_connType}
        homotopic     : {self.homotopic_connType}
        ipsilateral   : {self.ipsilateral_connType}
        other         : {self.other_connType}
        
        Thresholds :
        Weight     : ({self.WeightMin()}, {self.WeightMax()}) {self.discardWeight}
        Abs weight : ({self.AbsWeightMin()}, {self.AbsWeightMax()}) {self.discardAbsWeight}
        Rank       : ({self.RankMin()}, {self.RankMax()}) {self.discardRank}
        
        Regions:
        {self.discardInterRegConn.values()}
