...
class ParameterCatalogue(CamelModel):
    parameters: List[Parameter]

    def create_value_classes(
        self, parameter_catalogue: "ParameterCatalogue", parent_name: str = "InputParameterValue", class_registry=None
    ):
        """
        ...
        Note: Diff between ParameterCatalogue and output of this method (e.g. InputParameterValue)

        `ParameterCatalogue`: enables definition of parameter objects in
        ...
        a module (job). It is populated from ParameterCatalogue.
        """
        if class_registry is None:
            class_registry = {}

        fields = {}
        for param in parameter_catalogue.parameters:
            if param.value_type == "object" or param.value_type.startswith("List[object]"):
                # Create a nested model for 'object' or 'List[object]'
                nested_class_name = f"{camel_case_nested(param.name)}Value"
                nested_catalogue = ParameterCatalogue(parameters=param.parameters)
            ...
            fields[param.name] = (Optional[field_type] if not param.is_required else field_type, Field(default=param.default_value))

        created_class = create_model(parent_name, **fields)
...
class RunModule(CamelModel):
    ...
    parameter_catalogue: Optional[ParameterCatalogue] = None
