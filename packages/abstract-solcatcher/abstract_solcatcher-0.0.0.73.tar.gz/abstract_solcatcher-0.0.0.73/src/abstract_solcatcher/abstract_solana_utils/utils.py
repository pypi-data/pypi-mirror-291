...
def get_addres_lookup_table(txnData):
    return txnData.get('transaction', {})'message', {}).get('addressTableLookups', [])

def get_account_keys(txnData):
    return txnData.get('transaction', {})'message', {}).get('accountKeys', [])

def get_loaded_addresses(txnData):
    return txnData.get('meta'].get('loadedAddresses', [])
...
def get_log_messages(txnData):
    return txnData.get('meta', {}).get('logMessages', [])

def get_instructions(txnData):
  return txnData.get('transaction', {}).get('message'].get('instructions', [])

def get_inner_instructions(txnData):
  return txnData.get('meta', {}).get('innerInstructions', {})[0].get('instructions', [])

def update_instructions(txnData,instructions):
  txnData.get('transaction', {}).get('message','instructions', {}) = instructions
  return  txnData

def update_inner_instructions(txnData,inner_instructions):
  txnData.get('meta', {}).get('innerInstructions', {})[0].get('instructions', []) = inner_instructions
  return txnData
