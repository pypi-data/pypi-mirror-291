...
async def get_rate_limit_url(method_name, *args, **kwargs):
    endpoint = getEndpointUrl("/rate_limit")
    for arg in args:
...
async def log_response(method_name, response_data, *args, **kwargs):
    endpoint = getEndpointUrl("/log_response")
...
async def getAsyncLimitedRequest(endpoint,**kwargs):
    url = await get_rate_limit_url(endpoint)
    response = await asyncPostRequest(url=url,data=kwargs)
    await log_response(endpoint, response_data)
    return response
def callSolcatcherRpc(endpoint,**kwargs):
    response = asyncio.run(getAsyncLimitedRequest(endpoint=endpoint,data=kwargs))
    return response
