...
def get_addres_lookup_table(txnData):
    return txnData['transaction']['message'].get('addressTableLookups', [])

def get_account_keys(txnData):
    return txnData['transaction']['message']get('accountKeys', [])

def get_loaded_addresses(txnData):
    return txnData['meta']get('loadedAddresses', [])
...
def get_log_messages(txnData):
    return txnData['meta']get('logMessages', [])

def get_instructions(txnData):
  return txnData['transaction']['message']get('instructions', [])
