...
async def getAsyncLimitedRequest(endpoint,**kwargs):
    url = await get_rate_limit_url(method_name)
    response = await asyncPostRequest(url=url,data=kwargs)
    await log_response(method_name, response_data)
    return response
def callSolcatcherRpc(endpoint,**kwargs)
    response = asyncio.run(asyncPostRequest(endpoint=endpoint,data=kwargs))
    return response
