...
class MainTable(object):
    def __init__(self, qret_df, qret, rf_df, rf, date, sortbys, alpha_models,
        ...
        if raw_ret:
            self.excess = qret
            self.notes.add("The above table reports raw return")
        else:
            self.excess = 'excess'
            self.notes.add("The above table reports excess return rather than raw return")
    ...
    def get_alphas(self, date, qret_df, sortbys, excess, alpha_models, **kwargs):
        ...
        for n, model in enumerate(alpha_models):
            test_res, model_cols = self.test_mean(date=date, qret_df=qret_df, excess=excess,
