...
class read(object):
    ...
    def bubble_q(self, ek, ekq, niw4f=None, biw=0, norbs=[0,0], 
    ...
    def g4iw(self, norb=1, atom=1, worm=False, biw=0):
        strg = self.iter_str+"/ineq-"+str(atom).zfill(3)
        iwb = len(self.iwb)//2 + biw
        if worm:
            f = h5py.File(self.file,"r")
            g4_all_components = compose_g4iw_worm(f[strg], self.beta, norb, iwb)
            spins = g4_all_components.shape[1]
            #niw4f = g4_all_components.shape[-2]
            niw4f = g4_all_components.shape[-1]
            #niw4b = g4_all_components.shape[-1]
            #g4iw = np.zeros([norb, spins, norb, spins, niw4f, niw4f, niw4b], dtype=complex)
            g4iw = np.zeros([norb, spins, norb, spins, niw4f, niw4f], dtype=complex)
    ...
    def chi_ph(self, biw=0, norb=1, norbs=[0,0], atom=1, worm=False):
        ...
        giw = self.giw(atom=atom, worm=worm)
        try:
            g4iw  = self.g4iw(worm=worm, norb=norb, biw=biw) 
        except KeyError:
        ...
        chi -= get_ggstraight_ph(giw, g4iw.shape[-1])
        chi *= self.beta
...
def compose_g4iw_worm(g4iw, beta, norb, iwb, channel="ph", qtype="value"):
