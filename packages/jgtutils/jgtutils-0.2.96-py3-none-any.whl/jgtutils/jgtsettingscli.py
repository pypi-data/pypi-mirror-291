...
def _init_dotenv_jgt_export_file():
    
    try:
        jgt_batch_path = get_jgt_env_export_path()
        with open(jgt_batch_path,"w") as f:
            #f.write("#!/bin/bash\n")
            f.write("# This file is generated by JGTSettingsCLI and should not be edited manually\n")
...
def _add_value_to_jgt_export_file(key,value,quiet=True):
    try:
        enquote = False
...
def export_env_vars(settings,keys=None,quiet=True):
    _init_dotenv_jgt_export_file()
    for key, value in settings.items():
        if key not in _JGTSET_EXCLUDED_ENV_EXPORT_KEYS:
            if keys is None:
                _export_key(key, value,quiet)
            else:
                if key in keys:
                    _export_key(key, value,quiet)
...
def _export_key(key, value,quiet=True,is_subkey=False):
    if isinstance(value, dict): 
        ...
        for subkey, subvalue in value.items():
            ...
            if isinstance(subvalue, dict):                
                _export_key(subkey_var_name, subvalue,quiet,is_subkey=True)
            else:
        ...
        if  not is_subkey and subkey_str_val != "":
            ...
            _add_value_to_jgt_export_file(key,subkey_str_val,quiet)
    else:
        if isinstance(value, list):
            ...
            _add_value_to_jgt_export_file(key,value,quiet)
...
def print_settings_in_json(_settings,keys=None):
    _what_to_export = _select_keys_in_settings(_settings, keys)
...
def _select_keys_in_settings(_settings, keys):
    _what_to_export = {}
    if keys is not None:
        for key in keys:
            if key in _settings:
...
def main():
    ...
    try:            
        ...
        if args.export_env and not args.json and not args.yml:
            export_env_vars(settings,keys=args.keys,quiet=False if not args.silent else True)
            return

        if args.export_env: #Quiet export with other outputs
            export_env_vars(settings,keys=args.keys,quiet=True)
