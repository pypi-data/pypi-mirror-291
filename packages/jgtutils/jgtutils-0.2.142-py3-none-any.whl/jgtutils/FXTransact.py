...
from jgtutils.jgtos import sanitize_filename;yaml = ruamel.yaml.YAML()
...
class FXTrade:
    ...
    def create_path(use_local):
        fxtransact_filepath=_mkfn_cfxdata_filepath(TRADE_FILE_PREFIX,use_local=use_local)
    ...
    def tojsonfile(self, filename:str=None,use_local=False):
        if not filename:
            filename = self.topath(use_local=use_local)
            # if use_local:
            #     filename = _mkfn_cfxdata_filepath(filename,use_local=use_local) #@STCGoal Gets saved in the local data directory if flag use_local is given without a filename
        try:
            with open(filename, 'w') as f:
                f.write(self.tojson())
        except Exception as e:
...
class FXTransactWrapper:
    ...
    def create_path(trade_id=None,fn=FXTRANSAC_FILE_PREFIX,ext="json",use_local=True):
        ...
        fxtransact_filepath=_mkfn_cfxdata_filepath(fpath,use_local=use_local)
        return fxtransact_filepath
