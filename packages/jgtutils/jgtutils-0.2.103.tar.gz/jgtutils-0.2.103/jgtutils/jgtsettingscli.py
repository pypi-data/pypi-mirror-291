...
def _select_keys_in_settings_or_env(_settings, keys):
    ...
    try:
        ...
        pass      
    try:
        if 'jgtset_included' in _settings:
            for key in _settings['jgtset_included'].split(","):
                if key in _settings and key not in _what_to_export:
                    _what_to_export[key] = _settings[key]
            _what_to_export.pop('jgtset_included')
    except:
        pass    
