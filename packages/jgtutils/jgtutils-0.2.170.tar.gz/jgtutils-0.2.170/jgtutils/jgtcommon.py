...
def get_timeframes(default_timeframes):
    _timeframes=args.timeframes if args.tflag else default_timeframes if not args.timeframe else [args.timeframe] if args.timeframe else os.getenv("T",TIMEFRAMES_DEFAULT_STRING).split(",")
    return _timeframes
...
def get_instruments(default_instruments):
    _instruments=args.instruments if args.iflag else default_instruments if not args.instrument else [args.instrument] if args.instrument else os.getenv("I",INSTRUMENT_ALL_STRING).split(",")
    return _instruments
