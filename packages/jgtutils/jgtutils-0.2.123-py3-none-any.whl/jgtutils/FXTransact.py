...
from jgtfxhelper import offer_id_to_instrument as _offer_id_to_instrument,mkfn_cfxdata_filepath as _mkfn_cfxdata_filepath 
...
class FXTrade:
    ...
    def _to_filename(self,ext="json",fn=TRADE_FILE_PREFIX):
        return FXTrade.mkfilename(self.trade_id,ext,fn)
    
    @staticmethod
    def mkfilename(trade_id,ext="json",fn=None):
        if fn is None:
    ...
    def mkpath(trade_id,ext="json",use_local=True,fn=None):
        fn=FXTrade.mkfilename(trade_id,ext,fn)
...
class FXOrder:
    ...
    def _to_filename(self,ext="json",fn=ORDER_FILE_PREFIX):
        return FXOrder.mkfilename(self.order_id,ext,fn)
    
    @staticmethod
    def mkfilename(order_id,ext="json",fn=None):
        if fn is None:
    ...
    def mkpath(order_id,ext="json",use_local=True,fn=None):
        fn=FXOrder.mkfilename(order_id,ext,fn)
        return _mkfn_cfxdata_filepath(fn,use_local=use_local)
