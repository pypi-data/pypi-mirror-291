...
def get_timeframes(envname = None, default_timeframes = None):
    if envname is not None and envname in os.environ:
        return os.getenv(envname).split(",")
    iterable_timeframes = _get_iterable_timeframes_from_args()
    if iterable_timeframes is None or len(iterable_timeframes) == 1:
        iterable_timeframes=default_timeframes
    return iterable_timeframes
...
def get_instruments(envname = None,default_instruments = None):
    if envname is not None and envname in os.environ:
        return os.getenv(envname).split(",")
    iterable_instruments = _get_iterable_instruments_from_args()
    if iterable_instruments is None or len(iterable_instruments) ==1:
        iterable_instruments=default_instruments
...
def __timeframes_post_parse()->argparse.Namespace:
    ...
    _timeframes=None
    if not hasattr(args, 'timeframe') or args.timeframe is None:
        return args
    
    if hasattr(args, 'timeframe') and ","  in getattr(args, "timeframe"):
...
def __instruments_post_parse()->argparse.Namespace:
    ...
    if hasattr(args, 'instrument') and "," in getattr(args, "instrument"):
        setattr(args, 'iflag',True)
    else:
        setattr(args, 'iflag',False)
    
    if hasattr(args, "instruments"):
        _instruments=getattr(args, "instruments")
    else:
        try:
    ...
    if _instruments is None and hasattr(args, 'instrument'):
        _instruments=getattr(args, 'instrument')
        #if we have coma in the string
        if _instruments is not None and "," in _instruments:
            _instruments=parse_instruments_helper(_instruments)
        else:
            _instruments = os.getenv("T",None)
