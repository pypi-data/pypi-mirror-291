...
class Preprocess(Workflow[Input, Any, Queues, Params, Artifacts, Pipelines]): # type: ignore
  ...
  def run(self, queues: Queues, params: Params) -> Artifacts:
    ...
    procs = {
      'extract': self.pipelines['extract'].run(queues['extract'], extr.Params(images=images, logger=logger.prefix('[EXTRACT]'), descaled_h=descaled_h, autocorrect=autocorrect)),
