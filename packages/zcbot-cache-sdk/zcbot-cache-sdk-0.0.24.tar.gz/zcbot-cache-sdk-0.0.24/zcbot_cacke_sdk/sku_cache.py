    ...
    def save(self, sku: SkuModel):
        ...
        try:
            ...
            if self.mongo_update_bulk and len(self.mongo_update_bulk) >= self.batch_size:
                # Mongo保存操作
                self.mongo_sku_collection.bulk_write(self.mongo_update_bulk, ordered=False, bypass_document_validation=True)
                logger.info(f'数据写入: {len(self.mongo_update_bulk)}')
                self.mongo_update_bulk.clear()
                # Redis插入操作
                self.rds_client.sadd(self.rds_cache_sku_key, *self.rds_update_bulk)
                logger.info(f'缓存写入: {len(self.rds_update_bulk)}')
    ...
    def flush(self):
        try:
            ...
            if self.mongo_update_bulk:
                self.mongo_sku_collection.bulk_write(self.mongo_update_bulk, ordered=False, bypass_document_validation=True)
                logger.info(f'flush数据写入: {len(self.mongo_update_bulk)}')
                self.mongo_update_bulk.clear()
                # Redis插入操作
                self.rds_client.sadd(self.rds_cache_sku_key, *self.rds_update_bulk)
                logger.info(f'flush缓存写入: {len(self.rds_update_bulk)}')
                self.rds_update_bulk.clear()
        except Exception as e:
            logger.error(type(e))
