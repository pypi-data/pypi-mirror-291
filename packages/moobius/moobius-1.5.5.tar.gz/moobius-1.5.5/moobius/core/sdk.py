...
class Moobius:
    ...
    def __init__(self, config: Optional[str|dict]=None, account_config: Optional[str|dict]=None, service_config: Optional[str|dict]=None, db_config: Optional[str|dict]=None, log_config: Optional[str|dict]=None):
        ...
        self._set_config(config, account_config, service_config, db_config, log_config)
        self._set_loguru()
    ...
    def _set_config(self, config: Optional[str|dict]=None, account_config: Optional[str|dict]=None, service_config: Optional[str|dict]=None, db_config: Optional[str|dict]=None, log_config: Optional[str|dict]=None):
        """Sets self.config, self.client_id, self.service_mode, and self._save_new_service_id. Accepts the generic config, the account credentials, the channels and more config, the homegrown database engine config, and the log-related config. Returns None"""

        # Settings, settings, settings...
        if not config:
            config = {}
        config = utils.recursive_json_load(config)
        ...
        if not config['log_config']:
            config['log_config'] = {}
        config['log_config']['log_level'] = log_config.get("log_level", "INFO")
        config['log_config']['log_retention'] = log_config.get('log_retention', {'rotation':"1 day", 'retention':"7 days"})
        config['log_config']['log_file'] = log_config.get('log_file', None)
        config['log_config']['error_log_file'] = log_config.get('error_log_file', config['log_config']['log_file'])
        config['log_config']['error_log_level'] = log_config.get('error_log_level', "WARNING")
        config['log_config']['terminal_log_level'] = log_config.get('terminal_log_level', "INFO")
        if type(config['log_config']['log_retention']) is str:
            config['log_config']['log_retention'] = {'retention':config['log_config']['log_retention']}
        config['log_config']['log_level'] = config['log_config']['log_level'].upper()
        config['log_config']['error_log_level'] = config['log_config']['error_log_level'].upper()

        the_id = service_config.get("service_id", "")
        self.service_mode = service_config.get('service_mode', True)
        ...
        if self.service_mode:
            self.client_id = service_config.get("service_id", "")
        if service_config and type(service_config) is str:
            self._save_new_service_id  = [service_config, ['service_id']] # Args to self.utils.update_jsonfile. Cannot be a function because it must pickle and be sent to another process.
        elif config and type(config) is str:
            self._save_new_service_id = [config, ['service_condig', 'service_id']]
        else:
            print("WARNING: No way to save service_id because no JSON.") # Logger may not be set up yet.
            self._save_new_service_id = None
        self.config = config
    ...
    def _set_loguru(self):
        ...
        #utils.set_terminal_logger_level(self.config['log_config']['log_level'])
        # logger.configure()
    ...
    async def start(self):
        ...
        if self.service_mode:
            if not self.client_id:
                ...
                if self._save_new_service_id:
                    utils.update_jsonfile(*(self._save_new_service_id+[self.client_id]))
    ...
    async def handle_received_payload(self, payload):
        ...
        if 'recipients' in payload_body:
            ...
            payload_body['recipients'] = await _group2ids(rec_group) # Use this instead of payload.body because less magical which-dataclass inference.
        ...
        if 'type' in payload_data:
            ...
            if payload.type == types.MESSAGE_DOWN:
                ...
                recipients = []
                if 'recipients' in payload_body and payload_body['recipients'] not in [types.SERVICE, 'null', '', None, False]:
                ...
                update = UpdateBody(**{**payload_body, **{'content':content, 'recipients':recipients}})
                await self.on_update(update)
            elif payload.type == types.MESSAGE_UP:
                # Spelling out all these sub-constructors helps catch errors more easily rather than "well, actually it stayed a dict."
                # TODO: Offer a utils class that does this automatically or figure out how to get dataclasses to do so.
                payload_body['content'] = MessageContent(**payload_body['content'])
