...
log_template = {
    ...
    "error_log_file":"logs/serviceerror.log"
}
db_template = {}
...
def submit(out):
    ...
    kys = ['main.py', 'readme.md', 'service.py', 'config/db.json', 'config/service.json', 'config/account.json', 'config/log.json']
    ...
    if out['template'] == 'Bot puppet':
        kys.extend(['user.py','config/usermode_service.json', 'config/usermode_account.json', 'config/usermode_db.json', 'config/usermode_log.json'])
    for ky in kys:
