        ...
        if service_config and type(service_config) is str:
            ...
            with open(config, 'r', encoding='utf-8') as f:
                ...
                if type(_sconf) is str:
                    self._save_new_service_id = [_sconf, ['service_id']]
                elif type(_sconf) is dict:
                    self._save_new_service_id = [config, ['service_config', 'service_id']]

        if not self._save_new_service_id and not service_config.get('no_warn_cannot_save_service_id'):
    ...
    async def start(self):
        ...
        if self.service_mode:
            if not self.client_id:
                ...
                if self._save_new_service_id: # None if no config json file given.
        ...
        self.scheduler.add_job(logger.catch(self.on_checkin), 'interval', seconds=self.checkin_interval) # This check-in must be after on_start()
