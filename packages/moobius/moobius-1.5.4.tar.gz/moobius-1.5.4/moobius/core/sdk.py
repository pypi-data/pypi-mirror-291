...
class Moobius:
    ...
    def __init__(self, config: Optional[str|dict]=None, account_config: Optional[str|dict]=None, service_config: Optional[str|dict]=None, db_config: Optional[str|dict]=None, log_config: Optional[str|dict]=None):
        ...
        """
        # Where to save the service id?
        if service_config and type(service_config) is str:
            self._save_new_service_id  = [service_config, ['service_id']]
        ...
        self.log_level = log_config.get("terminal_log_level", "INFO")
        self.log_retention = log_config.get('log_retention', {'rotation':"1 day", 'retention':"7 days"})
        self.log_file = log_config.get('log_file', None)
        self.error_log_file = log_config.get('error_log_file', None)
        self.error_log_level = log_config.get('error_log_level', "WARNING")
        ...
        self.client_id = service_config.get('service_id', None)
        ...
        http_server_uri = service_config.get("http_server_uri", "https://api.moobius.ai/")
        ws_server_uri = service_config.get("ws_server_uri", "wss://ws.moobius.ai/")
        email = account_config.get("email", None)
        password = account_config.get("password", None)
