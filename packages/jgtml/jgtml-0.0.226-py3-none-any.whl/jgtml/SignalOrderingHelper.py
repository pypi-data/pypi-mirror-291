...
def create_fdb_entry_order(i,signal_bar,current_bar,lots=1,tick_shift=2,quiet=True,valid_gator_mouth_open_in_mouth=False,valid_sig_out_mouth=True,t=None,validation_timestamp=None,verbose_level=0):
  ...
  if  current_bar_broke_signal and had_valid_signal:
    if verbose_level>0:print(f"## Current Bar Broke the Signal {i} {t} ")
    return None
  ...
  if valid_gator_mouth_open_in_mouth \
    ...
    if verbose_level>0:print(f"## Invalid Gator {i} {t} valid_gator_mouth_open_in_mouth")
    return None
  
  if valid_sig_out_mouth \
    and \
      not is_bar_out_of_mouth(current_bar,buysell):
    if verbose_level>0:print(f"## Invalid Gator {i} {t} not valid_sig_out_mouth")
    return None
