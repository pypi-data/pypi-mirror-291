                            ...
                            for session_dir in os.listdir(subject_path):
                                ...
                                if os.path.isdir(session_path):
                                    if has_bids_and_scans_files(session_path):
                                        ...
                                        exp_label = re.search(r'ses-\d+', session_dir)
                                        root = os.path.join(output_project_path)
                                        if exp_label:
                                            output = os.path.join(output_project_path, subject_dir, exp_label.group(0))
                                        else:
                                            output = os.path.join(output_project_path, subject.project, subject.label)
                                        convert2bids(session_path, root, output)
