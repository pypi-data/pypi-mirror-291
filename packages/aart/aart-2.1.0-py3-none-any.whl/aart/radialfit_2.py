...
def radialfit(visamps, freq, radonangles, mu_n, s_n, ulim1_fit, ulim2_fit, gfit_min, path, p0_ratio):
    ...
    if not isExist:
        os.makedirs(path)
    np.savez(path+"Visamp_fit_%s_%s_gfit_%s_mun_%s_sn_%s.npz"%(ulim1_fit,ulim2_fit, gfit_min, mu_n, s_n),**param_dict)

def radialfit_visibility(visamps, freq, radonangles, mu_n, s_n, ulim1_fit, ulim2_fit, gfit_min, path, p0_ratio, spin_case, i_case, gammap, mup, sigmap):
    """
    ...
    if not isExist:
        os.makedirs(path)
    np.savez(path+"Visibility_fit_%s_%s_gfit_%s_mun_%s_sn_%s_a_%s_i_%s_gp_%s_m_%s_s_%s.npz"%(ulim1_fit,ulim2_fit, gfit_min, mu_n, s_n, spin_case, i_case, gammap, np.round(mup, 3), sigmap),**param_dict)
