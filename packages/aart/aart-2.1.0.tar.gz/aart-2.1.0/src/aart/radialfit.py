...
# functional form of the visibility with aL and aR as functions of u using the upper and lower envelopes
def visamp_func_3(u, a, b, aL, aR, dphi):
    return np.exp(-a * (u ** b)) * np.sqrt(aL**2 + aR**2 + 2*aL*aR*np.sin(2*np.pi*dphi*u*muas_to_rad*1e9)) / np.sqrt(u)

## goodness of fit
def g_fit_3(a, b, aL, aR, d, x, y):
    deviation= np.sum((visamp_func_3(x, a, b, aL, aR, d)-np.abs(y))**2)
    avg_v_fit = np.sum(visamp_func_3(x, a, b, aL, aR, d))
    return np.exp(-np.sqrt(deviation/len(x))/(avg_v_fit/len(x)))

def compute_visamp_fit_3(freqs, visamp, a_max=20, b_max=3):
    vmax=np.max(visamp)
    umax=freqs[np.argmax(visamp)]
    vmin=np.min(visamp)
    umin=freqs[np.argmin(visamp)]
    ...
    # finding the best fit d_\varphi with prior that d\in[35, 45]
    params, pcov_v = curve_fit(visamp_func_3, freqs, visamp,p0 = p0, bounds=bounds, maxfev=6000)
...
def radialfit(visamps, freq, radonangles, mu_n, s_n, ulim1_fit, ulim2_fit, gfit_min, path, p0_ratio, method):
    ...
    try:
        for i in range(len(radonangles)):
            ...
            if method=="GLM":
            ...
            elif method=="Exponential 2":
                params = compute_visamp_fit_3(freqfit, data)
                a = params[0]
                b = params[1]
                aL = params[2]
                aR = params[3]
                ...
                ## corresponding goodness of fit for each d \in drange
                gvals = [g_fit_3(a, b, aL, aR, d, freqfit, data) for d in drange]
...
def radialfit_visibility(visamps, freq, radonangles, mu_n, s_n, ulim1_fit, ulim2_fit, gfit_min, path, p0_ratio, method):
    """
