...
----------
### 增强功能  
*   在[**pay (结账支付)**](https://global.alipay.com/docs/ac/ams/payment_cashier)、[**inquiryPayment (支付查询)**](https://global.alipay.com/docs/ac/ams/paymentri_online)和[**notifyPayment (支付通知)**](https://global.alipay.com/docs/ac/ams/paymentrn_online)API中添加了_funding_和_cardCategory_字段。
*   在[**consult (结账支付)**](https://global.alipay.com/docs/ac/ams/consult)API中添加了_funding_字段。
*   在[**pay (结账支付)**](https://global.alipay.com/docs/ac/ams/payment_cashier)API中添加了_requireIssuerAuthentication_字段。
*   更新了[**pay (结账支付)**](https://global.alipay.com/docs/ac/ams/payment_cashier)API中_selectedCardBrand_字段的描述。
*   在[**pay (结账支付)**](https://global.alipay.com/docs/ac/ams/payment_cashier)、[**createPaymentSession (结账支付)**](https://global.alipay.com/docs/ac/ams/session_cashier)和[**decide**](https://global.alipay.com/docs/ac/risk_control/decide)API中添加了_transit_、_lodging_和_gaming_字段。
*   添加了嵌入式Web/WEP卡支付集成指南。  
2024年2月
-------------  
### 增强功能  
*   添加了[**acceptDispute (接受争议)**](https://global.alipay.com/docs/ac/ams/accept)、[**supplyDefenseDocument (提供辩护文件)**](https://global.alipay.com/docs/ac/ams/supply_evidence)、[**downloadDisputeEvidence (下载争议证据)**](https://global.alipay.com/docs/ac/ams/download)和[**notifyDispute (通知争议)**](https://global.alipay.com/docs/ac/ams/notify_dispute)的争议相关API。
*   在[**notifyDispute (通知争议)**](https://global.alipay.com/docs/ac/ams/notify_dispute)API的_disputeNotificationType_字段中添加了枚举值`DEFENSE_DUE_ALERT`。
*   更新了[**notifyDispute (通知争议)**](https://global.alipay.com/docs/ac/ams/notify_dispute)API中_defenseDueTime_字段的描述。  
2024年1月
------------  
### 增强功能
* 添加了Yapily作为支付方式到[支付方式](https://global.alipay.com/docs/ac/ref/payment_method)文档中。
* 在[**创建订阅**](https://global.alipay.com/docs/ac/ams/create_sub)API中添加了_paymentMethodId_字段。
* 在[**创建订阅**](https://global.alipay.com/docs/ac/ams/create_sub)API的_paymentMethodType_字段中添加了枚举值`CARD`。
* 更新了[**创建订阅**](https://global.alipay.com/docs/ac/ams/create_sub)API中_subscriptionEndTime_字段的描述。
...
* 在[**pay (结账支付)**](https://global.alipay.com/docs/ac/ams/payment_cashier)API的字段_paymentMethod.paymentMethodMetaData_中添加了Pagaleve的使用场景。
* 更新了[**pay (结账支付)**](https://global.alipay.com/docs/ac/ams/payment_cashier)API中字段_order.buyer_和_order.buyer.buyerPhoneNo_的描述。
...
*   为欧洲地区的商户服务器添加了加速域名 [https://open-de-global.alipay.com](https://open-de-global.alipay.com)。
...
--------
### 增强功能  
*   为收银台支付和自动扣款添加了枚举值 `NAVERPAY`，以及为收银台支付添加了 `TOSS`，这些在[_支付方式_](https://global.alipay.com/docs/ac/ref/payment_method)文档中已有说明。
...
*   将`Request-Time`的格式修改为毫秒级精确的时间戳。详细信息请参阅[构造待签名内容](https://global.alipay.com/docs/ac/ams/digital_signature#构建待签名内容)。  
2023年5月
----------
### 改进  
...
*   更新了[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier) API 的请求参数中，针对Blik支付方式的 _order.env.clientIp_ 和 _order.env.userAgent_ 参数的描述。
...
*   在《使用非银行卡支付方式》的《获取异步通知》部分中，添加了Mercado Pago默认订单过期时间的注释。
*   在《使用非银行卡支付方式》的《收集或显示额外信息》部分中，添加了对Mercado Pago的描述。
...
*   将[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier) API 的请求参数中的 _allowedPspRegions_ 参数改名为 _allowedPaymentMethodRegions_。
...
* 新增参数`issuingCountry`，`funding`和`cardBrand`。更多详情，请参考以下文档：
  * [收银台交易项](https://global.alipay.com/docs/ac/cashierpay/transactionitems) - 收银台支付
  * [收银台结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付
* 新增参数`interchangeFeeAmountValue`，`interchangeFeeCurrency`，`schemeFeeAmountValue`，`schemeFeeCurrency`，`AcquirerMarkupFeeAmountValue`和`AcquirerMarkupFeeCurrency`。更多详情，请参考以下文档：
  * [收银台结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付
...
*   更新了参数_feeAmountValue_的描述。更多详情，请参阅以下文档：  
    *   [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
...
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier) API 的请求参数中，将 _blikCode_ 和 _payerEmail_ 作为子参数添加到 _paymentMethodMetaData_ 的字段中。
...
* 添加了字段`installmentsNum`。更多信息，请参考以下文档：
...
* 添加了字段`disputeHandlingFee`, `disputeHandlingFeeCurrency`, `disputeReverseFee`, 和 `disputeReverseFeeCurrency`。更多信息，请参考以下文档：
  * [结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付
...
*   更新了[请求签名和验证签名](https://global.alipay.com/docs/ac/ams/digital_signature)文档。
...
* 修改了以下接口示例代码中的海关代码：
    * 声明（declare）：将ZHENGZHOU修改为ZONGSHU
    * 查询声明请求（inquiryDeclarationRequests）：将shenzhen修改为ZONGSHU
...
*   删除以下API中的 _orderCodeForm.codeDetails.codeValueType_ 字段：
    *   [收银台支付](https://global.alipay.com/docs/ac/ams/payment_cashier)
    *   [自动扣款支付](https://global.alipay.com/docs/ac/ams/payment_agreement)
*   2021年5月
...
| **废弃术语** | **当前术语** | **备注** |
| --- | --- | --- |
| PSP | PMP | 废弃术语可能仍然显示在代码中，如API规范、Java代码规范或其他技术元素存在的地方。废弃术语的含义没有改变。 |
**2023年12月**
----------------
...
**2024年支付宝 法律信息** [https://global.alipay.com/docs/ac/platform/membership](https://global.alipay.com/docs/ac/platform/membership)
