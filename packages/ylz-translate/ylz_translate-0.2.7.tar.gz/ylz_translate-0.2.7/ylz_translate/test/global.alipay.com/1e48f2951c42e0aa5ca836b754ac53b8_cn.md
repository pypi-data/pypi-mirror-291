...
*   调用API时，您必须对发送给支付宝的请求进行签名，并相应地验证支付宝响应的签名。详情请参阅[调用API](#wPgXI)。
*   接收到通知时，您必须验证支付宝请求的签名。但不需要对通知的响应进行签名。详情请参阅[接收通知](#WMRu5)。
...
私钥用于生成请求的签名，您需要在**集成设置**下的**设置公钥**部分提交非对称公钥。支付宝使用公钥来验证您的请求签名。
...
![图片3：image.png](https://idocs-assets.marmot-cloud.com/storage/idocs87c36dc8dac653c1/1664440939174-81dee3b9-1fe5-426c-ae5f-1ef47e716c00.png)
...
*   `客户端ID`: 用于标识客户端。例如，`TEST_5X00000000000000`。可以从请求头中获取此字段。
*   `请求时间`: 指定请求发送的时刻。此字段的值必须精确到毫秒。例如，`1685599933871`。可以从请求头中获取时间戳。
*   `请求正文`: 请求的数据体。例如：  
复制  
```json
{
...
1. 根据以下语法组装签名字符串：  
   ...
   *   `generatedSignature`: 在[步骤2](#gNWs0)中生成的签名。  
   ...
   2. 将签名字符串添加到请求头中。关于请求头的详细信息，参见[消息结构](https://global.alipay.com/docs/ac/ams/api_fund#ML5ur)章节。  
   发送请求
   --------------
   通过在请求头中添加`Client-Id`, `Request-Time`, 和 `Signature` 属性来构造请求。构造好请求后，可以使用通用工具，如cURL或Postman发送请求。以下示例使用了cURL：  
   ```
   curl -X POST \
...
*   `HTTP-方法`：POST
*   `HTTP-URI`：例如，如果HTTP URL是[https://open-na-global.alipay.com/ams/api/v1/payments/pay](https://open-na-global.alipay.com/ams/api/v1/payments/pay)，此字段为`/ams/api/v1/payments/pay`。
*   `Client-Id`：用于识别客户端。您可以从响应头中获取此字段。
...
// 如果签名在响应中不存在，AMS返回SIGNATURE_INVALID
...
### 接收通知
#### 处理请求
在接收到支付宝的通知后，验证请求的签名。验证请求签名的过程与[处理响应](#J8HGf)部分介绍的过程类似。验证签名的步骤如下：
1.  获取用于验证签名的支付宝请求公钥。
2.  按照`待验证内容`的语法构建请求：
```  
<HTTP-方法> <请求-URI>
<Client-Id>.<请求-时间>.<请求-正文>  
```
3.  从请求头中获取签名。
4.  验证签名。
...
[步骤2：构建待验证内容](#hvhQp "步骤2：构建待验证内容")  
[步骤3：从响应头中获取签名](#FJhqP "步骤3：从响应头中获取签名")  
[步骤4：验证签名](#lt4nS "步骤4：验证签名")  
[接收通知](#WMRu5 "接收通知")  
[处理请求](#zQi3l "处理请求")