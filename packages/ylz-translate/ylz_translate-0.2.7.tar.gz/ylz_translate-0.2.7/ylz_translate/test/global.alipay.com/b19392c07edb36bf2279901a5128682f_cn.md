...
订阅更新
订阅产品更新，以接收关于文档更新的电子邮件通知。您可以通过右侧的“取消订阅”按钮或每封电子邮件通知中提供的链接取消订阅。  
...
*   更新了在《收银台支付》API的请求参数中，参数`order.shipping.shippingPhoneNo`, `order.buyer.buyerPhoneNo`, `order.buyer.buyerName.firstName`, 和 `order.shipping.shippingName.firstName`的描述。
*   将《收银台支付》、《自动扣款》、《用户出示模式支付》、《订单码支付》和《入口码支付》API的请求参数中，参数`clientIp`的长度从32位改为64位。
*   更新了《在线支付通知》API的请求参数中，参数`result.resultStatus`的描述。
*   更新了在《支付方式》中巴西、秘鲁、墨西哥和智利支持的支付卡品牌。
*   删除了在《取消》中关于已取消交易不包含在结算文件中的说明。
*   在《使用非银行卡支付方式》的《获取异步通知》部分，添加了Mercado Pago默认订单过期时间的注解。
...
* 在_field_ _paymentMethodType_中添加了枚举值`KONBINI`, `FPX`, 和 `PAYEASY`。更多详情，请参阅以下文档：
...
* 在_field_ _paymentMethodType_中添加了枚举值`PAYPAY`和`GrabPay`。更多详情，请参阅以下文档：
...
* 更新了“示例5”部分的报告样本。更多详情，请参阅以下文档：
  * [收银台支付结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
  * [自动扣款结算项目](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
* 更新了参数_originalTransactionId_的描述。更多详情，请参阅以下文档：
...
*   修改了文件路径命名规则和字段 _seq_ 的描述。更多详情，请参阅以下文档：  
    ...
    *   [自动扣款结算汇总](https://global.alipay.com/docs/ac/autodebitpay/settlementsummary)
* 更新了字段 _customerId_ 的描述。详细信息请参考以下文档：
    * [收银台支付的交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
    * [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
    * [收银台支付的结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary)
...
* 添加了字段 _acquirer_。详细信息请参考以下文档：
    ...
    * [自动扣款支付的结算项目](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
    * [自动扣款支付的结算汇总](https://global.alipay.com/docs/ac/autodebitpay/settlementsummary)
* 添加了字段 _acquirerReferenceNo_。详细信息请参考以下文档：
    * [收银台支付的交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
    * [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
    * [自动扣款支付的交易项目](https://global.alipay.com/docs/ac/autodebitpay/transactionitems)
    * [自动扣款支付的结算项目](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
...
*   删除以下字段：
    *   请求参数：
        *   是否授权
        *   支付因子.isPaymentEvaluation
        *   支付方式.paymentMethodMetaData
        *   支付验证数据
        *   payToMethod
    *   响应参数：
        *   授权过期时间
        ...
        *   订单代码表单
*   添加了结算策略字段。
*   将order.env和order.env.userAgent字段从可选改为必需。
**2021年12月**
---------------  
...
*   在线支付和店内支付产品的全新版本API于12月发布。API文档格式已更新为新版本，以提供更好的用户体验。现在，您可以在同一文档中访问子字段，而以前子字段仅在数据字典中可用。您可以前往**API参考** > **产品API** > **在线支付**（[https://global.alipay.com/docs/ac/ams/payment\_cashier](https://global.alipay.com/docs/ac/ams/payment_cashier)）查看新版本。
