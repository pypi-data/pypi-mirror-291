...
[首页](/docs/zh-cn/)  
[在线支付](/docs/zh-cn/onlinepayment)  
[店内支付](/docs/zh-cn/instorepayment)  
[解决方案](/docs/zh-cn/solutions)  
[营收提升](/docs/zh-cn/ac/revenuebooster_en/overview)  
[合并支付](/docs/zh-cn/ac/combinedpay_en/overview)  
[灵活结算](/docs/zh-cn/ac/flexiblesettlement_en/overview)  
[集成指南](/docs/zh-cn/integration_guide_en)  
[Antom 控制台](/docs/zh-cn/dashboard_en)  
[SDKs](/docs/zh-cn/sdks)  
...
[版本更新](/docs/zh-cn/releasenotes)  
[支持](/docs/zh-cn/support)  
[词汇表](/docs/zh-cn/glossary)  
...
*   添加了增值服务的文档：[收入增长器](https://global.alipay.com/docs/ac/revenuebooster_en/overview)，[组合支付](https://global.alipay.com/docs/ac/combinedpay_en/overview)和[灵活结算](https://global.alipay.com/docs/ac/flexiblesettlement_en)。
...
*   在[支付方式](https://global.alipay.com/docs/ac/ref/payment_method)中添加了卡品牌的枚举值 `AMEX`，`DISCOVER`，`DINERS`，`CUP`，`JCB`，`MAESTRO` 和 `CARTES_BANCAIRES`。
...
* 更新了[自动扣款](https://global.alipay.com/docs/ac/autodebit_en/overview)文档。新版本提供了重新结构化的集成和操作指南，遵循支付流程。
* 将产品名称从收银台支付更改为结账支付。
...
* 在[**通知授权**](https://global.alipay.com/docs/ac/ams/notifyauth)API的请求参数中的_field_`authorizationNotifyType`中添加了枚举值`TOKEN_CREATED`。
...
*   将[咨询](https://global.alipay.com/docs/ac/ams/authconsult)和[申请令牌](https://global.alipay.com/docs/ac/ams/accesstokenapp)API请求参数中的参数 _customerBelongsTo_ 的长度从16改为64。
*   将[支付（自动扣款）](https://global.alipay.com/docs/ac/ams/payment_agreement)API请求参数中的参数 _paymentMethod.paymentMethodType_ 的长度从32改为64。  
**2023年4月**  
----------  
### 改进  
...
### 文档改进  
*   在《使用非卡支付方式支付》的《收集或显示额外信息》部分，新增了对Blik的描述，链接为：[https://global.alipay.com/docs/ac/cashierpay/noncard_payment?version=v1.40&pageVersion=3](https://global.alipay.com/docs/ac/cashierpay/noncard_payment?version=v1.40&pageVersion=3)。
...
*   在[pay (收银台支付)](https://global.alipay.com/docs/ac/ams/payment_cashier) API的请求参数中，向`paymentMethod.paymentMethodMetaData`参数添加了针对Payeasy和Konbini场景的`payerEmail`参数。
...
*   在[consult (收银台支付)](https://global.alipay.com/docs/ac/ams/consult_cashier) API的响应参数中，向`paymentOptions.paymentMethodCategory`字段添加了枚举值`OTC`。
### 文档改进  
...
请注意，这两个文档分别介绍了在收银台支付和自动扣款场景下如何使用新的 `REFUND_REVERSAL` 枚举值进行结算汇总。
### 文档改进  
...
* 添加了参数 _issuingCountry_, _funding_, 和 _cardBrand_。详情请参考以下文档：
  * [收银台交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems) - 收银台支付
  * [收银台结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付
* 添加了参数 _interchangeFeeAmountValue, interchangeFeeCurrency, schemeFeeAmountValue, schemeFeeCurrency, AcquirerMarkupFeeAmountValue_ 和 _AcquirerMarkupFeeCurrency_。详情请参考以下文档：
  * [收银台结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付
...
### 文档改进  
*   更新了[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)、[notifyPayment](https://global.alipay.com/docs/ac/ams/paymentrn_online)和[inquiryPayment](https://global.alipay.com/docs/ac/ams/paymentri_online) API 中的错误代码RISK_REJECT的描述。
...
*   在[咨询（收银员支付）](https://global.alipay.com/docs/ac/ams/consult_cashier)API的响应参数中，向字段`paymentOptions.paymentMethodType`添加了枚举值。
...
*   在[支付（收银员支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中，向字段`paymentMethod.paymentMethodType`添加了枚举值。
*   在[支付（收银员支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中，将字段`card`作为子参数添加到`paymentMethodMetaData`字段内。
...
*   修改了文件路径命名规则和字段 _seq_ 的描述。更多详情，请参阅以下文档：  
    *   [收银台支付的交易条目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
    *   [收银台支付的结算条目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
    *   [收银台支付的结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary)
    *   [自动扣款的交易条目](https://global.alipay.com/docs/ac/autodebitpay/transactionitems)
    *   [自动扣款的结算条目](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
...
*   新增了字段 _taxFeeAmountValue_ 和 _taxFeeCurrency_。详情请参考以下文档：
    *   [收银台支付结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems) 
    *   [收银台支付结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary) 
    *   [自动扣款结算项](https://global.alipay.com/docs/ac/autodebitpay/settlementitems) 
    *   [自动扣款结算汇总](https://global.alipay.com/docs/ac/autodebitpay/settlementsummary) 
...
| **废弃术语** | **当前术语** | **备注** |
| --- | --- | --- |
| PSP | PMP | 废弃术语可能仍会显示在代码中，如API规范、Java代码规范或其他技术元素存在的地方。废弃术语的含义未变。 |
...
*   Alipay Docs的新版本在5月发布。原始门户（[https://global.alipay.com/open/doc.htm](https://global.alipay.com/open/doc.htm)）上的技术文档已合并到此新站点，以提供更好的用户体验。您可以前往**文档** > **旧版文档** ([https://global.alipay.com/docs/ac/legacy/legacydoc](https://global.alipay.com/docs/ac/legacy/legacydoc)) 查看详细信息。
...
**2024年版权信息**  
Alipay [法律信息](https://global.alipay.com/docs/ac/platform/membership)
...
- [2022年5月](#Ol3oh "2022年5月")
- [2022年4月](#RA3mX "2022年4月")
- [2022年3月](#dHoc6 "2022年3月")
- [2022年2月](#jRJhq "2022年2月")
- [2021年12月](#BI8S8 "2021年12月")
- [2021年11月](#pSfYF "2021年11月")
- [2021年10月](#cH6R0 "2021年10月")
- [2021年9月](#TBZWK "2021年9月")
