...
### 已废弃  
移除了由GAGW使用的域名：[https://open-global.alipay.com](https://open-global.alipay.com)。  
2023年10月  
--------------
### 增强功能  
...
*   向请求头中添加了可选字段 [_agent-token_](https://global.alipay.com/docs/ac/ams/api_fund#sjPLz)，以授权 ISV 调用 API。
### 已废弃  
...
### 已弃用  
*   在[收银台支付](https://global.alipay.com/docs/ac/ams/payment_cashier)、[自动扣款支付](https://global.alipay.com/docs/ac/ams/payment_agreement)、[用户出示模式支付](https://global.alipay.com/docs/ac/ams/upm)、[订单码支付](https://global.alipay.com/docs/ac/ams/oc)和[入口码支付](https://global.alipay.com/docs/ac/ams/ec)的请求参数中弃用了字段`customerId`。  
2023年6月  
--------
### 增强功能  
...
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中，向`paymentMethod.paymentMethodMetaData`参数添加了`payerEmail`，适用于Payeasy和Konbini场景。
...
*   在[_使用非银行卡支付方式_](https://global.alipay.com/docs/ac/cashierpay/noncard_payment?version=v1.40&pageVersion=3)的“获取异步通知”部分添加了Mercado Pago默认订单过期时间的注释。
*   在[_使用非银行卡支付方式_](https://global.alipay.com/docs/ac/cashierpay/noncard_payment?version=v1.40&pageVersion=3)的“收集或显示额外信息”部分添加了对Mercado Pago的描述。
* 在“使用非卡支付方式”中的“发起支付流程”部分，添加了关于_codeValue_的描述，参见[使用非卡支付方式](https://global.alipay.com/docs/ac/cashierpay/noncard_payment?version=v1.40&pageVersion=3)。
...
* 向_field_ _paymentMethodType_ 添加枚举值`KONBINI`, `FPX`, 和 `PAYEASY`。详情请参考以下文档：
...
* 向_field_ _paymentMethodType_ 添加枚举值`PAYPAY` 和 `GrabPay`。详情请参考以下文档：
...
请注意，这两个文档分别介绍了在收银台支付和自动扣款场景下，如何使用 `REFUND_REVERSAL` 枚举值来处理退款逆转的结算情况。
### 文档改进  
...
* 在参数 _paymentMethodType_ 中添加了枚举值 `AKULAKU_PAYLATER_PH` 和 `GRABPAY_MY`。详情可参考以下文档：
...
* 新增了参数 _issuingCountry_，_funding_ 和 _cardBrand_。详情可参考以下文档：
  * [收银台交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems) - 收银台支付
  * [收银台结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付
* 新增了参数 _interchangeFeeAmountValue_，_interchangeFeeCurrency_，_schemeFeeAmountValue_，_schemeFeeCurrency_，_AcquirerMarkupFeeAmountValue_ 和 _AcquirerMarkupFeeCurrency_。详情可参考以下文档：
  * [收银台结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付
  * [收银台结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary) - 收银台支付
* 在参数 _summaryType_ 中添加了枚举值 `AUTHORIZATION`，`VOID`，`CAPTURE` 和 `DISPUTE`。详情可参考以下文档：
  * [收银台结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary) - 收银台支付
### 文档改进  
...
*   **自动扣款结算项**：[结算项](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
...
*   在\_paymentMethodType\_\_字段中添加了枚举值`PAYPAY`。更多详情，请参阅以下文档：
    *   **收银台支付交易项**：[交易项](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
    *   **收银台支付结算项**：[结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
    *   **自动扣款交易项**：[交易项](https://global.alipay.com/docs/ac/autodebitpay/transactionitems)
    *   **自动扣款结算项**：[结算项](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
...
*   将字段 _paymentMethodType_ 的枚举值`MIXEDCARD`修改为`CARD`。详细信息见以下文档：  
...
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier)API的响应参数中，向字段`paymentOptions.paymentMethodType`添加了枚举值。
...
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中，向字段`paymentMethod.paymentMethodType`添加了枚举值。
...
*   **2021年9月**
    *   以下术语更名：
        ...
        *   H5 | WAP | 
        *   移动网站 | 
    *   添加了在线支付和店内支付场景的新介绍视频。您可以前往 [在线支付](https://global.alipay.com/docs/onlinepayment) 和 [店内支付](https://global.alipay.com/docs/instorepayment) 查看详情。
*   **2021年8月**
...
*   在线支付和店内支付产品的全新版本API于12月发布。API文档格式已更新为新版本，以提供更好的用户体验。现在，您可以在同一文档中访问子字段。以前，子字段仅在数据字典中可用。您可以访问 **API参考** > **产品API** > **在线支付** ([https://global.alipay.com/docs/ac/ams/payment_cashier](https://global.alipay.com/docs/ac/ams/payment_cashier)) 来查看新版本。
