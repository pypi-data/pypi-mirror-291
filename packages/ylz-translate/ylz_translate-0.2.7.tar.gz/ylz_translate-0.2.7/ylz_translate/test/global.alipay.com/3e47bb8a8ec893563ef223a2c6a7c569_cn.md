...
* 更新了[**pay（结账支付）**](https://global.alipay.com/docs/ac/ams/payment_cashier)API中_fields.order.buyer_和_fields.order.buyer.buyerPhoneNo_的描述。
...
*   将[交易明细报告](https://global.alipay.com/docs/ac/reconcile/transaction_details)和[结算明细报告](https://global.alipay.com/docs/ac/reconcile/settlement_details)文档中的_funding_字段的最大长度从6个字符增加到20个字符，这是在对账文档中的更新。
*   在[结算汇总报告](https://global.alipay.com/docs/ac/reconcile/settlement_details)文档的_summaryType_字段中，添加了枚举值`COLLATERAL_WITHHOLDING`、`RESERVE_WITHHOLDING`、`RESERVE_RELEASE`。
*   在[结算明细报告](https://global.alipay.com/docs/ac/reconcile/settlement_details)文档的_transactionType_字段中，添加了枚举值`COLLATERAL_WITHHOLDING`、`RESERVE_WITHHOLDING`、`RESERVE_RELEASE`。
...
*   为欧洲地区的商家服务器添加了加速域名 [https://open-de-global.alipay.com](https://open-de-global.alipay.com)。
...
*   将`Request-Time`的格式修改为毫秒级精确的时间戳。详细信息请参阅[构造签名内容](https://global.alipay.com/docs/ac/ams/digital_signature#构建签名内容)。  
2023年5月
----------
### 改进  
...
* 向_field_ _paymentMethodType_ 添加了枚举值`KONBINI`，`FPX`和`PAYEASY`。详情请参考以下文档：
...
* 向_field_ _paymentMethodType_ 添加了枚举值`PAYPAY`和`GrabPay`。详情请参考以下文档：
...
请注意，这些文档链接指向的是蚂蚁金服的全球版英文文档，实际内容需要根据蚂蚁金服的中文版文档库进行对应查阅。在中文文档中，您将找到关于如何处理退款逆转以及在收银台支付和自动扣款场景下如何进行结算汇总的详细信息。
### 文档改进  
...
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier)API的响应参数中，向字段`paymentOptions.paymentMethodType`添加枚举值`PROMPTPAY`。
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中，向字段`paymentMethod.paymentMethodType`添加枚举值`PROMPTPAY`。
*   在[咨询](https://global.alipay.com/docs/ac/ams/authconsult)和[申请令牌](https://global.alipay.com/docs/ac/ams/accesstokenapp)API的请求参数中，向字段`customerBelongsTo`添加枚举值`EASYPAISA`。
*   在[支付（自动扣款）](https://global.alipay.com/docs/ac/ams/payment_agreement)API的请求参数中，向字段`paymentMethodType`添加枚举值`EASYPAISA`。
...
*   向字段`paymentMethodType`添加枚举值`PROMPTPAY`。更多详情，请参阅以下文档：  
...
### 已废弃  
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中，废弃了字段`cookieId`。
### 文档改进  
...
* 为[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier) API的请求参数中的`creditPayPlan.installmentNum`字段增加了值的范围。
...
* 添加了字段`disputeHandlingFee`，`disputeHandlingFeeCurrency`，`disputeReverseFee`和`disputeReverseFeeCurrency`。更多信息，请参阅[结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付。
...
*   向收银台支付添加了Akulaku PayLater支付方式，向收银台支付和自动扣款添加了Boost eWallet。更多信息请参阅以下文档：
...
| **废弃术语** | **当前术语** | **备注** |
| --- | --- | --- |
| PSP | PMP | 废弃术语可能仍会在代码中显示，例如API规范、Java代码规范或其他技术元素存在的地方。废弃术语的含义没有改变。 |
