...
*   在[_使用非银行卡支付方式_](https://global.alipay.com/docs/ac/cashierpay/noncard_payment?version=v1.40&pageVersion=3)_的获取异步通知_部分添加了Mercado Pago的默认订单过期时间的注解。
...
1. **支付（收银台支付）API** - [pay (Cashier Payment)](https://global.alipay.com/docs/ac/ams/payment_cashier)、**通知支付API** - [notifyPayment](https://global.alipay.com/docs/ac/ams/paymentrn_online) 和 **查询支付API** - [inquiryPayment](https://global.alipay.com/docs/ac/ams/paymentri_online) 添加了错误代码 `FRAUD_REJECT`。
2. **通知支付API** 和 **查询支付API** 添加了错误代码 `SUSPECTED_RISK`。
3. **咨询（收银台支付）API** - [consult (Cashier Payment)](https://global.alipay.com/docs/ac/ams/consult_cashier) 的响应参数中，字段 `_paymentOptions.paymentMethodType` 添加枚举值 `PROMPTPAY`。
4. **支付（收银台支付）API** 的请求参数中，字段 `_paymentMethod.paymentMethodType` 添加枚举值 `PROMPTPAY`。
5. **咨询API** - [consult](https://global.alipay.com/docs/ac/ams/authconsult) 和 **申请令牌API** - [applyToken](https://global.alipay.com/docs/ac/ams/accesstokenapp) 的请求参数中，字段 `_customerBelongsTo` 添加枚举值 `EASYPAISA`。
6. **支付（自动扣款）API** - [pay (Auto Debit)](https://global.alipay.com/docs/ac/ams/payment_agreement) 的请求参数中，字段 `_paymentMethodType` 添加枚举值 `EASYPAISA`。
7. **咨询API** 和 **申请令牌API** 添加错误代码 `NO_PAY_OPTIONS`。
8. 字段 `_paymentMethodType` 添加枚举值 `PROMPTPAY`。更多详情，请参考以下文档：  
   **收银台支付交易项目** - [Transaction Items](https://global.alipay.com/docs/ac/cashierpay/transactionitems) for Cashier Payment
...
### 增强功能  
*   对API的数据字典进行了弃用。
*   弃用了在线支付注册相关的API：**registration**、**notifyRegistrationStatus**、**inquiryRegistrationStatus**和**inquiryRegistrationInfo**。
### 文档改进  
