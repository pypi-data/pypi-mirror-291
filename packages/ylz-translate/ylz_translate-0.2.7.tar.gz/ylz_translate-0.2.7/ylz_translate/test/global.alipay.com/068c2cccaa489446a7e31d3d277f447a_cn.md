...
订阅更新
订阅产品更新，以接收关于文档更新的电子邮件通知。您可以通过右侧的“取消订阅”按钮或每封邮件通知中提供的链接取消订阅。  
...
* 在“使用非卡支付方式”中的“发起支付流程”部分，添加了对`codeValue`的描述，参考：[使用非卡支付方式支付](https://global.alipay.com/docs/ac/cashierpay/noncard_payment?version=v1.40&pageVersion=3)。
* 更新了[notifyPayment](https://global.alipay.com/docs/ac/ams/paymentrn_online)通知的描述，见：“[通知](https://global.alipay.com/docs/ac/cashierpay/notifications)”部分。
...
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中，向`paymentMethod.paymentMethodMetaData`参数添加了用于Mercado Pago场景的`cpf`和`payerEmail`参数。
...
* 向字段`paymentMethodType`添加了枚举值`DOLFIN`, `GRABPAY_SG`, `LINKAJA`, `OVO`, `GOPAY_ID`, `OCTOCLICKS`, `ONLINEBANKING_BNI`, `ONLINEBANKING_MANDIRI`, `ONLINEBANKING_BRI`, `ONLINEBANKING_BCA`, `BANKTRANSFER_MAYBANK`, `BANKTRANSFER_BNI`, `BANKTRANSFER_PERMATA`, `CIMBNIAGA`, `BANKTRANSFER_MANDIRI`, `BANKTRANSFER_BSI`, `ATMTRANSFER_ID`, 和 `PAYNOW`。更多信息，参见以下文档：
    * [收银台支付的交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
    * [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
* 向字段`paymentMethodType`添加了枚举值`MAYA`。更多信息，参见以下文档：
    * [自动扣款的交易项目](https://global.alipay.com/docs/ac/autodebitpay/transactionitems)
**结算项目** - 自动扣款  
**2022年11月**  
### 更新内容  
...
*   修改了文件路径命名规则和字段 _seq_ 的描述。更多详情，请参阅以下文档：  
    ...
    *   [收银台支付的结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
    *   [收银台支付的结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary)
    *   [自动扣款的交易项](https://global.alipay.com/docs/ac/autodebitpay/transactionitems)
    *   [自动扣款的结算项](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
...
### 增强功能  
*   已弃用API的数据字典。
*   已弃用在线支付注册相关的API：**registration**、**notifyRegistrationStatus**、**inquiryRegistrationStatus**和**inquiryRegistrationInfo**。
### 文档改进  
...
*   从以下API中删除了 _orderCodeForm.codeDetails.codeValueType_ 字段：
    ...
    *   [自动扣款支付](https://global.alipay.com/docs/ac/ams/payment_agreement)
*   2021年5月
    *   支付宝文档全新设计，提升用户体验，采用新的信息架构。新增帮助新用户文档：
...
请注意，这个文档结构列出了一系列月份，每个月份下有“增强”、“废弃”和“文档改进”等标题，但没有提供具体的内容。要提供完整的翻译，需要有每个标题下的具体内容。
**2021年9月**
