...
*   如果*result.resultStatus*的值为`S`，退款成功。
*   如果*result.resultStatus*的值为`F`，退款失败。通常，失败原因是退款窗口已过期（*result.resultCode*=`REFUND_WINDOW_EXCEED`）。如果仍需处理退款，需要人工干预。
*   如果*result.resultStatus*的值为`U`，结果未知。可能由于系统或网络问题导致处理失败。使用相同的*refundRequestId*重试，或使用**inquiryRefund** API查询退款结果。
...
| MERCHANT\_BALANCE\_NOT\_ENOUGH | F | 商户余额不足。 | 等待商户余额充足后重新调用接口。重试时需要更改*refundRequestId*字段。 |
...
| REPEAT\_REQ\_INCONSISTENT | F | 金额或币种与之前的请求不一致。 | 确保所有请求字段相同，或使用新的*paymentRequestId*重新发起退款。 |
