...
* 支持了巴西本地的支付方式AlipayBR，以及巴西即时支付系统PIX。
### 已废弃  
...
*   更新了巴西、秘鲁、墨西哥和智利支持的支付卡品牌列表在《支付方式》中。
...
请注意，这些文档链接指向的是蚂蚁金服的全球版英文文档，实际内容需要根据蚂蚁金服的中文版文档库进行翻译或对照理解。如果需要准确的中文翻译，建议直接查阅蚂蚁金服的官方中文文档。
### 文档改进  
...
*   在字段`paymentMethodType`中添加枚举值`EASYPAISA`。更多详情，请参阅以下文档：
...
* 添加了字段 _installmentsNum_。更多详情，请参考以下文档：
...
* 添加了字段 _disputeHandlingFee_、_disputeHandlingFeeCurrency_、_disputeReverseFee_ 和 _disputeReverseFeeCurrency_。更多详情，请参考以下文档：
  * [结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付
...
*   在收银台支付中添加了Akulaku PayLater支付方式，以及在收银台支付和自动扣款中添加了Boost eWallet。更多详情，请参阅以下文档：
...
*   更新了以下接口的错误代码：**咨询**、**申请令牌**、**撤销**、**支付（收银台支付）**、**支付（自动扣款）**、**支付通知**、**查询支付**、**取消**、**退款**、**查询退款**、**退款通知**、**申报**、**查询申报请求**、**用户出示模式支付**、**订单码支付**、**入口码支付**。
...
*   删除了以下 API 中的 _orderCodeForm.codeDetails.codeValueType_ 字段：
    *   [收银台支付](https://global.alipay.com/docs/ac/ams/payment_cashier)
    *   [自动扣款支付](https://global.alipay.com/docs/ac/ams/payment_agreement)
*   2021年5月
