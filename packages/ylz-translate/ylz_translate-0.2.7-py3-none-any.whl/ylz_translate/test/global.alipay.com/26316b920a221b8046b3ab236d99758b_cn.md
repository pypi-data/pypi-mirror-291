...
*   更新了[非卡支付](https://global.alipay.com/docs/ac/cashierpay/noncard_payment)文档中的LINE Pay，Pay-easy和Konbini集成解决方案。
...
*   更新了[**查询支付**](https://global.alipay.com/docs/ac/ams/paymentri_online)和[**支付通知**](https://global.alipay.com/docs/ac/ams/paymentrn_online) API 中的 _paymentResultInfo_ 字段描述。
...
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的`paymentAmount.value`请求参数、[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier)API的`paymentAmount.value`请求参数、[扣款（收银台支付）](https://global.alipay.com/docs/ac/ams/capture)API的`captureAmount.value`请求参数、[支付（自动扣款）](https://global.alipay.com/docs/ac/ams/payment_agreement)API的`paymentAmount.value`请求参数、[退款](https://global.alipay.com/docs/ac/ams/refund_online)API的`refundAmount.value`请求参数以及[申报](https://global.alipay.com/docs/ac/ams/declare)API的`declarationAmount.value`请求参数中，添加了关于IDR金额四舍五入规则的注释。
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier)API的响应参数中，向字段`paymentOptions.paymentMethodType`添加了枚举值`DOLFIN`、`GRABPAY_SG`、`LINKAJA`、`OVO`、`GOPAY_ID`、`OCTOCLICKS`、`ONLINEBANKING_BNI`、`ONLINEBANKING_MANDIRI`、`ONLINEBANKING_BRI`、`ONLINEBANKING_BCA`、`BANKTRANSFER_MAYBANK`、`BANKTRANSFER_BNI`、`BANKTRANSFER_PERMATA`、`CIMBNIAGA`、`BANKTRANSFER_MANDIRI`、`BANKTRANSFER_BSI`、`ATMTRANSFER_ID`和`PAYNOW`。
...
1. 在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)、[通知支付](https://global.alipay.com/docs/ac/ams/paymentrn_online)和[查询支付](https://global.alipay.com/docs/ac/ams/paymentri_online)API中，新增了错误代码`FRAUD_REJECT`。
2. 在[通知支付](https://global.alipay.com/docs/ac/ams/paymentrn_online)和[查询支付](https://global.alipay.com/docs/ac/ams/paymentri_online)API中，新增了错误代码`SUSPECTED_RISK`。
...
7. 在[咨询](https://global.alipay.com/docs/ac/ams/authconsult)和[申请令牌](https://global.alipay.com/docs/ac/ams/accesstokenapp)API中，新增了错误代码`NO_PAY_OPTIONS`。
8. 字段`paymentMethodType`添加了枚举值`PROMPTPAY`。更多详情，请参阅以下文档：
   - [收银台支付交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
...
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier) API 的请求参数中，作为子参数向 _paymentMethodMetaData_ 添加了 _blikCode_ 和 _payerEmail_ 字段。
...
* 为 [pay (收银台支付)](https://global.alipay.com/docs/ac/ams/payment_cashier) API 请求参数中的 _creditPayPlan.installmentNum_ 添加了值的范围。
...
*   **新增支付方式枚举值 `PAYPAY`**：在 _paymentMethodType_ 字段中。详情请参考以下文档：
    ...
    *   **自动扣款结算项目**：[Settlement Items for Auto Debit](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
*   **更新了字段 _settlementAmountValue_，_feeAmountValue_ 和 _taxFeeAmountValue_ 的描述**：详情请参考以下文档：
    *   **收银台支付结算项目**：[Settlement Items for Cashier Payment](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
    *   **收银台支付结算汇总**：[Settlement Summary for Cashier Payment](https://global.alipay.com/docs/ac/cashierpay/settlementsummary)
    *   **自动扣款结算项目**：[Settlement Items for Auto Debit](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
    *   **自动扣款结算汇总**：[Settlement Summary for Auto Debit](https://global.alipay.com/docs/ac/autodebitpay/settlementsummary)
*   **新增适用于 Hundsun 收单方的示例 4**：详情请参考以下文档：
    *   **收银台支付结算项目**：[Settlement Items for Cashier Payment](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
    *   **收银台支付结算汇总**：[Settlement Summary for Cashier Payment](https://global.alipay.com/docs/ac/cashierpay/settlementsummary)
    *   **自动扣款结算项目**：[Settlement Items for Auto Debit](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
    *   **自动扣款结算汇总**：[Settlement Summary for Auto Debit](https://global.alipay.com/docs/ac/autodebitpay/settlementsummary)
...
*   更新了收银台支付下的交易项、结算项和结算汇总文档：
    *   在交易项和结算项的paymentMethodType字段中添加了以下新的枚举值：
        *   IDEAL
        *   GIROPAY
        *   SOFORT
        *   PAYU
...
*   2021年9月
    *   以下术语更名：
        ...
        *   H5 | WAP | 
        *   移动网站 | 
    *   添加了在线支付和店内支付场景的新介绍视频。可以前往[在线支付](https://global.alipay.com/docs/onlinepayment)和[店内支付](https://global.alipay.com/docs/instorepayment)查看详细信息。
*   2021年8月
...
*   2021年5月
    ...
    *   将以下文档移至相应产品文档：
        ...
        *   结算汇总
        *   交易项目
    *   如需查看详细信息，例如，可以前往“收银台支付”>“报表和对账”>“结算项目”([https://global.alipay.com/docs/ac/cashierpayment/settlementitems\_online](https://global.alipay.com/docs/ac/cashierpayment/settlementitems_online))。
术语更名。以下术语已被重命名：
