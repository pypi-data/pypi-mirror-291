...
*   在 [支付方式](https://global.alipay.com/docs/ac/ref/payment_method) 文档中为结账支付添加了 DOKU，KPLUS，Pay-easy 网上银行和 Pay-easy 银行转账的支付方式。
*   在 [支付方式](https://global.alipay.com/docs/ac/ref/payment_method) 文档中为自动扣款和订阅支付添加了 KPLUS 的支付方式。
...
--------
### 增强功能  
*   为收银台支付和自动扣款添加了枚举值 `NAVERPAY`，以及为收银台支付添加了 `TOSS`，详情见[_支付方式_](https://global.alipay.com/docs/ac/ref/payment_method)。
*   添加了[商家服务文档](https://global.alipay.com/docs/ac/merchant_service)，提供了商家操作教程。
...
*   将`Request-Time`的格式修改为毫秒级精度的时间戳。详细信息请参阅[构建待签名内容](https://global.alipay.com/docs/ac/ams/digital_signature#build_signature)。  
2023年5月
--------
### 增强功能  
...
*   更新了[Blik]支付方式在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API请求参数中，参数`order.env.clientIp`和`order.env.userAgent`的描述。
...
*   在[_使用非卡支付方式_](https://global.alipay.com/docs/ac/cashierpay/noncard_payment?version=v1.40&pageVersion=3)的“获取异步通知”部分添加了Mercado Pago默认订单过期时间的注释。
*   在[_使用非卡支付方式_](https://global.alipay.com/docs/ac/cashierpay/noncard_payment?version=v1.40&pageVersion=3)的“收集或显示额外信息”部分添加了对Mercado Pago的描述。
...
*   在_field _paymentMethodType_中添加了枚举值`BILLEASE`, `QRIS`, `MERCADOPAGO_BR`, `MERCADOPAGO_MX`, `MERCADOPAGO_CL`, 和 `MERCADOPAGO_PE`。详情请参阅以下文档：  
...
*   在参数_field _transactionType_中添加了枚举值`REFUND_REVERSAL`。详情请参阅以下文档：  
    *   [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
...
请注意，这些文档链接指向的是蚂蚁金服的全球版英文文档，实际内容需要根据蚂蚁金服的中文文档平台进行查找或翻译。在中文环境中，相关文档可能有对应的中文名称和链接。
### 文档改进  
...
* 新增参数 _issuingCountry_，_funding_ 和 _cardBrand_。更多详情可参考以下文档：
  * [收银台交易项](https://global.alipay.com/docs/ac/cashierpay/transactionitems) - 收银台支付
  * [收银台结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付
* 新增参数 _interchangeFeeAmountValue_，_interchangeFeeCurrency_，_schemeFeeAmountValue_，_schemeFeeCurrency_，_AcquirerMarkupFeeAmountValue_ 和 _AcquirerMarkupFeeCurrency_。更多详情可参考以下文档：
  * [收银台结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付
...
* 在参数 _summaryType_ 中添加了枚举值 `AUTHORIZATION`，`VOID`，`CAPTURE` 和 `DISPUTE`。更多详情可参考以下文档：
  * [收银台结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary) - 收银台支付
### 文档改进  
*   修改了[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier) API响应参数中的参数 _PaymentOptions.installment_ 的数据类型为对象。
*   更新了[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier) API响应参数中的参数 _PaymentOptions.paymentMethodRegion_ 的描述。
*   更新了参数 _feeAmountValue_ 的描述。更多详情，请参阅以下文档：  
    *   [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
...
* 向字段 _paymentMethodType_ 添加了枚举值 `DOLFIN`, `GRABPAY_SG`, `LINKAJA`, `OVO`, `GOPAY_ID`, `OCTOCLICKS`, `ONLINEBANKING_BNI`, `ONLINEBANKING_MANDIRI`, `ONLINEBANKING_BRI`, `ONLINEBANKING_BCA`, `BANKTRANSFER_MAYBANK`, `BANKTRANSFER_BNI`, `BANKTRANSFER_PERMATA`, `CIMBNIAGA`, `BANKTRANSFER_MANDIRI`, `BANKTRANSFER_BSI`, `ATMTRANSFER_ID`, 和 `PAYNOW`。更多详情可参考以下文档：
    * [收银台支付的交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
    * [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
* 向字段 _paymentMethodType_ 添加了枚举值 `MAYA`。更多详情可参考以下文档：
...
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier)API的响应参数中，字段`paymentOptions.paymentMethodType`添加枚举值`PROMPTPAY`。
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中，字段`paymentMethod.paymentMethodType`添加枚举值`PROMPTPAY`。
*   在[咨询](https://global.alipay.com/docs/ac/ams/authconsult)和[申请令牌](https://global.alipay.com/docs/ac/ams/accesstokenapp)API的请求参数中，字段`customerBelongsTo`添加枚举值`EASYPAISA`。
*   在[支付（自动扣款）](https://global.alipay.com/docs/ac/ams/payment_agreement)API的请求参数中，字段`paymentMethodType`添加枚举值`EASYPAISA`。
...
* 添加了`paymentMethod.paymentMethodMetaData.cpf`字段的描述，该字段位于[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中。
...
*   在 **结算与对账** 文档中添加了 _processingFeeAmountValue_ 和 _processingFeeCurrency_ 字段。详情请参阅以下文档：  
    *   [收银台支付 - 结算与对账](https://global.alipay.com/docs/ac/cashierpay/reconcile_settle)
...
*   在收银台支付中添加了Akulaku PayLater支付方式，同时在收银台支付和自动扣款中添加了Boost eWallet。更多信息，请参阅以下文档：
...
请注意，文档中提到的接口和字段是蚂蚁金服支付服务的一部分，这些更改可能影响到使用这些服务的开发者。
*   redirectActionForm
*   orderCodeForm
*   将以下字段从可选改为必填：
*   请求参数：
...
*   从以下API中删除了 _orderCodeForm.codeDetails.codeValueType_ 字段：
    ...
    *   [支付（自动扣款）](https://global.alipay.com/docs/ac/ams/payment_agreement)
*   2021年5月
    *   改版Alipay Docs，提升用户体验并采用新的信息架构。新增帮助新用户文档：
