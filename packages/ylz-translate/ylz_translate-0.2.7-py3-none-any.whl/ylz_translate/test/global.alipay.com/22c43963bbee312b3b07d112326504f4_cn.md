...
* 更新了结账支付中银行相关支付[Web/WEP](https://global.alipay.com/docs/ac/cashierpay/bank_webwap)、[Android](https://global.alipay.com/docs/ac/cashierpay/bank_android)和[iOS](https://global.alipay.com/docs/ac/cashierpay/bank_ios)集成指南里组件名称为`AMSCashierPayment`。
...
*   更新了结算与交易文档。
### 已弃用  
*   对于直接借记和定期订阅，已弃用Dolfin、BPI和bKash的支付方式。对于直接借记和定期订阅，也已弃用Easypaisa的支付方式。  
2023年8月
### 增强功能  
...
### 文档改进  
*   对于收银台支付和自动扣款，已更新API请求发送的域名。详情请参阅[调用API](https://global.alipay.com/docs/ac/ams/api#call_api)。
*   将`Request-Time`的格式修改为毫秒级精确的时间戳。详情请参阅[构造待签名内容](https://global.alipay.com/docs/ac/ams/digital_signature#sign_content)。  
2023年5月
----------
### 改进  
...
*   在《使用非银行卡支付方式》的《收集或显示额外信息》部分中，添加了关于Mercado Pago的描述。
...
* 向`paymentMethodType`字段添加了枚举值`KONBINI`、`FPX`和`PAYEASY`。详情见以下文档：
...
* 向`paymentMethodType`字段添加了枚举值`PAYPAY`和`GrabPay`。详情见以下文档：
...
* 更新了“示例5”部分的报告样本。详情见以下文档：
  * [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
  * [自动扣款的结算项目](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
* 更新了参数`originalTransactionId`的描述。详情见以下文档：
...
请注意，这两个文档分别介绍了在收银支付和自动扣款场景下，如何使用 `REFUND_REVERSAL` 枚举值来处理退款逆转的结算情况。
### 文档改进  
...
*   在收银台支付中添加了 Akulaku PayLater 支付方式，在收银台支付和自动扣款中添加了 Boost eWallet。更多信息，请参阅以下文档：
