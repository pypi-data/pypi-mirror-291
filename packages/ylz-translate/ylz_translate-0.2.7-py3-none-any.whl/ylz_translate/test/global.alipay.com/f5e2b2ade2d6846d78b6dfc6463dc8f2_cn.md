...
-----------
### 增强功能  
*   在[**pay（结账支付）**](https://global.alipay.com/docs/ac/ams/payment_cashier) API 的 _order.buyer.buyerEmail_，_order.buyer.buyerEmail.buyerPhoneNo_，_order.buyer.buyerEmail.buyerName.firstName_ 和 _order.buyer.buyerEmail.buyerName.lastName_ 字段中，增加了 KONBINI，BANKTRANSFER\_PAYEASY 和 ONLINEBANKING\_PAYEASY 的使用场景。
...
*   在 [支付方式](https://global.alipay.com/docs/ac/ref/payment_method) 文档中为结账支付添加了 DOKU，KPLUS，Pay-easy 网银支付和 Pay-easy 银行转账支付方式。
...
2023年8月
### 增强功能  
*   添加了三个关于商家服务操作的视频教程：[如何退款](https://global.alipay.com/docs/ac/merchant_service/videos#如何退款)，[如何查看交易详情](https://global.alipay.com/docs/ac/merchant_service/videos#如何查看交易详情)，以及[如何管理发票](https://global.alipay.com/docs/ac/merchant_service/videos#如何管理发票)。
...
*   在《使用非银行卡支付方式》的《获取异步通知》部分增加了Mercado Pago的默认订单过期时间的注释。
...
* 在`paymentMethodType`字段中添加了枚举值`KONBINI`、`FPX`和`PAYEASY`。详情请参阅以下文档：
  * [收银支付的交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
  * [收银支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
...
* 更新了参数`originalTransactionId`的描述。详情请参阅以下文档：
  * [收银支付的结算和对账](https://global.alipay.com/docs/ac/cashierpay/reconcile_settle)
  * [自动扣款的结算和对账](https://global.alipay.com/docs/ac/autodebitpay/reconcile_settle)
*   **收银台支付的交易项目**：[交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
*   **自动扣款的交易项目**：[交易项目](https://global.alipay.com/docs/ac/autodebitpay/transactionitems)
*   **收银台支付的结算项目**：[结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
*   **自动扣款的结算项目**：[结算项目](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
...
请注意，这些文档链接指向的是蚂蚁金服的全球版英文文档，实际的中文文档链接可能不同，您可能需要在蚂蚁金服的中文开发者平台上查找对应的文档。
### 文档改进  
...
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier) API 的响应参数中，向参数 _PaymentOptions.paymentOptionDetail.supportCardBrands.cardBrand_ 添加了一组枚举值，详情参见[支付方式枚举](https://global.alipay.com/docs/ac/ref/payment_method)。
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier) API 的响应参数中，向参数 _PaymentOptions.paymentOptionDetail.supportCardBrands.logo.logoName_ 添加了一组枚举值，详情参见[支付方式枚举](https://global.alipay.com/docs/ac/ref/payment_method)。
...
*   将[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier) API 的响应参数中的 paymentOptions.installments.interestRate 字段改为可选。
...
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier)API的请求参数中添加了_merchantRegion_字段。
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier)API的响应参数中向_paymentOptions.paymentMethodType_字段添加了枚举值。
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier)API的响应参数中添加了_paymentOptions.paymentMethodCategory_字段。
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier)API的响应参数中添加了_paymentOptions.paymentOptionDetail_字段。
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中添加了_paymentFactor_和_paymentMethod.paymentMethodId_字段。
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中向_paymentMethod.paymentMethodType_字段添加了枚举值。
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中，将_card_作为子参数添加到_paymentMethodMetaData_字段中。
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中添加了_merchantRegion_字段。
*   添加了[扣款（收银台支付）](https://global.alipay.com/docs/ac/ams/capture)API。
*   在[通知支付](https://global.alipay.com/docs/ac/ams/paymentrn_online)API的请求参数中添加了_cardInfo_和_acquirerReferenceNo_字段。
...
*   新增了字段 _taxFeeAmountValue_ 和 _taxFeeCurrency_。详情请参阅以下文档：
    *   [收银台支付结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems) 
    *   [收银台支付结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary) 
    *   [自动扣款结算项](https://global.alipay.com/docs/ac/autodebitpay/settlementitems) 
    *   [自动扣款结算汇总](https://global.alipay.com/docs/ac/autodebitpay/settlementsummary) 
...
*   2021年5月
    ...
    *   将以下文档移至相应产品文档：
        ...
        *   结算汇总
        *   交易项目
例如，您可以在收银台支付 > 报表和对账 > 结算项目([https://global.alipay.com/docs/ac/cashierpayment/settlementitems\_online](https://global.alipay.com/docs/ac/cashierpayment/settlementitems_online))中查看详细信息。
术语更名。以下术语已被重命名：
...
| **过时术语** | **当前术语** | **备注** |
| --- | --- | --- |
| PSP | PMP | 过时术语可能仍会显示在代码中，例如API规范、Java代码规范或其他存在技术元素的地方。过时术语的含义没有改变。 |
**2023年12月**
----------------
