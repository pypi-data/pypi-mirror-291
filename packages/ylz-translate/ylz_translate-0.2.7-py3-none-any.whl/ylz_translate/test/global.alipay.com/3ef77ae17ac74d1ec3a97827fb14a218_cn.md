...
*   将`Request-Time`的格式修改为毫秒级精确的的时间戳。详细信息请参阅[构建待签名内容](https://global.alipay.com/docs/ac/ams/digital_signature#构建待签名内容)。  
2023年5月
----------
### 增强功能  
...
*   将《收银台支付》、《自动扣款》、《用户出示模式支付》、《订单码支付》和《入口码支付》API的请求参数中的`clientIp`参数长度从32位改为64位。
...
* 向字段 _paymentMethodType_ 添加了枚举值 `DOLFIN`, `GRABPAY_SG`, `LINKAJA`, `OVO`, `GOPAY_ID`, `OCTOCLICKS`, `ONLINEBANKING_BNI`, `ONLINEBANKING_MANDIRI`, `ONLINEBANKING_BRI`, `ONLINEBANKING_BCA`, `BANKTRANSFER_MAYBANK`, `BANKTRANSFER_BNI`, `BANKTRANSFER_PERMATA`, `CIMBNIAGA`, `BANKTRANSFER_MANDIRI`, `BANKTRANSFER_BSI`, `ATMTRANSFER_ID`, 和 `PAYNOW`。更多详情参见以下文档：
    * [收银台支付的交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
    * [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
* 向字段 _paymentMethodType_ 添加了枚举值 `MAYA`。更多详情参见以下文档：
...
* 添加了`paymentMethod.paymentMethodMetaData.cpf`字段的描述，作为[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API请求参数。
* 为[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API请求参数中的`creditPayPlan.installmentNum`字段添加了值范围。
...
*   在[退款](https://global.alipay.com/docs/ac/ams/refund_online) API 中增加了两个错误代码 `REFUND_NOT_SUPPORTED` 和 `PARTIAL_REFUND_NOT_SUPPORTED`。
...
*   修改以下接口的 _orderCodeForm.codeDetails_ 字段，从必填变为可选：
...
*   删除以下 API 中的 _orderCodeForm.codeDetails.codeValueType_ 字段：
    *   [收银台支付](https://global.alipay.com/docs/ac/ams/payment_cashier)
    *   [自动扣款支付](https://global.alipay.com/docs/ac/ams/payment_agreement)
*   2021年5月
    ...
    *   以下文档标题更名：
        *   API 基础 -> [概览](https://global.alipay.com/docs/ac/ams/api_fund)
...
| **废弃术语** | **当前术语** | **备注** |
| --- | --- | --- |
| PSP | PMP | 废弃术语可能仍会显示在代码中，例如API规范、Java代码规范或其他存在技术元素的地方。废弃术语的含义未变。 |
2023年12月
----------

在线支付和店内支付产品的API新版本在12月发布。API文档格式已更新为新版本，以提供更好的用户体验。现在，您可以在同一文档中访问子字段，而以前子字段只在数据字典中可用。您可以前往**API参考** > **产品API** > **在线支付**（[https://global.alipay.com/docs/ac/ams/payment\_cashier](https://global.alipay.com/docs/ac/ams/payment_cashier)）来查看新版本。

2020年5月
---------

5月份发布了新版Alipay Docs。原有的技术文档（[https://global.alipay.com/open/doc.htm](https://global.alipay.com/open/doc.htm)）已合并到这个新网站，以提供更好的用户体验。您可以前往**文档** > **旧版文档**（[https://global.alipay.com/docs/ac/legacy/legacydoc](https://global.alipay.com/docs/ac/legacy/legacydoc)）查看详细信息。
