版本更新 | 领先中国的第三方在线支付解决方案支付宝
==================================

[![ 图片1：领先中国的第三方在线支付解决方案支付宝](https://ac.alipay.com/storage/2024/3/26/d66c43c0-440d-4c97-9976-f2028a2c8c5e.svg) ![ 图片2：领先中国的第三方在线支付解决方案支付宝](https://ac.alipay.com/storage/2024/3/26/a48bd336-aea0-4f16-bf83-616eacbb4434.svg)](/docs/)
...
*   在[支付方式](https://global.alipay.com/docs/ac/ref/payment_method)文档中，为结账支付添加了 DOKU，KPLUS，Pay-easy 网银支付和 Pay-easy 银行转账支付方式。
...
### 已废弃  
*   从[**pay（结账支付）**](https://global.alipay.com/docs/ac/ams/payment_cashier)API的`paymentMethod.paymentMethodMetaData`字段中移除了便利店（Pay-easy）的场景。
...
* 支持了巴西本地的支付方式AlipayBR，以及巴西快速支付协议PIX。
### 已弃用  
...
* 向`paymentMethodType`字段添加了枚举值`KONBINI`、`FPX`和`PAYEASY`。详情参阅以下文档：
...
* 向`paymentMethodType`字段添加了枚举值`PAYPAY`和`GrabPay`。详情参阅以下文档：
...
请注意，这些文档链接指向的是蚂蚁金服的国际版（Alipay Global）相关接口说明，用于了解如何在收银台支付和自动扣款场景下处理退款逆转的结算情况。
### 文档改进  
...
* 向字段`paymentMethodType`添加了枚举值`DOLFIN`, `GRABPAY_SG`, `LINKAJA`, `OVO`, `GOPAY_ID`, `OCTOCLICKS`, `ONLINEBANKING_BNI`, `ONLINEBANKING_MANDIRI`, `ONLINEBANKING_BRI`, `ONLINEBANKING_BCA`, `BANKTRANSFER_MAYBANK`, `BANKTRANSFER_BNI`, `BANKTRANSFER_PERMATA`, `CIMBNIAGA`, `BANKTRANSFER_MANDIRI`, `BANKTRANSFER_BSI`, `ATMTRANSFER_ID`, 和 `PAYNOW`。详细信息请参考以下文档：
    * [收银台支付的交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
    * [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
* 向字段`paymentMethodType`添加了枚举值`MAYA`。详细信息请参考以下文档：
...
* 添加了字段 _disputeHandlingFee_、_disputeHandlingFeeCurrency_、_disputeReverseFee_ 和 _disputeReverseFeeCurrency_。更多详情，请参阅 [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems) 文档。
...
*   弃用了[通知纠纷](https://global.alipay.com/docs/ac/ams/notify_dispute)API的错误代码。
### 文档改进  
...
*   更新了字段 _settlementBatchId_ 的描述。更多信息请查看以下文档：
    *   [收银台支付结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems) 
    *   [收银台支付结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary) 
    *   [自动扣款结算项](https://global.alipay.com/docs/ac/autodebitpay/settlementitems) 
    *   [自动扣款结算汇总](https://global.alipay.com/docs/ac/autodebitpay/settlementsummary) 

*   添加了字段 _taxFeeAmountValue_ 和 _taxFeeCurrency_。详情请参阅以下文档：
    *   [收银台支付结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems) 
    *   [收银台支付结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary) 
    *   [自动扣款结算项](https://global.alipay.com/docs/ac/autodebitpay/settlementitems) 
    *   [自动扣款结算汇总](https://global.alipay.com/docs/ac/autodebitpay/settlementsummary) 
...
*   从以下API中删除了 _orderCodeForm.codeDetails.codeValueType_ 字段：
    ...
    *   [支付（自动扣款）](https://global.alipay.com/docs/ac/ams/payment_agreement)
*   2021年5月
    *   改版了支付宝文档，提升了用户体验和新的信息架构。新增帮助新用户文档：
...
*   在以下API中添加了一个新字段和枚举（appId和MINI\_APP）：
    *   [支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)
    *   [支付（自动扣款）](https://global.alipay.com/docs/ac/ams/payment_agreement)
    *   [咨询](https://global.alipay.com/docs/ac/ams/authconsult)
