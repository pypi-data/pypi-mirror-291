...
*   添加了卡支付的嵌入式[Web/WEP](https://global.alipay.com/docs/ac/cashierpay/embeded_webwap_en)集成指南。  
**2024年2月**
-------------  
### 增强功能  
...
*   在 [支付方式](https://global.alipay.com/docs/ac/ref/payment_method#vgXw6) 文档中添加了 `DIRECT_DEBIT_SIAMCOMMERCIALBANK` 和 `DIRECT_DEBIT_KRUNGTHAIBANK` 两种支付方式。
...
*   将对账单文档中的[交易明细报告](https://global.alipay.com/docs/ac/reconcile/transaction_details)和[结算明细报告](https://global.alipay.com/docs/ac/reconcile/settlement_details)中的_funding_字段的最大长度从6个字符修改为20个字符。
...
* 添加了订阅支付的[**create**](https://global.alipay.com/docs/ac/ams/create_sub)（创建订阅）、[**notifySubscription**](https://global.alipay.com/docs/ac/ams/notify_sub)（订阅通知）、[**notifyPayment**](https://global.alipay.com/docs/ac/ams/notify_subpayment)（支付通知）、[**change**](https://global.alipay.com/docs/ac/ams/change_sub)（修改订阅）和[**cancel**](https://global.alipay.com/docs/ac/ams/cancel_sub)（取消订阅）API。
...
* 更新了[自动扣款](https://global.alipay.com/docs/ac/autodebit_en/overview)文档。新版本提供了重新结构化的集成和操作指南，以及支付后的操作。
* 将产品名称从收银台支付更新为结账支付。
...
--------
### 增强功能  
*   为收银台支付和自动扣款添加了枚举值 `NAVERPAY`，以及为收银台支付添加了 `TOSS`，这些在[_支付方式_](https://global.alipay.com/docs/ac/ref/payment_method)文档中进行了说明。
...
*   在[支付（收银台支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的响应参数中，弃用了 `_redirectActionForm` 参数。
*   在[咨询](https://global.alipay.com/docs/ac/ams/authconsult)API的响应参数中，弃用了 `_authUrl` 参数。
*   在[查询支付](https://global.alipay.com/docs/ac/ams/paymentri_online)API的响应参数中，弃用了 `_cardInfo.eci` 参数。
*   在[通知支付](https://global.alipay.com/docs/ac/ams/paymentrn_online)API的请求参数中，弃用了 `_cardInfo.eci` 参数。
### 文档改进  
...
* 在_field_ _paymentMethodType_中添加了枚举值`KONBINI`, `FPX`, 和 `PAYEASY`。详细信息请参阅以下文档：
...
* 在_field_ _paymentMethodType_中添加了枚举值`PAYPAY`和`GrabPay`。详细信息请参阅以下文档：
...
* 更新了“示例5”部分的报告样本。详细信息请参阅以下文档：
  * [收银台支付结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
  * [自动扣款支付结算项](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
* 更新了参数_originalTransactionId_的描述。详细信息请参阅以下文档：
...
* 向字段`paymentMethodType`添加了枚举值`DOLFIN`, `GRABPAY_SG`, `LINKAJA`, `OVO`, `GOPAY_ID`, `OCTOCLICKS`, `ONLINEBANKING_BNI`, `ONLINEBANKING_MANDIRI`, `ONLINEBANKING_BRI`, `ONLINEBANKING_BCA`, `BANKTRANSFER_MAYBANK`, `BANKTRANSFER_BNI`, `BANKTRANSFER_PERMATA`, `CIMBNIAGA`, `BANKTRANSFER_MANDIRI`, `BANKTRANSFER_BSI`, `ATMTRANSFER_ID`, 和 `PAYNOW`。详情参见以下文档：
    * [收银台支付的交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
    * [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
* 向字段`paymentMethodType`添加了枚举值`MAYA`。详情参见以下文档：
...
* 为 [pay (收银台支付)](https://global.alipay.com/docs/ac/ams/payment_cashier) API 的请求参数中的 _creditPayPlan.installmentNum_ 添加了值的范围。
...
* 添加了字段 _installmentsNum_。详情请参阅以下文档：
...
* 添加了字段 _disputeHandlingFee_, _disputeHandlingFeeCurrency_, _disputeReverseFee_, 和 _disputeReverseFeeCurrency_。详情请参阅 [结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems) - 收银台支付。
...
*   在[咨询（收银台支付）](https://global.alipay.com/docs/ac/ams/consult_cashier) API 的响应参数中，已弃用 _paymentOptions.paymentOptionDetail_ 字段的 _card_ 子字段。
...
*   修改了 _paymentMethod.paymentMethodMetaData_ 字段的数据类型为对象，并更新了其描述。
...
*   将 _paymentMethodType_ 的枚举值`MIXEDCARD`修改为`CARD`。更多详情参见以下文档：  
    *   [收银台支付的交易项目](https://global.alipay.com/docs/ac/cashierpay/transactionitems)
    *   [收银台支付的结算项目](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
...
*   在[支付（收银员支付）](https://global.alipay.com/docs/ac/ams/payment_cashier)API的请求参数中，将`card`作为子参数添加到`paymentMethodMetaData`字段中。
...
*   修订了字段 _settlementBatchId_ 的描述。更多信息，请参阅以下文档：
    ...
    *   [自动扣款结算项](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
    *   [自动扣款结算汇总](https://global.alipay.com/docs/ac/autodebitpay/settlementsummary)
*   添加了字段 _taxFeeAmountValue_ 和 _taxFeeCurrency_。更多详情，请参阅以下文档：
    *   [收银台支付结算项](https://global.alipay.com/docs/ac/cashierpay/settlementitems)
    *   [收银台支付结算汇总](https://global.alipay.com/docs/ac/cashierpay/settlementsummary)
    *   [自动扣款结算项](https://global.alipay.com/docs/ac/autodebitpay/settlementitems)
    *   [自动扣款结算汇总](https://global.alipay.com/docs/ac/autodebitpay/settlementsummary)
...
*   2021年5月
    ...
    *   将以下文档移至对应产品文档：
        ...
        *   结算汇总
        *   交易项目
    *   如需查看详细信息，可以前往“收银台支付”>“报表与对账”>“结算项目”([https://global.alipay.com/docs/ac/cashierpayment/settlementitems\_online](https://global.alipay.com/docs/ac/cashierpayment/settlementitems_online))。
术语更名。以下术语已被重命名：
...
-   **2024年3月** [查看](#qe1T2 "2024年3月")
-   **增强** [查看](#NbsIu "增强")
-   **2024年2月** [查看](#pOU4K "2024年2月")
-   **增强** [查看](#MOtKI "增强")
-   **2024年1月** [查看](#28qpf "2024年1月")
-   **增强** [查看](#ZmIHJ "增强")
-   **2023年12月** [查看](#tp1qM "2023年12月")
-   **增强** [查看](#zUTuB "增强")
-   **2023年11月** [查看](#mQybi "2023年11月")
-   **增强** [查看](#ip1Wv "增强")
-   **弃用** [查看](#sjXzp "弃用")
-   **2023年10月** [查看](#v6dZi "2023年10月")
-   **增强** [查看](#BYA9u "增强")
-   **弃用** [查看](#fcd9y "弃用")
-   **2023年9月** [查看](#nDrFA "2023年9月")
-   **增强** [查看](#gCLUn "增强")
-   **弃用** [查看](#sOtQI "弃用")
-   **2023年8月** [查看](#bFp89 "2023年8月")
-   **增强** [查看](#Y22UQ "增强")
**2023年7月**
### 增强
### 废弃
...
请注意，这些章节标题表示了不同月份的增强功能、废弃的特性以及文档的改进情况。每个章节下原本应包含具体的更新内容，但由于提供的信息仅为章节标题，所以这里没有列出详细内容。如果需要每个章节的详细翻译内容，请提供每个章节下的具体信息。
**2021年9月**
