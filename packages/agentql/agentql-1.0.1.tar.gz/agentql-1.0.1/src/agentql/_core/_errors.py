...
class BaseAgentQLError(Exception):
    def __init__(self, error, error_code, include_fallback_message=True):
        self.error = error + (SUPPORT_MESSAGE if include_fallback_message else "")
...
class APIKeyError(BaseAgentQLError):
    def __init__(
        ...
        super().__init__(
            textwrap.dedent(message), AGENTQL_1000_API_KEY_ERROR, include_fallback_message=False
        )
