...
def _download_sample_script():
    ...
    try:
        response = requests.get(SAMPLE_SCRIPT_URL, timeout=DEFAULT_REQUEST_TIMEOUT_IN_SECONDS)
        response.raise_for_status()

        with open("example_script.py", "wb") as file:
            file.write(response.content)
    except requests.RequestException:
        typer.echo(
            f"Error downloading the example script. Example script could be found at {SAMPLE_SCRIPT_URL}.",
            err=True,
        )
