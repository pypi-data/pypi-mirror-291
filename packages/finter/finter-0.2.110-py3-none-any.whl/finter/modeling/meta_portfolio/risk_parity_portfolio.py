...
class RiskParityMetaPortfolio(BaseMetaPortfolio):
    ...
    @staticmethod
    def calculate_previous_start_date(alpha_set):
        first_idxes = []
        for alpha in alpha_set:
            first_idx = ModelData.load("alpha." + alpha).index[0]
            first_idxes.append(int(first_idx.strftime("%Y%m%d")))
        return min(first_idxes)
    ...
    def get(self, start, end):
        ...
        pre_start = self.calculate_previous_start_date(self.alpha_set)
        ...
        if cf:
            ...
            if universe == "vn_stock":
                from finter import BaseAlpha

                vnm_etf = BaseAlpha.get_cm(
                    "content.spglobal.compustat.price_volume.vnm-etf-price_close.1d"
                ).get_df(pre_start, end)
                self.close = pd.concat([self.close, vnm_etf], axis=1)
        else:
