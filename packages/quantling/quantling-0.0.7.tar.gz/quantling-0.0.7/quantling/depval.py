...
class DepValAnalyzer():
    ...
    def pvp(self,pos=None,target='dependency'):
        ...
        govs = sorted([(i[0],i[1]/sum(govs.values())) for i in govs.items()],key=lambda x: x[1],reverse = True)
        deps = sorted([(i[0],i[1]/sum(deps.values())) for i in deps.items()],key=lambda x: x[1],reverse = True)
