...
logger = logging.getLogger("uvicorn")  # ("spyctl_api")
...
def filter_and_project(model: dict) -> dict | None:
    ...
    if model.get("kind") == "Node":
        rv = {
            ...
            "allocatable_memory": model["k8s_status"]["allocatable"]["memory"],
...
class OpsReporter(Reporter):
    ...
    def update_model_data(self, data: Iterable[dict], slice_delta: int):
    ...
    def update_dataframe(
        ...
        if orig.empty:
            ...
            if check_dupes and (dupes := rv.index.duplicated()).any():
    ...
    def update_dataframes(self, data: list):
        ...
        self.df = self.update_dataframe(self.df, df, "df")
        self.cluster_stats = self.update_dataframe(
            self.cluster_stats, cluster_stats, "cluster_stats"
        )
    ...
    def renderer(
        ...
        if fmt == "mdx":
            ...
            if are_nodes_missing_agents := (
                ...
                ctx["are_nodes_missing_agents"] = are_nodes_missing_agents
