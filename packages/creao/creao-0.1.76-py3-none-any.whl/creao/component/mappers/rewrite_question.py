...
from creao.core.Endpoints import MiniMaxPro
...
class RewriteQuestion:
    def __init__(self, service:str = "default"):
        ...
        if self.service == "default":
            self.llm = MiniMaxPro(bot_name="question rewriter assistant", bot_content="You are given a question. Your task is to rewrite the question in a conversational manner. Reflective Thinking: Before you generate the final list of questions, pause to examine your assumptions, biases, and the mental models that the question might use. Consider how the question influences the relevance to the passage, and how they prioritize the relevance. Be open to learning from previous similar tasks and improving the outcome. Explicitly share your reflective thought process before giving the final answer.")
    ...
    def run(self, documents:List[Document]):
        ...
        def process_question(question):
            if self.service == "default":
                ...
                try:
                    raw_answer = json.loads(self.llm.invoke(prompt,json_schema))
                except Exception as e:
                    print(f"RewriteQuestion json decode error:{e}")
...
class PersonaToWritingStyle:
    def __init__(self, service:str = "default"):
        ...
        self.service = service
        if self.service == "default":
            self.llm = MiniMaxPro(bot_name="persona to writing style assistant", bot_content="You are given a persona. Your task is to generate the writing style of the persona. Reflective Thinking: Before you generate the final list of writing styles, pause to examine your assumptions, biases, and the mental models that the persona might use. Consider how the persona influences the writing style, and how they prioritize the writing style. Be open to learning from previous similar tasks and improving the outcome. Explicitly share your reflective thought process before giving the final answer.")
        ...
        for persona in personas:
            if self.service == "default":
                ...
                try:
                    ...
                    print(f"PersonaToWritingStyle json decode error:{e}")
...
class RewriteQuestionByPersona:
    def __init__(self, service:str = "default"):
        ...
        self.service = service
        if self.service == "default":
            self.llm = MiniMaxPro(bot_name="question rewriter assistant", bot_content="You are given a question and a persona. Your task is to rewrite the question in the writing style of the persona. Reflective Thinking: Before you generate the final list of questions, pause to examine your assumptions, biases, and the mental models that the question might use. Consider how the question influences the relevance to the passage, and how they prioritize the relevance. Be open to learning from previous similar tasks and improving the outcome. Explicitly share your reflective thought process before giving the final answer")
        ...
        def process_question(question):
            ...
            for style in writing_styles:
                if self.service == "default":
                    ...
                    try:
                        response_raw = self.llm.invoke(prompt,json_schema)
                        raw_answer = json.loads(response_raw)
                    except Exception as e:
                        print(f"RewriteQuestionByPersona json decode error:{e}")
