...
def build(args: Namespace) -> None:
    ...
    if os.system("poetry install"):
        fail("Could not install dependencies.")

    headerprint("Checking pyinstaller")
    if os.system("poetry show pyinstaller"):
        fail(
            "No instance of pyinstaller found inside poetry environment",
            "Tip: Add pyinstaller as a development dependency of your project:\n"
    ...
    for specfile in specfiles:
        if os.system(f"poetry run pyinstaller {specfile}"):
            fail(f"Build of {specfile} failed")
