...
class nnUNetInferenceInputSpec(CommandLineInputSpec):
    in_dir = Directory(
        mandatory=True, argstr="-i %s", position=0, desc="the input folder"
    )
    out_dir = Directory(
        mandatory=True, argstr="-o %s", position=-1, desc="the output folder"
    )
    ...
    all_in_gpu = traits.Str(
        argstr="--all_in_gpu %s",
        desc="all in gpu: 'True', 'False' or 'None'. Default 'None'",
    )
    mode = traits.Str(argstr="--mode %s", desc="mode: default normal")
