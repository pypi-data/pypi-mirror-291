...
class CalculateROIVolumeInputSpec(BaseInterfaceInputSpec):
    ...
    map_derived_roi = File(
        exists=True, mandatory=True, desc="the derived roi mapping file"
    )
    ...
    out_dir_roi_masks = Directory(
        mandatory=False, desc="the output dir for individual rois"
    )
...
class CalculateROIVolume(BaseInterface):
    ...
    def _run_interface(self, runtime: Any) -> Any:
        ...
        for in_img_name in infiles:
            ...
            bnames.append(
                utils.get_basename(in_img_name, self.inputs.in_suff, [img_ext_type])
            )
        ...
        for i, in_bname in enumerate(bnames):
            ...
            out_img_name = os.path.join(
                self.inputs.out_dir, in_bname + self.inputs.out_img_suff + img_ext_type
            )
            out_csv_name = os.path.join(
                self.inputs.out_dir, in_bname + self.inputs.out_csv_suff + out_ext_type
            )

            calcroivol.create_roi_csv(
                scan_id,
                in_img_name,
            ...
            out_img_pref = os.path.join(
                self.inputs.out_dir_roi_masks, in_bname + self.inputs.out_img_suff
            )
            if self.inputs.extract_roi_masks:
                calcroivol.extract_roi_masks(
                    in_img_name, self.inputs.map_derived_roi, out_img_pref
                )
        # And we are done
        return runtime

    def _list_outputs(self) -> Any:
