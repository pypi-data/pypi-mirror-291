...
#ifndef POLYSTAR_GEOMETRY_H_
#define POLYSTAR_GEOMETRY_H_
/*! \file
    \author Greg Tucker
...
//    }
  ...
  plane_points_from_normal(const A<T> & n, const A<T> & p) {
    ...
    auto is_n = a_dot_n.row_is(polystar::cmp::eq, a); // FIXME add tolerances
    if (is_n.any()) {
...
//      if (i != j) {
//        dotij = vecs.dot(i, j);
//        polystar::utils::vector_cross(crsij, vecs.ptr(i), vecs.ptr(j));
//        x = dotij * vecs.view(i);
//        y = vecs.view(j) - x;
//        y_len = y.norm(0) * (std::signbit(polystar::utils::vector_dot(crsij, n.ptr(0))) ? -1 : 1);
//        angij = std::atan2(y_len, dotij);
//        angles[j] = angij < 0 ? angij + 2 * polystar::math::pi : angij;
//      }
//    return angles;
//  }
...
//        angles[j] = ang_ij < 0 ? ang_ij + 2 * polystar::math::pi : ang_ij;
...
//      auto o3dk = orient3d(a.ptr(0), b.ptr(0), c.ptr(0), v.ptr(k));
      ...
      if (polystar::approx_float::scalar(o3dj, 0., tol) && polystar::approx_float::scalar(o3dk, 0., tol)) ok = false;
    }
    A<T> at;
    if (ok) {
