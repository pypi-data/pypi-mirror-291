...
class NaverGameScraper(Scraper[int]):
        ...
        for i, episode in enumerate(content_raw_data, 1):
            ...
            for component in contents["document"]["components"]:
                ...
                if ctype == "image":
                    ...
                    if "src" in component:
                        logger.error(
                            f"A component with ctype: {ctype} has `src` key with value {component['src']!r}. "
                            "It won't be downloaded and need to be checked."
                        )
    ...
    def _extract_webtoon_id(cls, url) -> int | None:
        if url.host != "game.naver.com":
            return None
        matched = re.match(r"/original_series/(?P<webtoon_id>\d+)", str(url.path))
        if not matched:
            return None
        # season_str = url.query.params.get("season")  # season은 없어도 작동함.
