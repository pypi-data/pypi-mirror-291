...
from ._02_naver_webtoon_extra import NaverWebtoonCommentsDownloadOption, NaverWebtoonMetaInfoScraper
...
class AbstractNaverWebtoonScraper(Scraper[int]):
    ...
    def _extract_webtoon_id(cls, url) -> int | None:
        ...
        if url.path != f"/{cls.PATH_NAME}/list":
            return None
        webtoon_id_str = url.query.params.get("titleId")
...
class NaverWebtoonScraper(
    ...
    def from_url(
        ...
        *,
        ...
        furl_ = furl(url)
        try:
            webtoon_id, platform = cls._extract_webtoon_id(furl_)
    ...
    @classmethod
    def _extract_webtoon_id(cls, url: furl) -> tuple[int | None, str | None]:
    ...
    @classmethod
    def _extract_naver_me(cls, url: furl) -> furl | None:
