...
class NaverBlogScraper(Scraper[tuple[str, int]]):
    ...
    @classmethod
    def _extract_webtoon_id(cls, url) -> NaverBlogWebtoonId | None:
        if url.host != "m.blog.naver.com":
            if url.host == "blog.naver.com":
                raise ValueError("PC version of blog is not supported since it does not show category. "
                                 "Use mobile version of blog.")
            return None
        matched = re.match(r"/(?P<blog_id>\w+)", str(url.path))
        if not matched:
            return None
        blog_id = matched["blog_id"]
        category_no_str = url.query.params.get("categoryNo")
        if not category_no_str:
            return None
        return NaverBlogWebtoonId(blog_id, int(category_no_str))
