...
from .common import to_module_name, to_type_name, to_package_module_name


class SimosPackageGenerator(TemplateBasedGenerator):
    """Basic generator, one template, one output file"""
    ...
    def __generate_package_file(self, package: Package, template: Template, pkg_dir: Path):
        name = to_package_module_name(package)
        module_model = {}
        module_model["name"] = name
        module_model["types"] = [{
            "name": to_type_name(blueprint),
            "module": to_module_name(blueprint.get_path())
        } for blueprint in package.blueprints]

        pkg_dir.mkdir(parents=True, exist_ok=True)
        filename = pkg_dir / (name + ".f90")
