...
class Client(Singleton):
    def __init__(self, root_node_id, host, redis_client, cache_path='../cache', workers=8, put_topic='fs_put', get_topic='fs_get', health_topic='fs_health') -> None:
        ...
        if os.path.isfile('hmfs.conf'):
            try:
                ...
                self.direct_minio_client = MinioAgentAsync(conf['FS_HOST'], conf['HOST_WEIGHTS'], check_awailable_ts=20, timeout=10)
            except:
                pass
    ...
    async def async_publish(self, topic, message, tries=0):
        ...
        async with httpx.AsyncClient(timeout=20) as client:
            try:
                if self.host.startswith('http'):
                    api_path = '{}/api/trade/platform/common_api/mqtt_pub'.format(self.host)
                else:
                    api_path = 'https://{}/api/trade/platform/common_api/mqtt_pub'.format(self.host)
