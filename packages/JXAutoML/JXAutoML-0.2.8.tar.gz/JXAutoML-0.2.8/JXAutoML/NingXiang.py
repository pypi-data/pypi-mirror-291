...
class NingXiang:
    ...
    def get_rf_based_feature_combinations(self, min_features=0, gap=1, n_jobs=1):
        ...
        if self.clf_type == 'Regression':
            self.rf = RandomForestRegressor(n_estimators=100, max_depth=12, max_features=0.75,
                                            random_state=self._seed, ccp_alpha=0, max_samples=0.75, n_jobs=n_jobs)
        elif self.clf_type == 'Classification':
            self.rf = RandomForestClassifier(n_estimators=100, max_depth=12, max_features=0.75,
                                             random_state=self._seed, ccp_alpha=0, max_samples=0.75, n_jobs=n_jobs)
        ...
        self.feature_importance = {self.train_x.columns[i]: self.rf.feature_importances_[
            i] for i in range(len(self.train_x.columns))}

        # use handle (which can be used on its own) to generate the ningxiang output
        self.ningxiang_output = self.get_rf_based_feature_combinations_from_feature_importance(
    ...
    def export_ningxiang_output(self, address):
        ...
        # Export
        object_saving_address_split = self.object_saving_address.split('.pickle')[
            0]
