...
from tiny_diff.models.vae.components import Decoder, Encoder
from tiny_diff.modules import ConvBlock
from tiny_diff.utils import device
...
class VAE(nn.Module, ABC):
    ...
    def __init__(
        ...
        encoder_cls: Optional[type[Encoder]] = None,
        decoder_cls: Optional[type[Decoder]] = None,
        transform: Optional[Callable] = None,
        component_kwargs: Optional[dict[str, Any]] = None,
    ):
        super().__init__()
        component_kwargs = component_kwargs or {}
        self.encoder_cls = encoder_cls or Encoder
        self.decoder_cls = decoder_cls or Decoder
    ...
    def forward_encode(self, x: Tensor) -> dict[str, Tensor]:
    ...
    def forward_decode(self, z: Tensor) -> dict[str, Tensor]:
        """Decoder side forward."""
        return {"x_hat": self.decoder(z)}

    @override
    def forward(self, x: Tensor) -> dict[str, Tensor]:
        enc_outputs = self.forward_encode(x)
        dec_outputs = self.forward_decode(enc_outputs["z"])
