...
def load_SoGL():
    ...
    if lib == None: return lib
        ...
        lib['soglInit'].restype = None

        lib['sgContextDebug'].argtypes = [ POINTER(SGcontext) ]
        lib['sgContextDebug'].restype = None

        lib['sgPollEvents'].restype = None

        lib['sgShouldQuit'].restype = c_ubyte
        lib['sgShouldQuit'].argtypes = [ c_void_p ] 

        lib['sgSetWindowIcon'].restype = None
        lib['sgSetWindowIcon'].argtypes = [ c_void_p, c_void_p ]

        lib['sgSetWindowTitle'].restype = None
        lib['sgSetWindowTitle'].argtypes = [ c_void_p,  c_char_p ]

        lib['sgMakeWindow'].restype = None
        lib['sgMakeWindow'].argtypes = [ c_uint32, c_uint32, c_char_p ]

        lib['sgClearColor'].restype = None
        lib['sgClearColor'].argtypes = [ c_uint32, c_uint32, c_uint32, c_uint32 ]
        
        lib['sgBufferSwap'].restype = None
        lib['sgBufferSwap'].argtypes = [ c_void_p ]
        
        lib['sgWindowValid'].restype = None
        lib['sgWindowValid'].argtypes = [ c_void_p ]
