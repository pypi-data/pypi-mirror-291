...
# Helper function to get the absolute path of the shared library
def get_shared_library_path():
    package_dir = os.path.dirname(__file__)
    if system == "Windows":
        if arch == "32bit":
            return os.path.join(package_dir, 'rdrand32.dll')
        else:
            return os.path.join(package_dir, 'rdrand64.dll')
    elif system == "Linux":
        if arch == "32bit":
            return os.path.join(package_dir, 'librdrand.so')
        else:
            return os.path.join(package_dir, 'librdrand64.so')
    else:
        raise RuntimeError("Unsupported operating system")

# Get the shared library path
lib_path = get_shared_library_path()
c_uint_type = ctypes.c_uint32 if "32bit" in arch else ctypes.c_uint64
