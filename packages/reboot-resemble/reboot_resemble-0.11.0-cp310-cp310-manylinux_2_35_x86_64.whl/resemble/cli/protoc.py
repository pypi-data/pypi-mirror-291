...
def register_protoc(parser: ArgumentParser):
    ...
    parser.subcommand('protoc').add_argument(
        ...
        type=str,
        help=
        "command to run after `protoc` to compile TypeScript files, i.e. 'npx tsc'",
        default=None,
    )
...
async def protoc_direct(
    ...
    if es_out_language is None and args.tsc is not None:
        terminal.fail(
            "You must specify either '--react' or '--nodejs' to use '--tsc'."
        )
    ...
    for protos in protos_by_directory.values():
        for file in protos:
            if os.stat(file).st_size == 0:
                terminal.error(
                    f"'{file}' is empty. "
                    f"See {DOCS_BASE_URL}/docs/model/overview for "
    ...
    # Print if we failed.
    if returncode != 0:
        terminal.error(f'`protoc` failed with return code {returncode}'
                       '\n')
    ...
    if (
        ...
        protoc_args = [
            ...
            if not arg.startswith('--resemble_react_out=') and
            not arg.startswith('--resemble_nodejs_out=')
        ]

        returncode = grpc_tools_protoc.main(protoc_args)

    if args.tsc is not None and returncode == 0:
        async with subprocesses.shell(
            f'{args.tsc}',
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.STDOUT,
