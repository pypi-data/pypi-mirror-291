import pytest
from etripy.sync import AnalysisClient, AnalysisCode
...
# 언어 분석
def test_analysis(analysis: AnalysisClient):
    r = analysis.analysis(text=analysis_text, analysis_code=AnalysisCode.morp)
    assert r.data


# 문장 패러프레이즈 인식
def test_paraphrase(analysis: AnalysisClient):
    r = analysis.paraphrase(sentences[0], sentences[1])
    assert r.data


# 어휘 정보
def test_wordinfo(analysis: AnalysisClient):
    r = analysis.wordinfo(word=word_1)
    assert r.data


# 동음이의어 정보
def test_wordinfo(analysis: AnalysisClient):
    r = analysis.homonym(word=word_1)
    assert r.data


# 다의어 정보
def test_polysemy(analysis: AnalysisClient):
    r = analysis.polysemy(word=word_1)
    assert r.data


# 어휘 간 유사도 분석
def test_wordrel(analysis: AnalysisClient):
    r = analysis.wordrel(first_word=word_1, second_word=word_2)
    assert r.data


# 개체 연결
def test_wordrel(analysis: AnalysisClient):
    r = analysis.nelinking(contents=NE_text)
    assert r[0].data


# 상호참조 해결
def test_coreference(analysis: AnalysisClient):
    r = analysis.coreference(text=conreference_content)
    assert r.data
