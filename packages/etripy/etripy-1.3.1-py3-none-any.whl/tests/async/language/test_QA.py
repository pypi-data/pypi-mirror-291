import pytest
from etripy.client import QAClient, WikiType
...
# 질문 분석
@pytest.mark.asyncio
async def test_analysis(qa: QAClient):
    r = await qa.qanal(text=text)
    assert r.data


# 기계 독해
@pytest.mark.asyncio
async def test_mrc(qa: QAClient):
    r = await qa.mrcservlet(question=mrc_q, passage=mrc_p)
    assert r.data


# 위키백과
@pytest.mark.asyncio
async def test_wiki(qa: QAClient):
    r = await qa.wiki(type=WikiType.hybridqa, question=wiki_q)
    assert r.data


# 법률
@pytest.mark.asyncio
async def test_legal(qa: QAClient):
    r = await qa.legal(question=legal_q)
    assert r.data


# 행정문서
@pytest.mark.asyncio
async def test_docfile(qa: QAClient):
    r = await qa.doc(doc_key=doc_key, question=doc_q)
    assert r.data
