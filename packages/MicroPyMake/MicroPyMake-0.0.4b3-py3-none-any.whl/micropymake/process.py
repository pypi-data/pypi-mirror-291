...
def process_project(config: dict, port: str):
    ...
    if config.get('AutoFlash', {}).get('enable', False):
        ...
        if config['AutoFlash'].get('eraseBeforeFlash', False):
            run_esptool(['--chip', 'esp32', '--port', port, 'erase_flash'])
        ...
        for cmd_list in config['AutoFlash']['flashCommands']:
            for i, cmd in enumerate(cmd_list):
                cmd_list[i] = cmd.replace("{{ port }}", port)
                cmd_list[i] = cmd.replace("{{ flashFile }}", str(flash_file_path))
            
            # 调用 run_esptool 并传入修改后的 cmd_list
            run_esptool(cmd_list)
