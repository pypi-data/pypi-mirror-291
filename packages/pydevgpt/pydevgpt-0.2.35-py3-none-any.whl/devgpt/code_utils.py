...
def decide_use_docker(use_docker: Optional[bool]) -> Optional[bool]:
    if use_docker is None:
        env_var_use_docker = os.environ.get("DEVGPT_USE_DOCKER", "True")
        ...
        if env_var_use_docker_lower in truthy_values:
            ...
            raise ValueError(
                f'Invalid value for DEVGPT_USE_DOCKER: {env_var_use_docker}. Please set DEVGPT_USE_DOCKER to "1/True/yes", "0/False/no", or "None".'
            )
    return use_docker
    ...
    if use_docker is not None:
        ...
        if use_docker and not inside_docker and not docker_installed_and_running:
            raise RuntimeError(
                ...
                '- Set DEVGPT_USE_DOCKER to "0/False/no" in your environment variables'
            )
