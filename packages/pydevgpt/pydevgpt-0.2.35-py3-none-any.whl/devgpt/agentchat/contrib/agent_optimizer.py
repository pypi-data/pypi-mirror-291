...
import devgpt
from devgpt.code_utils import execute_code
...
class AgentOptimizer:
    """
    Base class for optimizing DevGpt agents. Specifically, it is used to optimize the functions used in the agent.
    ...
    def __init__(
        ...
        self.llm_config["config_list"] = devgpt.filter_config(
            llm_config["config_list"], {"model": [self.optimizer_model]}
        )
        self._client = devgpt.OpenAIWrapper(**self.llm_config)
    ...
    def step(self):
        ...
        See example: https://github.com/khulnasoft/devgpt/blob/main/notebook/agentchat_agentoptimizer.ipynb
        """
