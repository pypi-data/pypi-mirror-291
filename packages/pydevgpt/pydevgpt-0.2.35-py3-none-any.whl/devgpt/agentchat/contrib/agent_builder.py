...
def _retrieve_json(text):
    match = re.findall(devgpt.code_utils.CODE_BLOCK_PATTERN, text, flags=re.DOTALL)
    if not match:
        return text
    code_blocks = []
    for _, code in match:
...
"""
    ...
    def __init__(
        ...
        builder_config_list = devgpt.config_list_from_json(config_file_or_env, filter_dict=builder_filter_dict)
        ...
        self.builder_model = devgpt.OpenAIWrapper(config_list=builder_config_list)
        ...
        self.agent_procs_assign: Dict[str, Tuple[devgpt.ConversableAgent, str]] = {}
    ...
    def _create_agent(
        ...
        if use_oai_assistant:
            ...
            model_class = devgpt.AssistantAgent
            if model_path:
                module_path, model_class_name = model_path.replace("/", ".").rsplit(".", 1)
                module = importlib.import_module(module_path)
                model_class = getattr(module, model_class_name)
                if not issubclass(model_class, devgpt.ConversableAgent):
    ...
    def build(
        ...
        use_oai_assistant: Optional[bool] = False,
        user_proxy: Optional[devgpt.ConversableAgent] = None,
        max_agents: Optional[int] = None,
        **kwargs,
    ) -> Tuple[List[devgpt.ConversableAgent], Dict]:
        """
    ...
    def _build_agents(
        self, use_oai_assistant: Optional[bool] = False, user_proxy: Optional[devgpt.ConversableAgent] = None, **kwargs
    ) -> Tuple[List[devgpt.ConversableAgent], Dict]:
        """
