...
Example:
    ...
    agent = devgpt.AssistantAgent("my_agent", llm_config=llm_config)
...
class GeminiClient:
    ...
    Please visit this [page](https://github.com/khulnasoft/devgpt/issues/2387) for the roadmap of Gemini integration
    of DevGpt.
    """

    # Mapping, where Key is a term used by Devgpt, and Value is a term used by Gemini
    PARAMS_MAPPING = {
    ...
    def create(self, params: Dict) -> ChatCompletion:
        ...
        if not model_name:
            raise ValueError(
                ...
                "See this [LLM configuration tutorial](https://khulnasoft.github.io/devgpt/docs/topics/llm_configuration/) for more details."
            )
        ...
        generation_config = {
            gemini_term: params[devgpt_term]
            for devgpt_term, gemini_term in self.PARAMS_MAPPING.items()
            if devgpt_term in params
        }
        if self.use_vertexai:
