...
def get_current_devgpt_env_var():
    return os.environ.get("DEVGPT_USE_DOCKER", None)


def restore_devgpt_env_var(current_env_value):
    if current_env_value is None:
        del os.environ["DEVGPT_USE_DOCKER"]
    else:
        os.environ["DEVGPT_USE_DOCKER"] = current_env_value


def test_decide_use_docker_truthy_values():
    current_env_value = get_current_devgpt_env_var()

    for truthy_value in ["1", "true", "yes", "t"]:
        os.environ["DEVGPT_USE_DOCKER"] = truthy_value
        assert decide_use_docker(None) is True

    restore_devgpt_env_var(current_env_value)
...
def test_decide_use_docker_falsy_values():
    ...
    for falsy_value in ["0", "false", "no", "f"]:
        os.environ["DEVGPT_USE_DOCKER"] = falsy_value
...
def test_decide_use_docker():
    ...
    os.environ["DEVGPT_USE_DOCKER"] = "none"
    assert decide_use_docker(None) is None
    os.environ["DEVGPT_USE_DOCKER"] = "invalid"
...
def test_decide_use_docker_with_env_var():
    ...
    os.environ["DEVGPT_USE_DOCKER"] = "false"
    assert decide_use_docker(None) is False
    os.environ["DEVGPT_USE_DOCKER"] = "true"
    assert decide_use_docker(None) is True
    os.environ["DEVGPT_USE_DOCKER"] = "none"
    assert decide_use_docker(None) is None
    os.environ["DEVGPT_USE_DOCKER"] = "invalid"
...
def _test_improve():
    try:
        ...
        return
    config_list = devgpt.config_list_openai_aoai(KEY_LOC)
    ...
    suggestion, _ = improve_code(
        ["devgpt/code_utils.py", "devgpt/math_utils.py"],
