...
from devgpt.logger.logger_utils import get_current_ts, to_dict
...
def db_connection():
    devgpt.runtime_logging.start(config={"dbname": ":memory:"})
    con = devgpt.runtime_logging.get_connection()
...
def get_sample_chat_completion(response):
    return {
        ...
        "agent": devgpt.AssistantAgent(name="TestAgent", code_execution_config=False),
    }
...
def test_log_completion(response, expected_logged_response, db_connection):
    ...
    devgpt.runtime_logging.log_chat_completion(**sample_completion)
...
def test_log_function_use(db_connection):
    ...
    source = devgpt.AssistantAgent(name="TestAgent", code_execution_config=False)
    func: Callable[[str, int], Any] = dummy_function
    args = {"foo": "bar"}
    returns = True

    devgpt.runtime_logging.log_function_use(agent=source, function=func, args=args, returns=returns)
...
def test_log_new_agent(db_connection):
    from devgpt import AssistantAgent
    ...
    devgpt.runtime_logging.log_new_agent(agent, init_args)
...
def test_log_oai_wrapper(db_connection):
    ...
    devgpt.runtime_logging.log_new_wrapper(wrapper, init_args)
...
def test_log_oai_client(db_connection):
    ...
    devgpt.runtime_logging.log_new_client(client, Mock(), openai_config)
...
def test_to_dict():
    from devgpt import Agent

    agent1 = devgpt.ConversableAgent(
        "alice",
    ...
    agent2 = devgpt.ConversableAgent(
        "bob",
        human_input_mode="NEVER",
        llm_config=False,
    ...
    for agent in result["agents"]:
        assert "devgpt.agentchat.conversable_agent.ConversableAgent" in agent
    assert "devgpt.agentchat.conversable_agent.ConversableAgent" in result["first_agent"]
