...
try:
    ...
    from devgpt.retrieve_utils import (
    ...
    from devgpt.token_count_utils import count_token
except ImportError:
    skip = True
else:
    skip = False
...
expected_text = """DevGpt is an advanced tool designed to assist developers in harnessing the capabilities
of Large Language Models (LLMs) for various applications. The primary purpose of DevGpt is to automate and
...
class TestRetrieveUtils:
    ...
    def test_split_files_to_chunks(self):
        ...
        assert all(
            isinstance(chunk, str) and "DevGpt is an advanced tool designed to assist developers" in chunk.strip()
            for chunk in chunks
        )
    ...
    def test_get_files_from_dir(self):
        ...
        files = get_files_from_dir(
            [
                ...
                "https://raw.githubusercontent.com/khulnasoft/devgpt/main/README.md",
            ],
            recursive=True,
        )
        assert all(os.path.isfile(file) if isinstance(file, str) else os.path.isfile(file[0]) for file in files)
    ...
    def test_query_vector_db(self):
        ...
        results = query_vector_db(["devgpt"], client=client)
        assert isinstance(results, dict) and any("devgpt" in res[0].lower() for res in results.get("documents", []))
    ...
    def test_custom_vector_db(self):
        try:
            ...
            return
        from devgpt.agentchat.contrib.retrieve_user_proxy_agent import RetrieveUserProxyAgent
    ...
    def test_custom_text_split_function(self):
        ...
        results = query_vector_db(["devgpt"], client=client, collection_name="mytestcollection", n_results=1)
        assert (
            "DevGpt is an advanced tool designed to assist developers in harnessing the capabilities"
    ...
    def test_retrieve_utils(self):
        ...
        create_vector_db_from_dir(
            ...
            client=client,
            collection_name="devgpt-docs",
            custom_text_types=["txt", "md", "rtf", "rst"],
            get_or_create=True,
        )
        results = query_vector_db(
            query_texts=[
                "How can I use DevGpt UserProxyAgent and AssistantAgent to do code generation?",
            ],
            n_results=4,
            client=client,
            collection_name="devgpt-docs",
            search_string="DevGpt",
        )
        print(results["ids"][0])
