...
int preceedsby1(int_64 i, int_64 j, int m1)
{
	int_64 b= i & j;
	if(b==i) // i subset of j
	{
		if(bitweight(int_64(~i & j))==1) return 1*m1;
	}
	if(b==j)
	{
...
vector<bool> booleanlatticerestrictednonredundant(int n, int k, int &r, int m1)
{
//Devuelve la matriz del retículo booleano Bn
//	int_64 s = (int_64)1<<n;
	// count the number of items as the sum of Cin
	...
	for(int i=1;i<r;i++)
	{
		...
		for(int j=0;j<i;j++){
			...
			if(IsSubset(a,b) && (preceedsby1(a,b, m1)==-1)) B[r*i+j]=true;
		}
