...
from analytics import create_flat_ir_yield_curve
...
def create_bond_curve_build_settings(curve_name, curve_type,
    ...
    '''
    p_build_settings = dqCreateProtoBondYieldCurveBuildSettings(curve_name.upper(), curve_type,
                                                                interp_method, extrap_method)
...
def create_bond_par_curve(as_of_date, currency, inst_names, quotes, quote_type, curve_name):
    ...
    pb_input = dqCreateProtoCreateBondParCurveInput(as_of_date,
                                                    currency.upper(),
                                                    p_pillars,
                                                    quote_type,
                                                    curve_name.upper())
...
def build_bond_yield_curve(build_settings,
    ...
    pb_input = dqCreateProtoBondYieldCurveBuildingInput(as_of_date,
...
def build_bond_sprd_curve(build_settings,
    ...
    if fwd_curve is None:
        fwd_curve = create_flat_ir_yield_curve(as_of_date, par_curve.currency, 0.0)
    pb_input = dqCreateProtoBondCreditSpreadCurveBuildingInput(as_of_date,
                                                               par_curve, discount_curve, fwd_curve,
                                                               curve_name, build_method, calc_jacobian)
...
def create_fi_mkt_data_set(as_of_date,
    ...
    '''
    return dqCreateProtoFiMktDataSet(as_of_date,
                                     discount_curve,
                                     credit_sprd_curve,
                                     forward_curve,
                                     underlying_discount_curve,
...
def vanilla_bond_pricer(instrument, pricing_date, mkt_data, pricing_settings, risk_settings):
    ...
    '''
    pb_input = dqCreateProtoVanillaBondPricingInput(pricing_date, instrument, mkt_data, pricing_settings, risk_settings,
                                                    False, b'', b'', b'', b'')
...
def yield_to_maturity_calculator(calculation_date, compounding_type, bond, forward_curve, price, price_type, frequency):
    ...
    pb_input = dqCreateProtoYieldToMaturityCalculationInput(calculation_date, compounding_type, bond, forward_curve, price, price_type, frequency)
