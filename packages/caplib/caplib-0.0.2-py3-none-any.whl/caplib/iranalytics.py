...
def create_ir_par_rate_curve(as_of_date, currency, curve_name, inst_names, inst_types, inst_terms, factors, quotes):
    ...
    '''
    pb_input = dqCreateProtoCreateIrParRateCurveInput(as_of_date, currency, curve_name,
    ...
    for i in range(len(target_curves)):
        p_build_settings.append(
            dqCreateProtoIrSingleCurrencyCurveBuildingInput_IrYieldCurveBuildSettingsContainer(target_curves[i],
                                                                                               ...
                                                                                               compounding_type,
                                                                                               frequency))
    pb_input = dqCreateProtoIrSingleCurrencyCurveBuildingInput(as_of_date, p_build_settings, other_curves,
                                                               building_method, 
                                                               calc_jacobian, 
                                                               shift, 
...
def create_ir_mkt_data_set(as_of_date,
    ...
    '''
    return dqCreateProtoIrMktDataSet(as_of_date, discount_curve, underlyings, forward_curves)
...
def ir_vanilla_instrument_pricer(instrument, pricing_date, mkt_data, pricing_settings, risk_settings):
    ...
    '''
    pb_input = dqCreateProtoIrVanillaInstrumentPricingInput(pricing_date, instrument, mkt_data, pricing_settings,
                                                            risk_settings,
                                                            False, b'', b'', b'', b'')
    ...
    for i in range(len(target_curves)):
        p_build_settings.append(
            dqCreateProtoIrCrossCurrencyCurveBuildingInput_IrYieldCurveBuildSettingsContainer(target_curves[i],
                                                                                              ...
                                                                                              compounding_type,
                                                                                              frequency))
    pb_input = dqCreateProtoIrCrossCurrencyCurveBuildingInput(as_of_date,
                                                              p_build_settings,
                                                              other_curves,
                                                              fx_spot,
                                                              building_method, 
