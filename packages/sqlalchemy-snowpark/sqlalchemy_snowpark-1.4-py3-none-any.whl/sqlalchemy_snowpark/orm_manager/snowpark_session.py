        ...
        def get_tuple_value(data):
            ...
            for col, value in data.items():
                if isinstance(value, str):
                    value.replace("'", "''")
                elif value is None and columns_info.get(col) == "BOOLEAN":
                    print("do nothing here ...", end="\n\n*5" )
                elif value is None and columns_info.get(col) == "VARCHAR":
                    value = "NULL"
                list_value.append(value)
            return tuple(list_value)

        values = get_tuple_value(data)
        insert_query = f"""
