...
def generate_random_contact() -> dict:
    ...
    """
    fake = Faker("uk_UA" if app_settings.language == "ua" else "en_US")
    ...
    data = {
        ...
        "birthday": (
            fake.date_of_birth().strftime(app_settings.date_format)
            if random() < 0.7 else ""
        ),
        "email": (fake.email() if random() < 0.7 else ""),
        "address": (
            f"{fake.street_address()}, {fake.city()}, {fake.country()}"
            if random() < 0.7
            else ""
        ),
    }
