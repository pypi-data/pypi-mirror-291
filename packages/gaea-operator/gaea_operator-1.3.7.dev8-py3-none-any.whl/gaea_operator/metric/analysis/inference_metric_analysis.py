    ...
    def _format_input(self, predictions: List[Dict], references: List[Dict]):
        ...
        for item in references:
            ...
            for anno in item["annotations"]:
                if anno.get("ocr"):
                    array_item = anno.get("ocr", {}).get("word", "")
                    break
        ...
        for img_id, anno in reference_dict.items():
            ...
            if img_id in prediction_dict:
                ...
                if len(anno) > 0 and not isinstance(anno[0], np.ndarray):
                    # 都为空字符串
                    if anno == "" and prediction_anno == "":
                        array_item_ref[0] = 0
                        array_item_pred[0] = 0
                    # 预测和真实标注相同且不为空字符串
                    elif anno == prediction_anno and anno != "":
                        array_item_ref[0] = 1
                        array_item_pred[0] = 1
                    # 真实标注为空，预测结果不为空
                    elif anno == "" and prediction_anno != "":
                        array_item_ref[0] = 0
                        array_item_pred[0] = 1
                    # 真实标注不为空，预测结果为空
                    elif anno != "" and prediction_anno == "":
                    ...
                    reference_list.extend(array_item_ref)
                    prediction_list.extend(array_item_pred)
                else:
