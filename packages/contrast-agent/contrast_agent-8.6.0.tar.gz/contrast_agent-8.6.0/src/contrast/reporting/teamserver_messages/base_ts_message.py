...
class BaseTsMessage:
    ...
    @property
    def ignore_404(self) -> bool:
        """
        Overridden by endpoints that we implement before teamserver. Overriding this to
        True should be temporary and accompanied by a ticket number.
        """
        return False
    ...
    def process_response_code(
        ...
        if response.status_code == 404:
            if self.ignore_404:
                logger.debug(
                    "Ignoring 404 response from Teamserver and proceeding normally",
                    message_type=self.class_name,
                )
                return True
