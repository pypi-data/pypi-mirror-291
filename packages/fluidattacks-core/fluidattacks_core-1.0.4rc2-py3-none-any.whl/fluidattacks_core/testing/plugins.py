...
class CustomFixturesPlugin:
    ...
    def moto_session(self) -> GeneratorFixture[str]:
        """Starts the moto server and mocks the boto3 and aioboto3 sessions."""
        mocked_boto = _moto.mock_aws()
        aio_session = _aioboto3.Session()

        sessions: list[Patch] = [
            patch("aioboto3.Session", lambda _: aio_session),
            patch("aioboto3.session.Session", lambda _: aio_session),
        ]
    ...
    def dynamodb_resource(
        ...
        resource = session.resource(
            endpoint_url=moto_session, service_name="dynamodb"
        )
    ...
    async def async_dynamodb_resource(
        ...
        async with session.resource(
            endpoint_url=moto_session, service_name="dynamodb", config=config
        ) as resource:
