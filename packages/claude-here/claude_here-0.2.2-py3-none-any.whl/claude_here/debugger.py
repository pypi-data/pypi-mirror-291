...
def filter_traceback(orig_tb):  #123 (line in Coconut source)
    """Filter out traceback frames from claude_here."""  #124 (line in Coconut source)
    new_tb_top = orig_tb  #125 (line in Coconut source)
    while new_tb_top is not None and new_tb_top.tb_frame.f_globals.get("__package__") in IGNORE_PACKAGES:  #126 (line in Coconut source)
        new_tb_top = new_tb_top.tb_next  #127 (line in Coconut source)

    if new_tb_top is not None:  #129 (line in Coconut source)
        tb_cursor_plus_1 = new_tb_top  #130 (line in Coconut source)
        tb_cursor = tb_cursor_plus_1.tb_next  #131 (line in Coconut source)
        while tb_cursor is not None:  #132 (line in Coconut source)
            tb_cursor_minus_1 = tb_cursor.tb_next  #133 (line in Coconut source)
            if tb_cursor.tb_frame.f_globals.get("__package__") in IGNORE_PACKAGES:  #134 (line in Coconut source)
                tb_cursor_plus_1.tb_next = tb_cursor_minus_1  #135 (line in Coconut source)
                tb_cursor_plus_1, tb_cursor = (tb_cursor_plus_1, tb_cursor_minus_1)  #136 (line in Coconut source)
            else:  #140 (line in Coconut source)
                tb_cursor_plus_1, tb_cursor = (tb_cursor, tb_cursor_minus_1)  #141 (line in Coconut source)

    return new_tb_top  #146 (line in Coconut source)



def get_tb_frames(tb):  #149 (line in Coconut source)
    """Yield all the frames in the given traceback."""  #150 (line in Coconut source)
    while tb is not None:  #151 (line in Coconut source)
        yield tb.tb_frame  #152 (line in Coconut source)
        tb = (tb).tb_next  #153 (line in Coconut source)



def collect_exc_info(exc_type, exc_val, exc_tb):  #156 (line in Coconut source)
    """Collect information about the given exception for sending to Claude."""  #157 (line in Coconut source)
    filtered_tb = (lambda _coconut_x: exc_tb if _coconut_x is None else _coconut_x)(filter_traceback(exc_tb))  #158 (line in Coconut source)

    extra_info = {"traceback": (("".join)(traceback.format_exception(exc_type, exc_val, filtered_tb)))}  #160 (line in Coconut source)

    if filtered_tb is None:  #167 (line in Coconut source)
        return DebugContext(name="exception", frame_info=None, source_type=None, raw_source="", extra_info=extra_info)  #168 (line in Coconut source)

    else:  #176 (line in Coconut source)
        all_frames = (tuple)((get_tb_frames)(filtered_tb))  #177 (line in Coconut source)
        verbosity = get_verbosity()  #178 (line in Coconut source)
        for frame in all_frames[:verbosity // 2] + all_frames[-(1 + verbosity // 2):-1]:  #179 (line in Coconut source)
            collect_frame_info("exception_frame", frame)  #180 (line in Coconut source)

        return collect_frame_info("exception", all_frames[-1], extra_info)  #182 (line in Coconut source)
