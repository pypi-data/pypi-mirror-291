...
@pytest.mark.usefixtures("backup_warehouse")
...
def shared_database_available(cursor):
    ...
    if shared_database is None or len(shared_database) == 0 \
        or shared_database[2] != 'SFSALESSHARED.SFC_SAMPLES_PROD3':
...
def my_integration_exists(cursor):
    ...
    if integrations is None or len(integrations) == 0 \
        or integrations[1] != 'my_integration':
