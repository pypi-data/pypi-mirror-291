...
from ..utils import (
    backup_existing_create_and_use_new_database_and_schema,
    connection_config,
    connection_keys,
...
def run():
    ...
    with snowflake.connector.connect(**{k: config[k] for k in _keys if k in config}) as connection:
        ...
        try:
            database_name, schema_name = backup_existing_create_and_use_new_database_and_schema(
                cursor, TEST_DATABASE_NAME, TEST_SCHEMA
            )
