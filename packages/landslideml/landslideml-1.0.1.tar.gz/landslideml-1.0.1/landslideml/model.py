...
class MlModel:
    ...
    def setup(self, **kwargs):
        ...
        invalid_kwargs = [
            kwarg for kwarg in kwargs if kwarg not in self.model.get_params()
        ]
        if invalid_kwargs:
        ...
        if type_of_data == pd.DataFrame:
            ...
            if not os.path.isfile(data):
                ...
                if values.shape[1] != len(features_list):
                    raise ValueError(
                        "The number of features does not match the "
                ...
                raise ValueError(
                    "Invalid file format. Supported formats are CSV and NetCDF."
                )
        else:
            raise ValueError(
                "Unsupported data type. Supported types are pandas DataFrame, "
