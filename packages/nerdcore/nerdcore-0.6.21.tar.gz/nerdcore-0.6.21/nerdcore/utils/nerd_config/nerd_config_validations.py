...
def validate_config_name(config_name: str = None) -> str:
    if not config_name.replace('-', '').isalpha():
        raise TypeError(f"Config name must contain only letters and hyphens")
    return config_name
