...
class TimeSeriesImputer(AzureMLTransformer, _GrainBasedStatefulTransformer):
    ...
    PANDAS_INT_TYPES = (pd.Int16Dtype, pd.Int32Dtype, pd.Int64Dtype,
    ...
    def _impute_with_fillna_single_group(cls, single_group_data: pd.DataFrame, **kwargs: Any) -> pd.DataFrame:
        ...
        if value_ is not None:
            # The value may be not compatible with column dtype,
            # leading to errors. It is especially a problem for pandas uint types.
            if isinstance(value_, dict):
                for k, v in value_.items():
                    if k in single_group_data.columns:
                        single_group_data = TimeSeriesImputer._convert_column_type_maybe(
                            k, single_group_data, v)
            else:
                for col in single_group_data.columns:
                    single_group_data = TimeSeriesImputer._convert_column_type_maybe(
                        col, single_group_data, value_)
            single_group_data = single_group_data.fillna(value=value_)

        return single_group_data

    @staticmethod
    def _convert_column_type_maybe(column_name: str, data: pd.DataFrame, imputation_value: Any) -> pd.DataFrame:
        """
        Convert column to float dtype if otherwise the imputation will fail.

        The failure can happen if we are trying to impute column of int type by the float value.
        :param column_name: The name of a column to be converted.
        :param data: The target data frame.
        :param imputation_value: Imputation value.
        :return: The change happens inplace, but the data frame is returned for convenience.
        """
        # If there is no null values, there is nothing to do.
        if data[column_name].isnull().sum() == 0:
            return data
        # If the type is not a pandas type, it will be handled appropriately.
        if not isinstance(data[column_name].dtype, TimeSeriesImputer.PANDAS_INT_TYPES):
            return data
