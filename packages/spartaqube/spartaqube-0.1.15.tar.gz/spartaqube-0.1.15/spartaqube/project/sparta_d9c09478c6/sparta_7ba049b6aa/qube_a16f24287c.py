...
from project.sparta_d9c09478c6.sparta_b30304f138 import qube_6d5627cb99 as qube_6d5627cb99
def sparta_83e822149c():return str(uuid.uuid4())
def sparta_ecc8a29791(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_cbbff38f51(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_037b52f6a1(path):
	with open(path,'a'):os.utime(path,None)
def sparta_f1e98e51cf(json_data,userObj):
	A=json_data;print('CREATE RESOURCE');print(A);C=A[_B];D=sparta_cbbff38f51(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_037b52f6a1(B)
	...
	return{_A:1}
def sparta_e8b08d8158(json_data,userObj):
	...
	return{_A:1}
def sparta_f469db892a(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_cbbff38f51(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_56fb05fa01(json_data,userObj):A=json_data[_B];B=qube_6d5627cb99.sparta_f7806229ef(A);C={_A:1,'folderStructure':B};return C
def sparta_caab3d1049(json_data,userObj):
	A=json_data
	try:
...
def sparta_c6b268829a(json_data,userObj):
	...
	return{_A:1}
def sparta_1407f22b87(json_data,userObj):
	...
	return{_A:1}
def sparta_901f626407(json_data,userObj):
...
def sparta_68ca4a75d9(json_data,userObj):
	...
	return{_A:-1}
def sparta_0a557e5b21(json_data,userObj):
