...
def sparta_241a35df77(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_3f38ec1348(G,F,A,B);H={_A:1,_C:C};return H
def sparta_3f38ec1348(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_3a0f679c37(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_d93e0670f7(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_f0e9ac630f(json_data,user_obj):A=sparta_3a0f679c37(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_4b33f9efc8(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_39b764d86a(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_3f38ec1348(G,A,I,C)
	J={_A:1};return J
def sparta_a40ed1f7b9(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_7204d7a68e(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_b6def299fb(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_7dfcf3974e(json_data,user_obj):
