...
from spartaqube_app.secrets import sparta_02059ae89a
from project.sparta_866369bc29.sparta_ca928df26a.qube_fd598af40b import sparta_d93d051884
import pytz
UTC=pytz.utc
class dotdict(dict):__getattr__=dict.get;__setattr__=dict.__setitem__;__delattr__=dict.__delitem__
def sparta_e37e202a94(appViewsModels):
	A=appViewsModels
	...
	return A
def sparta_8457d1d8dd(thisText):A=os.path.dirname(os.path.dirname(os.path.abspath(__file__)));A=A+str('/log/log.txt');B=open(A,'a');B.write(thisText);B.writelines('\n');B.close()
def sparta_9a1f514272(request):A=request;return{'appName':'Project','user':A.user,'ip_address':A.META['REMOTE_ADDR']}
def sparta_7e85a19e0f():return conf_settings.PLATFORM
def sparta_da71b00c6d():
...
def sparta_a8bb6e74e5(request):
	...
	try:
		...
		if F.count()==0:AppVersioning.objects.create(last_check_date=E);A=_A
			...
			D=F[0];G=D.last_check_date;H=E-G;I=sparta_d93d051884();J=D.last_available_version_pip
			if not I==J:A=_A
...
def sparta_51c79229a7(captcha):
	D='errorMsg';B='res';A=captcha
	try:
		if A is not _C:
			if len(A)>0:
				E=sparta_02059ae89a()['CAPTCHA_SECRET_KEY'];F=f"https://www.google.com/recaptcha/api/siteverify?secret={E}&response={A}";C=requests.get(F)
				if int(C.status_code)==200:
					G=json.loads(C.text)
					if G['success']:return{B:1}
	except Exception as H:return{B:-1,D:str(H)}
	return{B:-1,D:'Invalid captcha'}
def sparta_94e407787e(password):
...
def sparta_7e43fffad4(user):
	...
	A[_D]=sparta_da71b00c6d();return A
def sparta_3b31098948(user):A=dict();A[_D]=sparta_da71b00c6d();return A
def sparta_3263366db0():
	try:socket.create_connection(('1.1.1.1',53));return _A
	except OSError:pass
	return _B
def sparta_0695838d08():A=socket.gethostname();B=socket.gethostbyname(A);return A,B