...
from project.sparta_addce897a6.sparta_cca83b7c2a.qube_bb2a0cc88d import sparta_51c79229a7,sparta_5e7859315f,sparta_94e407787e
from project.sparta_866369bc29.sparta_a6cd567dfa.qube_bc116ca566 import sparta_ba4d577718
def sparta_86be17f644(json_data,user_obj):
...
def sparta_ea8d7c724f(message,typeCase=0,companyName=None):
	...
	if C.count()>0:
		...
		A.send()
def sparta_6160ce69a7(json_data,userObj):
	...
	return B
def sparta_ce20163d5f(json_data,userObj):
	...
	return A
def sparta_037d26bd66(json_data,userObj):
	D=json_data;F=D['old_spartaqube_code'];G=D['new_spartaqube_code']
	if not sparta_ba4d577718(F):return{_A:-1,_B:'Invalid current code'}
	...
	return{_A:1}
def sparta_3674dcf3e4(json_data,userObj):A=userObj;C=json_data['base64image'];K=hashlib.sha256((str(A.id)+'_'+A.email+str(datetime.now())).encode(_D)).hexdigest();D,E=C.split(';base64,');F,L=D.split('/');G=F.split(':')[-1];B=UserProfile.objects.get(user=A);H=datetime.now();I=Avatar.objects.create(avatar=G,image64=E,date_created=H);B.avatar=I;B.save();J={_A:1};return J
def sparta_02b2b5f537(json_data,userObj):B=json_data['bDarkTheme'];A=UserProfile.objects.get(user=userObj);A.is_dark_theme=B;A.save();C={_A:1};return C
def sparta_f43d950765(json_data,userObj):B=json_data['theme'];A=UserProfile.objects.get(user=userObj);A.editor_theme=B;A.save();C={_A:1};return C
def sparta_43abdb22a6():B='spartaqube-reset-password';A=B.encode(_D);A=hashlib.md5(A).hexdigest();A=base64.b64encode(A.encode(_D));return A.decode(_D)
def sparta_cc3a4e0034(json_data):
	A=json_data;C=A[_F];E=A[_G];B=A[_H];F=A[_Q]
	if not sparta_94e407787e(E):return{_A:-1,_B:_I}
...
def sparta_3cb94ef006(json_data):
	A=json_data;E=A[_E];B=A[_F];F=A[_G];G=sparta_51c79229a7(E)
	if G[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_5e7859315f(F):return{_A:-1,_B:_I}
...
def sparta_6d068456dc(request,json_data):
	A=json_data;F=A[_E];D=A[_F];G=A[_G];H=A[_T];E=A[_H];I=A[_Q];J=sparta_51c79229a7(F)
	if J[_A]!=1:return{_A:-1,_B:_S}
	if not sparta_5e7859315f(G):return{_A:-1,_B:_I}
