...
from project.sparta_866369bc29.sparta_2bf8c7b5ce import qube_d735150d5c as qube_d735150d5c
def sparta_35b79e3565():return str(uuid.uuid4())
def sparta_4e2a2605c2(app_id):
	B=coreApps.get_app_folder_default_path();A=os.path.join(B,app_id)
	if not os.path.exists(A):os.makedirs(A)
	return A
def sparta_75d61e4d30(project_path):
	A=project_path
	try:
		if not os.path.exists(A):os.makedirs(A)
		return{_A:1}
	except Exception as B:return{_A:-1,_C:str(B)}
def sparta_cdc7c6d9b0(path):
	with open(path,'a'):os.utime(path,None)
def sparta_89d2d9b6d2(json_data,userObj):
	A=json_data;print('CREATE RESOURCE');print(A);C=A[_B];D=sparta_75d61e4d30(C)
	if D[_A]==-1:return D
	...
	try:
		...
		elif not os.path.exists(B):sparta_cdc7c6d9b0(B)
	...
	return{_A:1}
def sparta_de82ececc6(json_data,userObj):
	...
	return{_A:1}
def sparta_7864523e78(json_data,user_obj,file_obj_to_upload):
	D=file_obj_to_upload;B=json_data;A=B[_B];E=sparta_75d61e4d30(A)
	if E[_A]==-1:return E
	...
	I={_A:1};return I
def sparta_f4930e5af6(json_data,userObj):A=json_data[_B];B=qube_d735150d5c.sparta_936dc63709(A);C={_A:1,'folderStructure':B};return C
def sparta_17439e2bce(json_data,userObj):
	A=json_data
	try:
...
def sparta_862d432060(json_data,userObj):
	...
	return{_A:1}
def sparta_4a6357ee1c(json_data,userObj):
	...
	return{_A:1}
def sparta_b7521164b8(json_data,userObj):
...
def sparta_20631d492c(json_data,userObj):
	...
	return{_A:-1}
def sparta_de80ed9fb3(json_data,userObj):
