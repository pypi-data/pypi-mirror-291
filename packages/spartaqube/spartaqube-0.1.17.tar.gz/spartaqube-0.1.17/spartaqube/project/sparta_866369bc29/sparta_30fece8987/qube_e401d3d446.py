...
def sparta_20cac710d1(json_data,user_obj,group_id=None):
	...
	F=UserGroup.objects.create(name=E,group_id=C,user_creator=B,date_created=A,last_update=A);UserGroupUser.objects.create(user_group=F,user=B,b_admin=_B,date_created=A,last_update=A);sparta_f9bfb8f884(G,F,A,B);H={_A:1,_C:C};return H
def sparta_f9bfb8f884(membersArr,user_group_obj,dateCreation,userMe):
...
def sparta_b2d5721e85(user_obj):A=user_obj;B=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_B,user_group__is_delete=0).all();C=UserGroupUser.objects.filter(is_delete=0,user=A,b_admin=_D,user_group__is_delete=0).all();D={_E:B,_F:C};return D
def sparta_e9fa5cd25c(user_obj):return UserGroupUser.objects.filter(is_delete=0,user=user_obj,user_group__is_delete=0)
def sparta_d3ebaf770a(json_data,user_obj):A=sparta_b2d5721e85(user_obj);B=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_E]];C=[{_C:A.user_group.groupId,_G:A.user_group.name}for A in A[_F]];return{_A:1,_E:B,_F:C}
def sparta_23febc1374(json_data,user_obj):
	...
	B={_A:-1};return B
def sparta_b089ca0c78(json_data,user_obj):
	...
	if D.count()>0:
		...
		if H.count()>0:A.name=F;A.save();I=datetime.now();sparta_f9bfb8f884(G,A,I,C)
	J={_A:1};return J
def sparta_62999214a0(json_data,user_obj):
	...
	I={_A:1};return I
def sparta_e483b01f80(json_data,user_obj):
	...
	D={_A:1};return D
def sparta_6843290f50(json_data,user_obj):
	...
	H={_A:1};return H
def sparta_d1cb62cbf1(json_data,user_obj):
