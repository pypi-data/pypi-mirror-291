...
from project.sparta_addce897a6.sparta_cca83b7c2a.qube_bb2a0cc88d import sparta_e37e202a94
ADMIN_EMAIL=conf_settings.ADMIN_EMAIL_TICKET
MAX_TICKETS=conf_settings.MAX_TICKETS
def sparta_f2f76a8db7(json_data,user_obj):
...
def sparta_558cc3abbf(message,typeCase=0,companyName=None):
	...
	if E.count()>0:
		...
		A.send()
def sparta_8c7c37392b(json_data,user_obj):
	...
	if E.is_staff:
		...
		if B.count()>0:
			for D in B:A=sparta_e37e202a94(model_to_dict(D));del A[G];A[_E]=naturalday(parser.parse(str(A[_E])));C.append(A)
		return{_A:1,H:C}
	else:
		B=ticket.objects.filter(user=E,is_delete=0,has_user_closed=F).order_by('-date_created');C=[]
		if B.count()>0:
			for D in B:A=sparta_e37e202a94(model_to_dict(D));del A[G];C.append(A)
		return{_A:1,H:C}
def sparta_708aa57296(json_data,user_obj):
	...
	if F.count()>0:
		...
		if B.count()>0:
			...
			for(K,L)in enumerate(B):
				H=L.user;A=sparta_e37e202a94(model_to_dict(L));A[_E]=naturalday(parser.parse(str(A[_E])))
				if D==H:A['me']=1
				else:A['me']=0
				if H==J:
					C.append(A)
...
def sparta_a39e518ee9(json_data,user_obj):
	...
	if C.count()>0:
		...
		if A.b_send_email and not B.is_staff:A.b_send_email=_B;A.save();sparta_558cc3abbf(F,A.type_ticket,None)
...
def sparta_d0fa843efc(json_data,user_obj):
	...
	return{_A:1}
def sparta_354786e2f9(json_data):
	...
	return{_A:1}
def sparta_43348df6a7(user_obj):A=ticket.objects.filter(user=user_obj,b_show_user_notification=_C,has_user_closed=_B);return{_A:1,'nbNotifications':A.count()}