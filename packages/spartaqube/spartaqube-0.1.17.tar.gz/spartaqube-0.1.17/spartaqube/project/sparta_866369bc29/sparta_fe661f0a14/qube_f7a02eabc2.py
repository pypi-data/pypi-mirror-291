...
from project.sparta_866369bc29.sparta_b014af19be import qube_45ac68bd39
from project.sparta_866369bc29.sparta_fe661f0a14 import qube_bd3eb0133e as qube_bd3eb0133e
...
from project.sparta_866369bc29.sparta_fe661f0a14 import qube_fc7c4fa321 as qube_fc7c4fa321
...
def sparta_38b522527b(user_obj):
	...
	return B
def sparta_cb4fddd9a4(json_data,user_obj):
	D=user_obj;E=sparta_38b522527b(D)
...
def sparta_baa36393cc():return{_A:1,'available_engines':qube_45ac68bd39.sparta_baa36393cc()}
def sparta_12b163b5d5(json_data,user_obj):
	...
	return{_A:1}
def sparta_8f7018288e(json_data):
...
def sparta_f4ea943f29(json_data):
...
def sparta_e5bff1c519(json_data,user_obj):A=json_data;B=datetime.now().astimezone(UTC);C=str(uuid.uuid4());D=DBConnector.objects.create(connector_id=C,host=A[_S],port=A[_T],user=A[_U],password_e=qube_45ac68bd39.sparta_0f8c4d677a(A[_j]),database=A[_V],oracle_service_name=A[_W],keyspace=A[_X],library_arctic=A[_Y],database_path=A[_Z],csv_path=A[_a],csv_delimiter=A[_b],read_only=A[_c],json_url=A[_d],socket_url=A[_e],redis_db=A[_f],token=A[_k],organization=A[_l],lib_dir=A[_m],driver=A[_n],trusted_connection=A[_o],dynamic_inputs=json.dumps(A[_I]),py_code_processing=A[_g],db_engine=A[_N],name=A[_J],description=A[_K],date_created=B,last_update=B,last_date_used=B);E=ShareRights.objects.create(is_admin=_C,has_write_rights=_C,has_reshare_rights=_C,last_update=B);DBConnectorUserShared.objects.create(db_connector=D,user=user_obj,date_created=B,share_rights=E,is_owner=_C);return{_A:1}
def sparta_988160d3f1(json_data,user_obj):
	...
	if D.count()>0:
		A=D[D.count()-1];F=sparta_38b522527b(C)
...
def sparta_c5161773e6(json_data,user_obj):
	C=json_data[_H];A=sparta_9b5d44f713(C,user_obj)
...
def sparta_6152a5fb37(json_data,user_obj):
	A=json_data;H=A[_H];C=A[_O];G=A[_p];D=[];E=sparta_9b5d44f713(H,user_obj)
...
def sparta_8dd1eb0a10(json_data,db_connector_obj):
...
def sparta_5d329674d4(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,_B);F=int(A.get(_p,'0'))==1;B=sparta_9b5d44f713(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_8dd1eb0a10(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
			H=A[_y]
	...
	return{_A:1,_E:sparta_48b818a2d6(E)}
def sparta_47c14f5fa8(json_data,user_obj):
	A=json_data;G=A[_H];D=A.get(_O,'');F=A.get(_p,_D);K=A.get(_N,_B);B=sparta_9b5d44f713(G,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_8dd1eb0a10(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B)
	if F:
		if F:
...
def sparta_2b3df9d0cd(json_data,user_obj):
	C=json_data
	...
	return{_A:1,'table':B}
def sparta_21edeec68c(json_data,user_obj):
	A=json_data;print('json_data load_table_preview_explorer');print(A);D=A[_H];F=A.get(_O,_B);G=int(A.get(_p,'0'))==1;B=sparta_9b5d44f713(D,user_obj)
	if B is _B:return{_A:-1,_F:_h}
	sparta_8dd1eb0a10(A,B);C=Connector(db_engine=B.db_engine);C.init_with_model(B);E=C.get_db_connector().get_wss_structure();return{_A:1,_E:sparta_48b818a2d6(E)}
def sparta_12f0bcd5f9(json_data,user_obj):
	A=json_data;J=A[_L];D=_B
	if J:D=A[_s];D=qube_bd3eb0133e.sparta_ff6ad5ea42(D)
...
def sparta_a7fc26f223(json_data,user_obj):
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if J.is_admin or J.has_write_rights:
				M=B[_L];C=_B
				if M:C=B[_s];C=qube_bd3eb0133e.sparta_ff6ad5ea42(C)
	...
	return{_A:1}
def sparta_934eaf5618(json_data,user_obj):0
def sparta_68c014cfbc(json_data,user_obj):
	D=user_obj;F=sparta_38b522527b(D)
...
def exec_notebook_and_get_workspace_variables(full_code,data_source_variables,workspace_variables):
	...
	return B
def sparta_023e09ac51(json_data,user_obj):
			...
			if N.count()>0:
				...
				if b:0
					...
					for B in A.data_source_list:
						...
						if I:H.append(B[Z])
							...
							if _I in B:
								c=B[_H];Q=sparta_9b5d44f713(c,P);R=[]
				...
				if len(H)>0:
					if E is not _B:
						...
						for B in A.data_source_list:
							I=B[Y]
							if I:W=B[Z];i=h[W];B[_E]=sparta_48b818a2d6(sparta_be9cdd17e4(i,variable_name=W))
...
def sparta_0df7b1c982(json_data,user_obj):
	...
	return{_A:1}
def sparta_20eca253c7(session_id):
	...
	for(A,F)in E.items():
		...
		if C is not _B:
			...
			if A==_R:B[A]=D
			else:B[A]=sparta_48b818a2d6(sparta_be9cdd17e4(D,variable_name=A))
	return B
def sparta_877569cb3c(session_id):A=sparta_20eca253c7(session_id);B=sparta_193c15d1ab(b_return_type_id=_C);C=json.loads(A[_AS])[_E][0][0];D=[A for A in B if A['ID']==C][0][_AT];return{_A:1,_AU:A,_AV:A.get(_R,dict()),_Q:D}
def sparta_cce3ebf202(user_obj,session_id,widget_id):
	...
	if E.count()>0:
		...
		if not C:
			...
			if H.count()>0:C=_C
	if C:B=A.chart_params;B[_R]=json.loads(B[_R]);B[_AA]=json.loads(B[_E])[_AA];F=sparta_20eca253c7(session_id);I=sparta_193c15d1ab(b_return_type_id=_C);J=json.loads(F[_AS])[_E][0][0];K=[A for A in I if A['ID']==J][0][_AT];L=F.get(_R,dict());B[_R].update(L);return{_A:1,_AU:F,_AV:B,_Q:K}
	else:return{_A:-1,_F:'You do not have access to this template'}
def sparta_1bb2baaae7(json_data,user_obj):
	...
	try:
		...
		if L.count()>0:
			A=L[L.count()-1];T=sparta_38b522527b(K)
	...
	if A.is_expose_widget:
		if A.is_public_widget:
			if not F:return _C
				...
				try:
					if qube_bd3eb0133e.sparta_9530454362(A.widget_password_e)==password_widget:return _C
					else:return _D
				except:return _D
		else:return _D
	return _D
def sparta_407787c88f(plot_chart_id,user_obj,password_widget=_B):
		...
		if A.is_expose_widget:
			if A.is_public_widget:
				if not J:return{_A:1,_M:A}
					...
					try:
						if qube_bd3eb0133e.sparta_ff6ad5ea42(F)==A.widget_password_e:return{_A:1,_M:A}
						else:return{_A:3,_F:_A9,_M:A}
					except:return{_A:3,_F:_A9,_M:A}
			elif C.is_authenticated:
				G=sparta_38b522527b(C)
	...
	return{_A:-1}
def sparta_a2177a8814(plot_chart_id,user_obj):
	...
	if A.count()==1:D=_C
		...
		if A.count()==1:D=_C
	if D:
		B=A[A.count()-1];G=sparta_38b522527b(C)
	...
	return{_A:1,_AC:_D}
def sparta_e517a0c101(plot_db_chart_obj):
	...
	return C
def sparta_c3f32b1295(json_data,user_obj):
	B=user_obj;D=sparta_38b522527b(B)
	...
	if F.count()>0:
		...
		if G.count()>0:
			...
			if I.is_admin or I.has_write_rights:
				N=B[_L];C=_B
				if N:C=B[_s];C=qube_bd3eb0133e.sparta_ff6ad5ea42(C)
...
def sparta_c4496731f1(json_data,user_obj):
	...
	if E.count()>0:
		...
		if F.count()>0:
			...
			if H.is_admin or H.has_write_rights:
				K=B[_L];C=_B
				if K:C=B[_s];C=qube_bd3eb0133e.sparta_ff6ad5ea42(C)
	...
	return{_A:1}
def sparta_81e96ec293(json_data,user_obj):
	...
	return{_A:1}
def sparta_3af1e9f5c2(user_obj):
	B=user_obj;C=sparta_38b522527b(B)
	...
	return E
def sparta_440831e312(json_data,user_obj):
	B=user_obj;A=json_data;C=sparta_a2177a8814(A['widget_id'],B);D=C[_AC]
	if D:E=C[_M];A[_G]=E.plot_chart_id;F=sparta_023e09ac51(A,B);return{_A:1,_E:[A[_E]for A in F[_A8]]}
	return{_A:-1}
def sparta_6939326e72(json_data,user_obj):
	B='Error quantstats';A=json_data;C=A['service']
	if C=='quantstats':
		try:return qube_fc7c4fa321.sparta_4806e6511b(A,user_obj)
		except Exception as D:print(B);print(traceback.format_exc());print(B);return{_A:-1,_F:str(D)}
	return{_A:1}
def sparta_db696c0c3d(json_data,user_obj):
...
def sparta_723cfcd4b9(json_data,user_obj):
