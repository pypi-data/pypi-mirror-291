...
from audit_ninja.utils import model_delta, should_propagate_exceptions
...
def handle_signal_exception(signal):
    logger.exception(f"audit ninja had a {signal} exception.")
        ...
        with transaction.atomic(using=using):
            ...
            if not created:
                ...
                if not delta and getattr(
                    settings,
                    "DJANGO_AUDIT_NINJA_CRUD_EVENT_NO_CHANGED_FIELDS_SKIP",
                    False,
                ):
                    return False
...
def m2m_changed(sender, instance, action, reverse, model, pk_set, using, **kwargs):
    try:
        ...
        with transaction.atomic(using=using):
            ...
            if reverse:
                reverse_actions = {
                    "post_add": M2M_ADD_REV,
                    "post_remove": M2M_REMOVE_REV,
                    "post_clear": M2M_CLEAR_REV,
                }
                event_type = reverse_actions.get(action, M2M_CHANGE_REV)
                ...
                event_type = forward_actions.get(action, M2M_CHANGE)
...
# def handle_update_signal(sender, **kwargs):
#     print(f"Received update signal for {sender.__name__}")  # Debugging statement
#     updated_fields = kwargs.get('updated_fields')
#     instances = kwargs.get('instances')
#     count = kwargs.get('count')
#     print(f"Updated {count} instances of {sender.__name__}: {instances} with fields: {updated_fields}")


# def connect_signals():
if WATCH_MODEL_EVENTS:
    # from audit_ninja.crud_mixin import update_signal
    signals.post_save.connect(post_save, dispatch_uid="audit_ninja_signals_post_save")
    signals.pre_save.connect(pre_save, dispatch_uid="audit_ninja_signals_pre_save")
    signals.m2m_changed.connect(m2m_changed, dispatch_uid="audit_ninja_signals_m2m_changed")
    signals.post_delete.connect(post_delete, dispatch_uid="audit_ninja_signals_post_delete")
    # update_signal.connect(handle_update_signal, dispatch_uid="audit_ninja_signals_update")
    # print("Update signal connected")  # Debugging