...
class TestDownload(unittest.TestCase):
    ...
    def test_download(self, mock_open, mock_makedirs, mock_exists, mock_get, mock_head, mock_input, mock_getpass):
        ...
        try:
            # Mock responses
            mock_exists.side_effect = [False, False]  # For .netrc and .dodsrc
            ...
            mock_open.assert_any_call('downloaded_data\\file.nc4', 'wb')
    ...
    @patch('xcodex.Util.download.makedirs')
    @patch('builtins.open', new_callable=mock_open)
    def test_download_with_retries(self, mock_open, mock_makedirs, mock_exists, mock_get, mock_head, mock_input,
                                   mock_getpass):
        # Suppress stdout and stderr
        captured_output = io.StringIO()
        sys_stdout = sys.stdout
