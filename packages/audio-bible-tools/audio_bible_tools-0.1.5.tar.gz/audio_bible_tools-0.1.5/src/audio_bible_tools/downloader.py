...
class YouTubeDownloader:
    ...
    def process(self):
        ...
        for dir in os.listdir(self.project_dir_path):
            ...
            if os.path.isdir(dir_path):
              ...
              bookNumber = self.getBookNumber(dir_name)
    ...
    def getBookNumber(self, dirName):
        ...
        chapterNumber = re.search(r'(\d{1,2})', dirName)
        if chapterNumber:
            # format the number with leading zeros to ensure it is 2 digits
            return f"{int(chapterNumber.group(1)):02}"
        else:
            raise ValueError(f"No book number found in {dirName}")
