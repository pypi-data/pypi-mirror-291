...
class ProjectsRunner:
    ...
    async def run_project(self, basename, role, scheme, max_retries=10):
        ...
        result = await self.builderAgent.get_pipeline_plans(basename, tree)
        pipeline = result["pipeline"]

        if pipeline == "1":
            return await self.run_xcode_project(basename, role, scheme)
        elif pipeline == "2":
            return await self.run_normal_project(basename, role, scheme)
        else:
    ...
    async def run_normal_project(self, basename, role, max_retries=10):
        return []
