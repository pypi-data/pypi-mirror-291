...
class BugExplainer:
    ...
    def initial_setup(self, role):
        ...
        prompt = (
            f"You are a senior {role} working as a bug-fixing agent. Analyze the provided project context and current errors, and provide detailed steps to fix the project in a structured manner. "
            "Sometime even bugs are in multiple files but cause by one file, find the root cause to fix instead of fixing all files. "
            "You can only fix compile error by syntax error and clear way to solve by code, if the bugs is way harder"
