...
def test_health(client, mock_requestor):
    mock_requestor.get.return_value = ("OK", 200)
    response = client.health()
    assert response == "OK"
    mock_requestor.get.assert_called_once_with("http://localhost:8686/health")


def test_health_error(client, mock_requestor):
    mock_requestor.get.return_value = ("Error", 500)
    with pytest.raises(Exception):
        client.health()


def test_loaded_models(client, mock_requestor):
    mock_requestor.get.return_value = (["model1", "model2"], 200)
    response = client.loaded_models()
    assert response == ["model1", "model2"]
    mock_requestor.get.assert_called_once_with("http://localhost:8686/loaded_models")


def test_repository_models(client, mock_requestor):
    ...
    mock_requestor.get.assert_called_once_with(
        "http://localhost:8686/repository_models"
    )


def test_metrics(client, mock_requestor):
...
def test_load_clip_model(client, mock_requestor):
...
def test_load_sentence_transformer_model(client, mock_requestor):
    mock_requestor.post.return_value = ({"success": True}, 200)
...
def test_unload_model(client, mock_requestor):
    mock_requestor.post.return_value = ({"success": True}, 200)
...
def test_delete_model(client, mock_requestor):
    mock_requestor.post.return_value = ({"success": True}, 200)
...
def test_infer_text(client, mock_requestor):
...
def test_infer_image(client, mock_requestor):
...
def test_infer(client, mock_requestor):
