...
def _parse_args():  # noqa: WPS213
    ...
    remove_unit_node_command = sub_parser.add_parser(
        ...
        help='Remove dynamical node to already presented VirtualBox-based unit',
    )
    ...
    remove_unit_node_command.add_argument(
        '--node',
        required=True,
        help='Node of the VM (starts with "secondary").',
    )
    ...
    remove_unit_node_command.add_argument(
        ...
        action='store_true',
        default=True,
        help='Reboot all nodes into VM Group.',
    )
...
def main():
    ...
    try:  # noqa: WPS225
        ...
        if args.which == _COMMAND_UNIT_NODE_REMOVE:
            remove_node_to_unit(args.name, args.node, args.reboot)
