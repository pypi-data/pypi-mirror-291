...
class GATE(Exchange):
    ...
    def count_ledger_pnl(self, ledger) -> Ledger:
        if ledger["type"] == "trade" and ledger["info"]["type"] == "pnl":
            item = Ledger(
                ...
                ts=ledger["timestamp"],
                market_type="UFUTURES",
                market_id="%s_%s" % (ledger["timestamp"], ledger["info"]["trade_id"]),
                ledger_type=LedgerType.PNL.value,
    ...
    def count_ledger_fee(self, ledger) -> Ledger:
        ...
        if ledger["type"] == "fee" and ledger["info"]["type"] == "fund":
            item = Ledger(
                ...
                market_type="UFUTURES",
                market_id="%s_%s" % (ledger["timestamp"], ledger["info"]["trade_id"]),
                ledger_type=LedgerType.TRADE_PNL.value,
                amount=float(ledger["info"]["change"]),
                created_at=int(time.time() * 1000),
            )
            return item
