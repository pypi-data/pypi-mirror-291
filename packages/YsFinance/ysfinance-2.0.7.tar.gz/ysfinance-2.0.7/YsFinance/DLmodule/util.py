...
class StockDataset(Dataset):
    ...
    def __init__(self, features, label=None, num_feature_times=1, is_train=True, feature_stan=None):
        """
    ...
    def convert_to_3d_numpy(self, df, label=None):
    ...
    def __getitem__(self, idx):
        ...
        if self.is_train:
            ...
            stocks, features, labels = self.convert_to_3d_numpy(
                self.features.loc[start_date:end_date],label
            )
            return end_date, stocks, features, labels
        else:
            stocks, features = self.convert_to_3d_numpy(
                self.features.loc[start_date:end_date],label=None
            )
