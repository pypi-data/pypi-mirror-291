...
parser.add_argument \
    ("-o","--output",
     help="The output directory. The default is Prep",
     type=str,
     default="Prep",
     required=False )
...
parser.add_argument \
    ("-ts","--timestep",
     help="The time steps for each equilibrium steps: minimization (10000 steps), nvt (5ps restraint non-solvent), nvt (50ps restraint backbone only for protein system),  nvt (50ps without restraint), npt (200ps restraint non-solvent heavy atoms), npt (200ps without restraint). 5ns(5000ps) for complex restraint search. The unit is picosecond",
     type=int,
     nargs='+',
     default=[10000, 5, 50, 50, 200, 200, 5000],
     required=False )
...
if args.types == "lig":
    ...
    for lig_file in args.ligs:
        ...
        if os.path.exists(f"{path_name}/system.gro") or os.path.exists(f"{path_name}/system.top"):
            ...
            ## also save amber files just in case
...
"""%(lig_name))
    ...
    if args.protein == None:
        ...
        p06_RestraintSearch = BSE.Protocol.Production(
            runtime=args.timestep[6] * BSE.Units.Time.picosecond)
        ...
        for lig_file in args.ligs:
            ...
            new_sys = BSE.IO.readMolecules([f"{path_name}/system.gro",f"{path_name}/system.top"])

            ### just in case that the ligand does not rank in the first place
            n_residues = [mol.nResidues() for mol in new_sys]
            n_atoms = [mol.nAtoms() for mol in new_sys]
            ...
            if isinstance(lig_idx,int):
                ...
                decouple_lig = BSE.Align.decouple(new_sys[lig_idx])
                new_sys.updateMolecule(lig_idx, decouple_lig)
            else:
            ...
            BSE.FreeEnergy.RestraintSearch(new_sys, p06_RestraintSearch,name="06_RestraintSearch", engine='gromacs', work_dir=f"{path_name}/06_RestraintSearch")
