...
def test_create_env_empty(mocker):
    ...
    create_env(name=MOCK_CONDA_ENV_NAME, variables=MOCK_VARIABLES)  # no `pip` or `conda` dependencies
...
def test_get_envs(mocker):
    ...
    mock_get_envs_filtered = mocker.patch("covalent_cloud.swe_management.swe_manager._get_envs_filtered")
...
def test_dependency_is_missing_returns_true_when_exact_match():
    """Test the dependency is missing function."""
    dependencies = ["mock-dependency-1", "pip:"]
    assert dependency_is_missing("pip", dependencies, True) is True


def test_dependency_is_missing_returns_false_when_not_missing_with_exact():
    """Test the dependency is missing function."""
    dependencies = ["python=3.10", "pip", "pip:"]
    assert dependency_is_missing("pip", dependencies, True) is False
