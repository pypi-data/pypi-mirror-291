...
def _get_character_definition(
    ...
    *,
    ...
    # e.g. Chinese character takes 2 tokens while English takes 1/4 tokens
    if smart_truncation and truncation_length / 4 < len(definition) < truncation_length:
        tokens = get_tokenizer().tokenize(definition)
        desired_truncation_tokens = int(truncation_length / 4)
        ratio = min(desired_truncation_tokens / len(tokens), 1.0)
        truncation_length = ratio * len(definition)
        logging.info(f"Smart truncation from {len(definition)} to {truncation_length}")
