...
def rsquared(y_true: npt.ArrayLike,
...
def chisqr(y_true: npt.ArrayLike, y_pred: npt.ArrayLike):
    """
...
def chisqr_p(y_true: npt.ArrayLike,
             y_pred: npt.ArrayLike,
             nvars_fitted: int):
    """
    计算卡方检验的p值。
    ...
    return stats.chi2.sf(chisqr(y_true, y_pred),
                         len(y_true) - nvars_fitted)


# noinspection DuplicatedCode
def redchi(y_true: npt.ArrayLike,
           y_pred: npt.ArrayLike,
           nvars_fitted: int):
    """
...
class FittingStatistics(object):
    ...
    def __init__(self, y_true: npt.ArrayLike,
                 y_pred: npt.ArrayLike,
        ...
        # 残差数据。
        self.__residuals = self.__y_true - self.__y_pred

        # 残差平方和。
        self.__ess = np.sum(self.__residuals ** 2)
