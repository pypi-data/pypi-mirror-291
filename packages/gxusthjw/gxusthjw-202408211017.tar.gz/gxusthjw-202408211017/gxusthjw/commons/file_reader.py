...
# Version:          0.0.1
# Created:          2012/01/01
# Description:      Main Function: 定义“读取文件”相关的类和方法。
...
#       Jiwei Huang        0.0.1         2023/11/30     revise
...
# -----------------------------------------------------------------
# 导包 =============================================================
from typing import Optional, Mapping, Dict, List, Union
import numpy as np
import pandas as pd

from .file_object import FileInfo, get_file_encoding_chardet
...
__doc__ = """
Defining the classes and functions associated with `reading files`.
"""
__all__ = [
    'read_txt'
]
...
def read_txt(file: Union[str, FileInfo],
    ...
    :param encoding: 文件编码，如果文件编码未知，则利用chardet尝试解析，如果未能解析出文件的编码，则以“GBK”读取文件。
    :return: 读取到的值。
    """
    ...
    # 尝试解析文件的编码。
    if encoding is None:
        encoding = get_file_encoding_chardet(file)
    ...
    with open(file, mode='r', encoding=encoding) as f:
        ...
        if isinstance(res_type, str) and res_type.lower() == "dict":
            ...
            for col_index in index_col_name_dict.keys():
                res_list.append(data_dict[index_col_name_dict[col_index]])
            return DataCapsule(*res_list, col_names=index_col_name_dict)
