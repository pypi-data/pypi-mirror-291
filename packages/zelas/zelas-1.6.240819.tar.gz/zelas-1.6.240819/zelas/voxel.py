...
class voxel:
    ...
    def WatchVoxel_Background_Monomer(self, num_L, L_un, dimidiate=256):
        '''
        ...
        plotter.add_mesh(grid, scalars=points_local_un_value_other, show_edges=False, cmap='gray', show_scalar_bar=False)  # 添加体素网格，并设置颜色映射
        # 建立单体化显示
        # 颜色赋值
        colors = np.array(sns.color_palette('bright', n_colors=num_L))
        colors = np.sum(colors, axis=1)
        ...
        grid = pv.UnstructuredGrid({pv.CellType.HEXAHEDRON: cells_hex}, voxel_8ps_2D)  # 建立体素网格
        plotter.add_mesh(grid, scalars=color_other, show_edges=False, cmap='tab20', show_scalar_bar=False)  # 添加体素网格，并设置颜色映射
        plotter.show()  # 显示绘图
...
class voxel_ρθY:
    ...
    def watch_voxel(self, r=2.75, n=0, color=0, cmap='gray',edge=True,colorbar=True):
        ...
        plotter = pv.Plotter()  # 创建绘图窗口
        plotter.background_color = 'white'  # 设置背景颜色为白色
        plotter.add_mesh(grid, scalars=color, show_edges=edge, cmap=cmap, show_scalar_bar=colorbar)  # 添加体素网格，并设置颜色映射
        print('已显示体素')
        plotter.show()  # 显示绘图
        return
