...
def _compute_overall_assessment(
    ...
    """
    Compute the overall assessment based on the individual assessment results.

    Overall assessment = grounded && correct:
      - YES if both grounded and correct are YES.
      - NO if either grounded or correct is NO.
      - None if either grounded or correct is not available.
    """
    grounded = _answer_assessment_is_yes(
        assessment_results, builtin_judge.GROUNDEDNESS.assessment_name
    )
    correct = _answer_assessment_is_yes(
        assessment_results, builtin_judge.CORRECTNESS.assessment_name
    )
    if grounded is not None and correct is not None:
        ...
        elif not grounded and not correct:
            return entities.Rating.value(
...
def _answer_assessment_is_yes(
    assessment_results: Collection[entities.AssessmentResult], assessment_name: str
) -> Optional[bool]:
    """
    Check if an answer assessment result is YES.

    :param assessment_results: The collection of assessment results.
    :param assessment_name: The name of the assessment to check.
    :return: True if the assessment result is YES, False if the assessment result is NO,
             and None if the assessment result is not found or the rating is None or UNKNOWN.
    """
    for assessment_result in assessment_results:
        if isinstance(assessment_result, entities.AnswerAssessmentResult):
            if (
                assessment_result.assessment_name == assessment_name
                and assessment_result.rating.categorical_value
                in [entities.CategoricalRating.YES, entities.CategoricalRating.NO]
            ):
                return (
