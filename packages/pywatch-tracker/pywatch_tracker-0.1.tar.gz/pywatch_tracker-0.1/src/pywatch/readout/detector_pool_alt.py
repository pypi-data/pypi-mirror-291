...
class DetectorPool:
    ...
    def __run_process(self, event_count: int, callback=None, *args) -> (int, Optional[Exception]):
        ...
        async def func():
            ...
            async with self:
                result = await self.__async_run_process(event_count, callback, *args)

        asyncio.run(func())

        return result

    async def __async_run_process(self, hits: int, callback=None, *args) -> (int, Optional[Exception]):
        finished = False
        counted_hits = 0
        lock = asyncio.Lock()
            ...
            while not finished:
                ...
                if dt[-1].comp_time - self._first_coincidence_hit_time <= self.threshold:
                    ...
                    if len(self._event_data.keys()) > 1:
                        await callback(self._event_data, *args)
