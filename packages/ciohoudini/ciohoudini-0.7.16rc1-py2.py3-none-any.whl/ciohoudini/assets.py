...
def resolve_payload(node, **kwargs):
    ...
    """
    # Get the HIP folder and HIP file path
    ...
    # Filter out paths that are within the output folder or within the HIP folder (except for the HIP file itself or extra assets)
    filtered_paths = [
        path for path in current_assets
        if not is_within_output_folder(path, output_folder) and
           (path == hip_file_path or not is_within_hip_folder(path, hip_folder) or is_extra_asset(path, node))
    ]

    if len(current_assets) > len(filtered_paths):
        node.parm("output_excludes").set(0)

    # Ensure the HIP file is always included
    if hip_file_path not in filtered_paths:
        filtered_paths.append(hip_file_path)

    return {"upload_paths": filtered_paths}


def is_within_hip_folder(path, hip_folder):
    # Normalize the paths to handle different platforms and spaces
    normalized_path = os.path.normpath(str(path))  # Convert path to string
    normalized_hip_folder = os.path.normpath(str(hip_folder))  # Convert path to string

    # Check if the normalized path is within the normalized HIP folder
    return normalized_path.startswith(normalized_hip_folder)


def is_extra_asset(path, node):
    """
    Check if the path corresponds to an extra asset added by the user.
    """
    num = node.parm("extra_assets_list").eval()
    extra_assets = [
        os.path.normpath(os.path.expandvars(node.parm(f"extra_asset_{i}").eval()))
        for i in range(1, num + 1)
    ]
    normalized_path = os.path.normpath(path)
    return normalized_path in extra_assets
