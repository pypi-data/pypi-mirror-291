...
def get_nn_vv_ppp_from_full_nnvvppp_algo_name(nnvvppp_algo_name: str)-> str:
    """
    récupère uniquement le nnvvppp du nom complet
    """
    split_list = nnvvppp_algo_name.split("_")[0]
    if len(split_list)<2:
        raise ValueError (f"Le chemin spécifié '{nnvvppp_algo_name}' n'est pas un nom valide, avoir un format nnvvppp_algoname.")
...
def get_algorithm_results_full_path(root_directory: str, dataset_name: str, test_case_number: int, nnvvppp_algoname: str) -> str:
    """
    ...
    return os.path.join(root_directory,
        ...
        get_algorithm_results_folder(root_directory, dataset_name, get_test_case_number_str(test_case_number), nnvvppp_algoname)
    )
...
def get_algorithm_results_folder(root_directory: str, dataset_name: str, test_case_number: int, nnvvpp_algoname: str) -> Optional[str]:
    """
    ...
    """
    test_case_folder_name = get_test_case_folder(root_directory, dataset_name, get_test_case_number_str(test_case_number))
