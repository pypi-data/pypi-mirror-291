...
class Application:
	...
	# 멤버 변수 목록.
	#--------------------------------------------------------------------------------
	__preparedData : PreparedData
	__internalData : InternalData
	__logger : Logger
	...
	#--------------------------------------------------------------------------------
	@property
	def PreparedData(self) -> PreparedData:
	...
	#--------------------------------------------------------------------------------
	@property
	def InternalData(self) -> InternalData:
	...
	# 생성됨.
	#--------------------------------------------------------------------------------
	def __init__(self, preparedDataDict : dict) -> None:
		self.__preparedData = PreparedData(preparedDataDict)
		self.__internalData = InternalData()
		self.__logger = Logger(self.__preparedData.Name)
	...
	def IsBuild(self) -> bool:
		return self.__internalData.IsBuild()
	...
	def Log(self, message : str, logType : LogType) -> None:
		...
		self.__logger.Print(f"[{timestamp}][{logName}] {message}", logName)
	...
	def LogException(exception : Exception, useTraceback : bool = True, useExit : bool = True) -> None:
		if useTraceback:
			...
			while tb:
				...
				Application.Log(f"Exception in {filename}, line {lineno}, in {funcname}", LogType.EXCEPTION)
				Application.Log(f"\t{line}", LogType.EXCEPTION)
				tb = tb.tb_next
		else:
			Application.Log(exception, LogType.EXCEPTION)
	...
	def IsDebug(self) -> bool:
		return self.__internalData.IsDebug()
	...
	def GetExecuteFileName(self) -> str:
		return self.__internalData.GetExecuteFileName()
	...
	def GetRootPath(self) -> str:
		return self.__internalData.GetRootPath()
	...
	def GetSourcePath(self) -> str:
		return self.__internalData.GetSourcePath()
	...
	def GetResourcePath(self) -> str:
		return self.__internalData.GetResourcePath()
	...
	def GetWorkspacePath(self) -> str:
		return self.__internalData.GetWorkspacePath()
	...
	def GetRootPathWithRelativePath(self, relativePath : str) -> str:
		rootPath = self.__internalData.GetRootPath()
		if not relativePath:
			return rootPath
	...
	#--------------------------------------------------------------------------------
	# 메타파일이 존재하는 경로에 상대경로를 입력하여 절대경로를 획득.
	#--------------------------------------------------------------------------------
	def GetMetaPathWithRelativePath(self, relativePath : str) -> str:
		metaPath = self.__internalData.GetMetaPath()
		if not relativePath:
			return metaPath
		relativePath = relativePath.replace(BACKSLASH, SLASH)
	...
	def GetSourcePathWithRelativePath(self, relativePath : str) -> str:
		sourcePath = self.__internalData.GetSourcePath()
		if not relativePath:
			return sourcePath
		relativePath = relativePath.replace(BACKSLASH, SLASH)
	...
	def GetResourcePathWithRelativePath(self, relativePath : str) -> str:
		resourcePath = self.__internalData.GetResourcePath()
		if not relativePath:
			return resourcePath
		relativePath = relativePath.replace(BACKSLASH, SLASH)
	...
	def GetWorkspacePathWithRelativePath(self, relativePath : str) -> str:
		workspacePath = self.__internalData.GetWorkspacePath()
		if not relativePath:
