...
class Launcher:
	...
	def Launch(self) -> int:
		...
		#--------------------------------------------------------------------------------
		# 프리페어 데이터 파일 로드.
		def PreparedDataConstrcutor(loader, node):
			values = loader.construct_mapping(node, deep=True)
			return PreparedData(**values)

		def ApplicationExecuteTypeConstrcutor(loader, node):
			value = loader.construct_sequence(node, deep=True)[0]
			return ApplicationExecuteType(value)
		...
		with open(prepareFilePath, READ, encoding = UTF8) as file:
			...
			yaml.add_constructor("!python/object:dduk.application.prepareddata.PreparedData", PreparedDataConstrcutor)
			yaml.add_constructor("!python/object/apply:dduk.application.applicationexecutetype.ApplicationExecuteType", ApplicationExecuteTypeConstrcutor)
			preparedDataDict : dict = yaml.load(yamlString, Loader = yaml.FullLoader)

		#--------------------------------------------------------------------------------
		# 애플리케이션 설정.
		self.__application = Application(preparedDataDict)
		self.__application.InternalData.SetBuild(isBuild)

		#--------------------------------------------------------------------------------
		# 심볼 설정.
		# 준비 데이터로부터 심볼목록을 불러와 설정.
		Application.Log("__PREPARED__")
