...
# 전역 상수 목록.
#--------------------------------------------------------------------------------
PREPAREDATANAME : str = "data"
PREPAREDATAFILENAME : str = f"__{PREPAREDATANAME}__.py"
BACKSLASH : str = "\\"
SLASH : str = "/"
COLON : str = "."
PYEXTENSION : str = ".py"
...
class Preparer:
	...
	def Prepare(self, applicationExecuteType : ApplicationExecuteType) -> None:
		...
		# 파일 경로 생성.
		prepareFilePath : str = f"{rootPath}/src/{PREPAREDATAFILENAME}"
		...
		if settings:
			...
			if prepareData.Type == ApplicationExecuteType.SOURCE:
				if ARGUMENTS in settings:
					...
					prepareData.Arguments.extend(argumentsString)

		# 코드 생성.
		pysonGenerator = PYSONGenerator()
		content = pysonGenerator.Generate(PREPAREDATANAME, prepareData)
