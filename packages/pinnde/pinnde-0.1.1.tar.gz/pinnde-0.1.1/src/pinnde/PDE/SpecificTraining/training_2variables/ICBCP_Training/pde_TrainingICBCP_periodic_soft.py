...
@tf.function
def trainStep(pdes, inits, model, t_order, eqnparam, extra_ders):
    print("AAAAAAAA")
    """
    ...
    mean squared error of loss along pdes points and inital values.
...
def PINNtrain(pde_points, init_points, t_order, setup_boundaries, epochs, eqn, N_pde, N_iv, model, extra_ders):
    """
    Main function called by PINNtrainSelect_tx when solving equation in tx with periodic boundaries with soft constraint.
    ...
    for i in range(epochs):
        ...
        for (pdes, inits) in ds:
            ...
            PDEloss, IVloss, grads = trainStep(pdes, inits, model, t_order, eqn, extra_ders)
