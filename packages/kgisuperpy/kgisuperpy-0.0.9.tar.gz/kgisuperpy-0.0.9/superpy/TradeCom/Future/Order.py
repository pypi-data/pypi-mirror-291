...
class Order:
    ...
    def FutOrder(self, type: ORDER_TYPE, market: MARKET_FLAG, 
        ...
        if type == ORDER_TYPE.OT_NEW:
            res = self.tradecom.Order(ORDER_TYPE.OT_NEW, market, RequestId, brokerId, account, '', symbolId, BS, pricefl, PRICE, TIF, QTY, pf, OFFICE_FLAG.OF_AS400)
        else:
            res = self.tradecom.Order(type, market, RequestId, brokerId, account, '', symbolId, BS, pricefl, PRICE, TIF, QTY, pf, OFFICE_FLAG.OF_AS400, webid, cnt, orderno)
        if res != 0:
