...
from openpyweb import App
from openpyweb.util.Exception import TemplateSyntaxError, TemplateError, TemplateSyntaxError
from openpyweb.Editor.Resolver import *
...
class _Call(_Node):
    ...
    def render(self, context):
        ...
        ob_dir = [os.path.join(str(os.path.dirname(__file__).replace('Editor', '')), "Functions"), os.path.join(str(host), "model")]
        ...
        for kind, value in self.args:
            if kind == 'name':
                value = value
            _value = self._call_each(str(value))
            resolved_args.append(_value)
        ...
        for key, (kind, value) in items:
            ...
            _value = self._call_each(str(value))
            try:
                _x_value = eval(_value)
            except Exception as err:
                _x_value = _value
            resolved_kwargs[key] = _x_value

        path = [os.path.join(str(ob_dir[0]), "%s.py"% str(self.callable)),  os.path.join(str(ob_dir[1]),"%s.py"% str(self.callable) )]
    ...
    def _call_each(self, context):
        ...
        if  VAR_TOKEN_START in context:
            ...
            for oparator_k in self.it.split(_op):
                if 'it.' in oparator_k:
