...
class Pattern:
    ...
    def search(self, string: _String, pos: _Integer = 0) -> dsl.Expression:
    ...
    def findall(self, string: _String) -> dsl.Expression:
...
def compile(regex: _String) -> dsl.Expression:
...
class Match(dsl.Producer):
    def __init__(self, graph:dsl.Graph, regex: Pattern | _String, string: _String, pos: _Integer = 0, _how: str = "match"):
        ...
        self.pos = pos
        self._how = _how
        self._fullmatch = std.Vars(1)
        self._start = std.Vars(1)
        if self._how == "match":
    ...
    def group(self, index: _Integer | _String = 0) -> dsl.Expression:
        if index == 0:
    ...
    def span(self) -> dsl.Expression:
        return self.start(), self.end()

    def __getitem__(self, index: _Integer | _String) -> dsl.Expression:
...
def search(regex: _String, string: _String) -> dsl.Expression:
...
def findall(regex: _String, string: _String) -> dsl.Expression:
    ix, match = std.Vars(2)
