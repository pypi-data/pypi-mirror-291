...
class NewSnapshotError(SnapshotError):
    def __init__(self, file_path: Path, code: str, task: Task):
        header = "No snapshot exists for the following block:"
        footer = "[yellow]To save the current result as a snapshot, run pytest with [bold]--snapshot-update[/bold][/yellow]"
        super().__init__(file_path, code, task, header, footer)
        ...
        for block in query_logger.blocks:
            ...
            try:
                ...
                if "doesn't exist" in lines[0]:
                    raise NewSnapshotError(file_path, code, block["task"]) from None
