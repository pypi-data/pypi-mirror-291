...
def authenticate(api_key = None):
	...
	try:
		...
		if api_key == None:
			print("Warning: An OpenAI API key is required to use AI model functions. Please provide the key by calling `authenticate(openai_api_key)` or ensure it is specified in the `.env` file as 'OPENAI_API_KEY'. If you set it in the `.env` file, call `authenticate()` or reload the package to proceed.")
			return
		else:
...
#         with output:
#             print("Starting 15 second recording")
#             print("Recording...")
#             audio_data = sd.rec(int(seconds * fs), samplerate=fs, channels=1, dtype='float64')
#             sd.wait()  # Wait until recording is finished
#             wav.write('your_recording.wav', fs, (audio_data * 32767).astype(np.int16))  # Save as WAV file with proper format
#             print("Recording complete! File name: your_recording.wav")
            
#             display(Audio('your_recording.wav'))  # Display the audio widget

#     #button to record
#     button = widgets.Button(description="Record Audio")
