...
def test_mock_schema() -> None:
    ...
    invalid = FrozenTools.freeze(
        {"foo": JsonValueFactory.from_unfolded("text")}
    )
    assert (
        MOCK_SCHEMA.validate(valid).alt(Unsafe.raise_exception).to_union()
        is None
    )
    assert (
        MOCK_SCHEMA.validate(invalid)
        .map(
            lambda _: Unsafe.raise_exception(ValueError("Should fail"))  # type: ignore[misc]
        )
        .to_union()
    )
