...
from QuantDataCollector.Global.settings import MYSQL_STOCK_BAISC_DATA_TABLE_NAME,\
                                    MYSQL_STOCK_DAILY_DATA_TABLE_NAME
...
class PostgresqlCacheUtil(AbstractCacheUtil):
    ...
    def has_stock_cache(self, stock_code, date):
        ...
        res, count = self.db.data_num(MYSQL_STOCK_DAILY_DATA_TABLE_NAME,filter_condition)
        return count > 0
    ...
    def get_stock_data(self, stock_code, date):
        ...
        """
        filter_condition = "code = '" + stock_code + "' AND date = '" + date + "'"
        succ, res =  self.db.query(MYSQL_STOCK_DAILY_DATA_TABLE_NAME, filter_condition = filter_condition)
        retval = {}
        if succ:
            column_names = self.db.get_table_columns(MYSQL_STOCK_DAILY_DATA_TABLE_NAME)
    ...
    def save_stock_data(self, stock_code, date, data):
        ...
        res, msg = self.db.insert(MYSQL_STOCK_DAILY_DATA_TABLE_NAME,data)
        if(res):
            return True
        else:
            return False
    ...
    def delete_stock_data(self, stock_code, date):
        ...
        res, msg = self.db.delete(MYSQL_STOCK_DAILY_DATA_TABLE_NAME,filter_condition)
        if res:
            return True
        else:
            return False
