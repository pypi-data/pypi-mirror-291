...
def write_ramps_to_task(task, num_ramps: int, arm_id: str, num_line2: int = 4):
    ...
    return expected_obs_time_list, bias_value, num_line1, num_read, num_line2
...
def make_ramp_data(dataset: RawRampHeaders, bias: float):
    ...
    if dataset.frame_in_ramp("") not in range(
        dataset.num_line1 + 1, dataset.num_line1 + dataset.num_read + 1
    ):
        # Bias
        value = bias
...
@pytest.mark.parametrize(
    "num_line2", [pytest.param(0, id="no_second_line"), pytest.param(4, id="second_line_present")]
)
def test_linearity_correction(
    linearity_correction_task, link_constants_db, arm_id, mocker, num_line2
):
    """
    ...
    expected_time_obs_list, bias_value, num_line1, num_read, num_line2 = write_ramps_to_task(
        task, num_ramps=num_ramps, arm_id=arm_id, num_line2=num_line2
    )
    ...
    for ramp_num, time_obs in enumerate(expected_time_obs_list, start=1):
        ...
        assert len(files) == 1
        # See `make_ramp_data` for where this comes from - we don't include num_line2 because we only care about the last read frame
        expected_value = ramp_num * 100 + (num_line1 + num_read) * 10 - bias_value
