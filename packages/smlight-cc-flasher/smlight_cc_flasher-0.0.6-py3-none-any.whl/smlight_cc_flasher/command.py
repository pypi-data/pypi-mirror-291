...
class Bootloader:
    ...
    def set_mode(self, mode: str, host: str | None = None) -> None:
        ...
        if mode == "sonoff":
            self._sonoff_usb = True
        elif mode == "network":
            self._smlight_net = True

        if self._sonoff_usb or self._smlight_net:
            self._generic = False
        return
    ...
    async def invoke_sonoff(self):
        _LOGGER.info("Activating Sonoff BSL")
        # ITead Sonoff Zigbee 3.0 USB Dongle. This dongle has a
