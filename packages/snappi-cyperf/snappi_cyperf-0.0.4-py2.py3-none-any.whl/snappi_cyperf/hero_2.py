...
for eni_index2, eni2 in enumerate(
    range(p.ENI_START, p.ENI_START + p.ENI_COUNT * p.ENI_STEP, p.ENI_STEP)
):  # Per ENI
    remote_mac_a_eni = ip_int.MAC_R_START + eni_index2 * ip_int.MAC_STEP_ENI
    gateway_mac = str(maca(remote_mac_a_eni))

    c_eth = c_device.ethernets.add()
    c_eth.name = "NETMAC%d" % eni2
    c_eth.mac = str(maca(remote_mac_a_eni))

    eni_mac = str(maca(ip_int.MAC_L_START + eni_index2 * ip_int.MAC_STEP_ENI))

    s_eth = s_device.ethernets.add()
    s_eth.name = "ENIMAC%d" % eni2
    s_eth.mac = eni_mac
