...
def get_connection(server, port, http_redirect=False):
    """
    getconnection perform connection to Cyperf and returns session Url.
    """
...
class Connection(object):
    ...
    def _request(self, method, url="", data="", option=None, params=None, headers=None):
        """ """
        if method == "POST":
            reply = self.http_post(url, data, params, headers)
            if not reply.ok:
            ...
            if option:
                return reply
            response = new_obj_path.split("/")[-1]
            return response
        elif method == "GET":
            response = self.http_get(url=url, option=option)
            return response
        elif method == "PATCH":
            reply = self.http_patch(url, data, params, headers)
            if not reply.ok:
                raise Exception(reply.text)
            return reply
        elif method == "DELETE":
            reply = self.http_delete(url, data, params, headers)
            if not reply.ok:
                raise Exception(reply.text)
            return reply
    ...
    def http_request(self, method, url="", data="", params=None, headers=None):
        ...
        result = self._get_http_session().request(
            method,
            abs_url,
            data=str(data),
            params=params,
    ...
    def http_get(
        self, url="", data="", option=None, params=None, headers=None, error_codes=None
    ):
        """
        ...
        if reply.status_code in error_codes:
            raise Exception(
                "Error on executing GET request on url %s: %s" % (url, reply.text)
            )
        if option:
            return reply.json()
        else:
    ...
    def http_options(
        self, url="", data="", params=None, headers=None, error_codes=None
    ):
        """
        ...
        if reply.status_code in error_codes:
            raise Exception(
                "Error on executing Options request on url %s: %s" % (url, reply.text)
            )
        return _format_response(reply.json(), url)
...
def _format_response(value, _url_=None):
    ...
    if isinstance(value, dict):
        if _url_ and bool(re.search(r"[\w.-]+/cyperf/stats/[\w().-]+/values", _url_)):
...
class WebList(list):
    ...
    def __init__(self, entries=None, _url_=None):
        ...
        if url and filter_syntax in url:
            url = url.split(filter_syntax)[
                0
            ]  # get everything on the left of the filter, removing the query param
        for item in entries:
            item_url = None
            if "objectID" in item:
