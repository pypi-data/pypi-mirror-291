...
class Agent:
    ...
    async def get_system_message(self, messages: list[Message]) -> str:
        ...
        for capability in self.capabilities:
            system = await capability.modify_prompt(system, messages)
        return system
    ...
    async def step(self, messages: list[Message]) -> list[AIMessage]:
        ...
        system = await self.get_system_message(messages)
