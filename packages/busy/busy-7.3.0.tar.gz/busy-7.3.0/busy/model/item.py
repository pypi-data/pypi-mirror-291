...
def elapsed_time_text(elapsed):
    if elapsed:
        hours = int(elapsed/60)
        minutes = elapsed % 60
        return f"{hours}h{minutes:>3}m" if hours else f"{minutes:>3}m"
    else:
        return ''
...
class Item:
    ...
    @property
    def elapsed_time_text(self):
        return elapsed_time_text(self.elapsed_time)
