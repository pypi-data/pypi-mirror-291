...
def _train_dl(nirs_train, labels_train, clf, batch_size, lr, max_epochs,
    ...
    for k, out_idx in enumerate(out_split):
        ...
        if os.path.isfile(f'{output_folder}/model_k{k}.pt'):
            ...
            clf, results = _train_dl(nirs_train, labels_train,
                                     clf, best_hps[0], best_hps[1], max_epochs,
...
def _proxy_selec(nirs_train, targets_train, groups_train, enc_class, dec_class,
                 batch_sizes, lrs, max_epochs, min_epochs, random_state,
                 output_folder):
    """
    Train and fine-tune encoder-decoder.
    """
    # Set device
...
def deep_transfer_learn(nirs, labels, groups, enc_class, dec_class,
    ...
    min_epochs : integer
        Minimum number of training epochs before early stopping. Defaults to ``1``.
...
def train_final(nirs, labels, model_class, batch_size, lr, n_epochs,
    ...
    if os.path.isfile(f'{output_folder}/final.pt'):
        print('Checkpoint found, loading it...', end=' ')
        clf.load_state_dict(torch.load(f'{output_folder}/final.pt'))
        print('Done!')
    else:
        # Min-max scaling
        if normalize is not None:
        ...
        clf.eval()
        clf.cpu()
        torch.save(clf.state_dict(), f'{output_folder}/final.pt')
