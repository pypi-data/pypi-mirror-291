...
from ..utils.another import checkFile


class Update:
    def upload(self, filename: str, title: str = None, artist: str = None, lyrics: str = None, genreId: int = None, removeFromSearchResults: bool = None, groupId: int = None) -> Union[Track, dict]:
        """
        ...
        :param lyrics: текст аудиотрека. (str, необязательно, временно не работает)
        :param genreId: жанр аудиотрека (в виде идентификатора). (int, необязательно, временно не работает)
        :param removeFromSearchResults: флаг, указывающий, будет ли аудиотрек скрыт из поисковой выдачи. (bool, необязательно, временно не работает)
        :param groupId: идентификатор группы, в музыку которой необходимо загрузить аудиотрек (int, необязательно, временно не работает)
        :return: загруженный аудиотрек в виде объекта модели `Track` или словаря.
        """
        ...
        fileSizeInMB = fileSizeInBytes / (1024 * 1024)  # Преобразование размера файла в мегабайты

        if fileSizeInMB > 200:  # Проверка, меньше ли размер файла 200 MB
            self._raiseError("MP3FileTooBig")

        uploadUrl = self._VKReq("getUploadServer").get("upload_url")

        with open(filename, "rb") as file:
            fileContent = file.read()

        uploadingFileResponse = self._client.sendReq(uploadUrl, files={"file": (filename, fileContent, "audio/mpeg")}, method="POST")

        server = uploadingFileResponse.get("server")
        audio = uploadingFileResponse.get("audio")
        hash = uploadingFileResponse.get("hash")

        response = self._VKReq("save", {"server": server, "audio": audio, "hash": hash, "title": title, "artist": artist})

        return self._finalizeResponse(response, Track) if not response.get("error") else response


    def add(self, ownerIds: Union[int, List[int]], trackIds: Union[int, List[int]], playlistId: Optional[int] = None, groupId: Optional[int] = None) -> Tuple[Union[bool, dict]]:
        """
        ...
        for ownerId, trackId in zip(ownerIds, trackIds):
            response = self._VKReq(method, {**({"owner_id": ownerId, "audio_id": trackId, "group_id": groupId} if not playlistId else {"owner_id": groupId, "audio_ids": f"{ownerId}_{trackId}"}), "playlist_id": playlistId})
            results.append(bool(response) if not (isinstance(response, dict) and response.get("error")) else self._raiseError("playlistNotFound"))

        return tuple(results)


    def remove(self, ownerIds: Union[int, List[int]], trackIds: Union[int, List[int]], playlistId: Optional[int] = None, groupId: Optional[int] = None, reValidateIds: bool = True) -> Tuple[Union[bool, dict]]:
        """
        ...
        :return: `True`, если аудиотрек(и) успешно удалён, `False` в противном случае.
        """
        ...
        if reValidateIds:
            if playlistId:
                playlist = self.getPlaylist(playlistId, groupId, includeTracks=True)
                if isinstance(playlist, dict) and playlist.get("error"):
                    return playlist

                if isinstance(playlist, dict):
                    existTracks = playlist.get("tracks")

                elif isinstance(playlist, Playlist):
                    existTracks = playlist.tracks if hasattr(playlist, "tracks") else None

            else:
                existTracks = self.getTracks(groupId)
            ...
            for index, (ownerId, trackId) in enumerate(zip(ownerIds, trackIds)):
                ...
                track = self.get(ownerId, trackId)
                ...
                if isinstance(track, dict):
                    ...
                    trackArtists = track.get("artists")
                    if not trackArtists:
                        trackArtists = track.get("artist")
                ...
                for existTrack in existTracks:
                    if isinstance(existTrack, dict):
                        existTrackTitle = existTrack.get("title")
                        existTrackArtists = existTrack.get("artists")
                        if not existTrackArtists:
                            existTrackArtists = existTrack.get("artist")

                        existTrackOwnerId = existTrack.get("ownerId")
                        existTrackId = existTrack.get("trackId")

                    elif isinstance(existTrack, Track):
        ...
        if playlistId:
            ...
            def removeTrackFromPlaylist(id: str) -> bool:
                response = self._VKReq("removeFromPlaylist", {"audio_ids": id, "owner_id": groupId, "playlist_id": playlistId})
                return bool(response) if not (isinstance(response, dict) and response.get("error")) else response

            tasks = [removeTrackFromPlaylist(f"{ownerId}_{trackId}") for ownerId, trackId in zip(ownerIds, trackIds)]

        else:
            def removeTrack(ownerId: int, trackId: int) -> bool:
                response = self._VKReq("delete", {"owner_id": ownerId, "audio_id": trackId, "group_id": groupId})
                return bool(response) if not (isinstance(response, dict) and response.get("error")) else response

            tasks = [removeTrack(ownerId, trackId) for ownerId, trackId in zip(ownerIds, trackIds)]

        return tuple(tasks)


    def edit(self, ownerId: int, trackId: int, title: Union[str, int] = None, artist: Union[str, int] = None, lyrics: Union[str, int] = None, genreId: int = None, removeFromSearchResults: bool = None) -> Union[bool, dict]:
        """
        Изменяет информацию об аудиотреке.

        Пример использования:\n
        result = client.edit(ownerId="yourOwnerId", trackId="yourTrackId", title="zapreti", artist="prombl", "lyrics"="", "genreId"=3, removeFromSearchResults=True)\n
        print(result)
        ...
        response = self._VKReq("edit", {"owner_id": ownerId, "audio_id": trackId,
    ...
    def restore(self, ownerId: int, trackId: int) -> Union[bool, dict]:
        """
        ...
        response = self._VKReq("restore", {"owner_id": ownerId, "audio_id": trackId})

        return bool(response) if not (isinstance(response, dict) and response.get("error")) else response


    def createPlaylist(self, title: Union[str, int], description: Union[str, int] = None, photo: str = None, groupId: int = None, chatId: int = None) -> Union[int, dict, None]:
        """
        ...
        playlist = self._VKReq(method, {"title": title, "description": description, "owner_id": groupId})
        ...
        if playlistId:
            ...
            """if photo:
                self._editPlaylistPhoto(playlistId, photo, groupId)"""

        return playlist


    def addPlaylist(self, playlistId: int, ownerId: int = None, groupId: int = None) -> Union[int, None, dict]:
        """
        ...
        response = self._VKReq("followPlaylist", {"owner_id": ownerId, "playlist_id": playlistId, **({"group_id": groupId} if groupId else {})})
        return response.get("id") if not (isinstance(response, dict) and response.get("error")) else response


    def removePlaylist(self, playlistId: int, groupId: int = None) -> Union[bool, dict]:
        """
        ...
        response = self._VKReq("deletePlaylist", {"owner_id": groupId, "playlist_id": playlistId})
        return bool(response) if not (isinstance(response, dict) and response.get("error")) else (response if response.get("error").get("code") != 17 else self._raiseError("playlistNotFound"))


    def _editPlaylistPhoto(self, playlistId: int, photo: str = None, groupId: int = None) -> Union[bool, dict]:
        if photo is None:
            return False
    ...
    def editPlaylist(self, playlistId: int, title: Union[str, int] = None, description: Union[str, int] = None, photo: str = None, groupId: int = None) -> Union[bool, dict]:
        """
        ...
        if not any(("title" in params, "description" in params, photo is not None)):
            ...
            if any(("title" in params, "description" in params)):
                response = self._VKReq("editPlaylist", params)
                editInfoStatus = bool(response) if not (isinstance(response, dict) and response.get("error")) else response

            else:
                editInfoStatus = None

        if any((editInfoStatus is True, editInfoStatus is None)) and photo is not None:
            editPhotoStatus = self._editPlaylistPhoto(playlistId, photo, groupId)
    ...
    def copyPlaylist(self, playlistId: int, ownerId: int = None, groupId: int = None, skipTitle: bool = False, skipDescription: bool = False, skipPhoto: bool = False) -> Union[int, None, dict]:
        """
        Копирует плейлист, принадлежий пользователю или группе в музыку пользователя или группы.

        Пример использования:\n
        result = client.copyPlaylist(playlistId=1, ownerId=-215973356, groupId: "yourGroupId", skipTitle=True, skipDescription=True, skipPhoto=True)\n
        print(result)
        ...
        :param skipTitle: флаг, указывающий, необходимо ли не копировать название плейлиста (если `True`, будут использованы текущие дата и время). (bool, по умолчанию `False`)
        :param skipDescription: флаг, указывающий, необходимо ли не копировать описание плейлиста. (bool, по умолчанию `False`)
        :param skipPhoto: флаг, указывающий, необходимо ли не копировать фото плейлиста. (bool, по умолчанию `False`)
        :return: идентификатор плейлиста, если плейлист успешно скопирован, `None` в противном случае.
        """

        if not ownerId:
            ownerId = self.getSelf().get("id")

        playlist = self.getPlaylist(playlistId, ownerId, True)
        ...
        if isinstance(playlist, dict):
            ...
            photo = playlist.photo if hasattr(playlist, "photo") else None

        if skipTitle:
            title = datetime.utcnow().replace(tzinfo=pytz.utc).astimezone(moscowTz).strftime("%d.%m.%Y / %H:%M:%S")
        ...
        newPlaylistId = self.createPlaylist(title, description, photo, groupId)
        if isinstance(newPlaylistId, dict) and newPlaylistId.get("error"):
            return newPlaylistId

        tracks = (playlist.tracks if hasattr(playlist, "tracks") else None) if isinstance(playlist, Playlist) else playlist.get("tracks") if isinstance(playlist, dict) else None
        if not tracks:
            return newPlaylistId
        ...
        for track in tracks[::-1]:
            ownerId, trackId = (track.ownerId, track.trackId) if isinstance(track, Track) else (track.get("ownerId"), track.get("trackId"))
            ownerIds.append(ownerId)
            trackIds.append(trackId)
        self.add(ownerIds, trackIds, newPlaylistId, groupId)

        return newPlaylistId


    def reorder(self, trackId: int, beforeTrackId: int = None, afterTrackId: int = None) -> Union[bool, dict]:
        """
        ...
        response = self._VKReq("reorder", {"audio_id": trackId, **({"before": beforeTrackId} if beforeTrackId else {"after": afterTrackId})})
        return bool(response) if not (isinstance(response, dict) and response.get("error")) else response


    def followArtist(self, artistId: int = None) -> Union[bool, dict]:
        """
        ...
        response = self._VKReq("followArtist", {"artist_id": artistId})
        return bool(response) if not (isinstance(response, dict) and response.get("error")) else response


    def unfollowArtist(self, artistId: int = None) -> Union[bool, dict]:
        """
        ...
        response = self._VKReq("unfollowArtist", {"artist_id": artistId})
        return bool(response) if not (isinstance(response, dict) and response.get("error")) else response


    def followCurator(self, curatorId: int = None) -> Union[bool, dict]:
        """
        ...
        response = self._VKReq("followCurator", {"curator_id": curatorId})
        return bool(response) if not (isinstance(response, dict) and response.get("error")) else response


    def unfollowCurator(self, curatorId: int = None) -> Union[bool, dict]:
        """
        ...
        response = self._VKReq("unfollowCurator", {"curator_id": curatorId})
        return bool(response) if not (isinstance(response, dict) and response.get("error")) else response


    def followOwner(self, ownerId: int = None) -> Union[bool, dict]:
        """
        ...
        response = self._VKReq("followOwner", {"owner_id": ownerId})
        return bool(response) if not (isinstance(response, dict) and response.get("error")) else response


    def unfollowOwner(self, ownerId: int = None) -> Union[bool, dict]:
        """
    ...
    def setBroadcast(self, ownerId: int = None, trackId: int = None, groupIds: Union[List[str], str] = None) -> Union[bool, dict]:
        """
        ...
        response = self._VKReq("setBroadcast", {**({"audio": f"{ownerId}_{trackId}"} if ownerId and trackId else {}), **({"target_ids": groupIds} if groupIds else {})})
        return True if not (isinstance(response, dict) and response.get("error")) else response