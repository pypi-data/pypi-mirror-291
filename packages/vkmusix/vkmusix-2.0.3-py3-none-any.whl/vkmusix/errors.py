...
errorsDict = {
    ...
    "usingSyncClientInAsyncContext": ("Вы не можете использовать синхронный клиент в асинхронном контексте. Если вы хотите использовать асинхронность, импортируйте `VKClientAsync`.", "You can't use the synchronous client in an asynchronous context. If you want to use asynchronous functionality, import `VKClientAsync`", True),
    "usingAsyncClientInSyncContext": ("Вы не можете использовать асинхронный клиент в синхронном контексте. Если вы не хотите использовать асинхронность, импортируйте `VKClient`.", "You can't use the asynchronous client in an synchronous context. If you don't want to use asynchronous functionality, import `VKClient`", True),

    "sessionClosed": ("Текущая сессия закрыта. Для отправки новых запросов потребуется создать новый объект класса `Client` или `ClientAsync`.", "The current session is closed. To send new requests, you need to create a new instance of the `Client` or `ClientAsync` class.", True),
    ...
    "trackRestorationTimeEnded": ("Время, в течение которого восстановление трека было возможно, закончилось.", "The time during which track restoration was possible has ended."),
    "accessDenied": ("Недостаточно прав доступа для совершения этого действия.", "Insufficient permissions to perform this action."),
    "accessDeniedWithoutCookie": ("Для совершения этого действия необходимо войти в аккаунт (Указать cookie при создании `Client` или `ClientAsync`).", "To perform this action, you need to log in (Specify cookies when creating `Client` or `ClientAsync`)."),
    ...
    "trackReorderNeedsBeforeOrAfterArgument": ("Для изменения порядка трека необходимо указать `beforeTrackId` или и `afterTrackId`.", "To change the order of the track, you need to specify either `beforeTrackId` or `afterTrackId`."),
    "trackReorderNeedsOnlyBeforOrAfterNotBoth": ("Для изменения порядка трека необоходимо указать `beforeTrackId` или `afterTrackId`, но не оба типа.", "To change the order of a track, you need to specify either `beforeTrackId`, or `afterTrackId`, but not both types."),
