...
class Client(Utils, Search, Get, Update):
    ...
    def __init__(self, VKToken: str = None, RuCaptchaKey: str = None, errorsLanguage: str = None, login: str = None, password: str = None, cookieFilename: str = None) -> None:
        ...
        if sys.version_info < (3, 7):
            ruError = "Внимание: некоторые методы этой библиотеки требуют Python 3.7 или выше. Вы используете версию {}."
            enError = "Attention: some methods of this library require Python 3.7 or higher. You are using version {}."

            from warnings import warn
            warn(
                (ruError if self._errorsLanguage == "ru" else enError if self._errorsLanguage == "en" else "🇷🇺: " + ruError + " 🇬🇧: " + enError).format(str(sys.version_info.major) + "." + str(sys.version_info.minor)),
                UserWarning
            )
